...
test('config.js defines shell API allowlist and isShellApiUrl', () => {
  ...
  // Main wording: shell-owned routes stay on Distress OS root (Vault publish must not 404 under /analyzer)
  assert.match(configSrc, /Shell-owned routes stay on Distress OS root|Never rewrite shell Vault/);
});
