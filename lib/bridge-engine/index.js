...
const { loadVaultAddressIndex } = require('../vault-address-index');
...
const {
  scoreTypeColumns,
  pickTypeColumn,
  pickSuggestedTypeColumn
} = require('../bridge-type-column-score');
const { enrichCandidatesWithKeepPreview } = require('../bridge-type-keep-preview');
const { MAX_OCR_PAGES } = require('./parsers/pdf-ocr');


/**
 * Map parse-level OCR / redaction flags onto processUpload processingMeta.
 * Prefer explicit process-meta names; fall back to pdf-ocr.js field names.
 * Batch merge: sum page counts + redactedSkipped; any-file ocrTruncated wins;
 * ocrPageCap is max of file caps (default MAX_OCR_PAGES).
 */
...
function ocrMetaFromParsed(parsed) {
  const p = parsed && typeof parsed === 'object' ? parsed : {};
  const ocrTruncated = Boolean(p.ocrTruncated);
  const hasOcrPageHints =
    ocrTruncated ||
    p.ocrPagesProcessed != null ||
    p.ocrPagesTotal != null ||
    p.ocrTotalPages != null ||
    p.ocrMaxPages != null ||
    p.ocrPageCap != null;

  let ocrPagesProcessed = numberOrNull(p.ocrPagesProcessed);
  if (ocrPagesProcessed == null && hasOcrPageHints) {
    ocrPagesProcessed = numberOrNull(p.pageCount);
  }

  let ocrPagesTotal = numberOrNull(p.ocrPagesTotal);
  if (ocrPagesTotal == null) {
    ocrPagesTotal = numberOrNull(p.ocrTotalPages);
  }

  let ocrPageCap = numberOrNull(p.ocrPageCap);
  if (ocrPageCap == null) ocrPageCap = numberOrNull(p.ocrMaxPages);
  if (ocrPageCap == null) ocrPageCap = MAX_OCR_PAGES;

  const redactedSkipped = numberOrNull(p.redactedSkipped);

  let rotatedBy = null;
  if (p.rotatedBy != null) {
    rotatedBy = p.rotatedBy;
  }
...
/**
 * Partial processingMeta for NO_USABLE_ROWS (and similar) throws.
 * Carries OCR honesty so 422 clients can warn about page-cap truncation
 * even when zero rows were kept. Full success meta is richer; this is the
 * minimum honesty surface when the pipeline aborts after parse.
 */
function partialProcessingMetaFromParsed(parsed, started) {
  const p = parsed && typeof parsed === 'object' ? parsed : {};
  return {
    parser: p.parser || null,
    parseMode: p.parseMode || null,
    sheetName: p.sheetName || null,
    delimiter: p.delimiter || null,
...
function maxNullable(nums, fallback) {
  let max = null;
  for (const n of nums) {
    if (n == null) continue;
    const v = Number(n);
...
function mergeOcrProcessingMeta(results) {
  const list = (results || []).filter(Boolean);
  const metas = list.map((r) => r.processingMeta || {});
  return {
    ocrTruncated: metas.some((m) => Boolean(m.ocrTruncated)),
    ocrPagesProcessed: sumNullable(metas.map((m) => m.ocrPagesProcessed)),
    ocrPagesTotal: sumNullable(metas.map((m) => m.ocrPagesTotal)),
    ocrPageCap: maxNullable(metas.map((m) => m.ocrPageCap), MAX_OCR_PAGES),
    redactedSkipped: sumNullable(metas.map((m) => m.redactedSkipped)),
    // Multi-file: rotation is per-file; surface null on merged meta
    rotatedBy: list.length === 1 ? (metas[0].rotatedBy ?? null) : null
  };
}
...
function noUsableRowsMessage({
  ...
  if (onlyImported) {
    return 'Every record in this file is already in The Vault. Nothing new to scrub — pick a different city file or remove those leads from Vault first.';
  }
...
function mapCandidates(ranked, rows, limit = 8) {
  return enrichCandidatesWithKeepPreview(ranked, rows || [], {
...
function resolveTypeColumnGate({
  ...
  const suggested = pickSuggestedTypeColumn(ranked);
  ...
  if (needConfirm && !hasConfirm) {
    ...
    err.details = {
      ...
      candidates: mapCandidates(ranked, sampleRows),
      suggestedHeader: suggested ? suggested.header : null,
...
async function processUpload(opts = {}) {
  ...
  const fromOcrParsed = Boolean(
    parsed.fromOcr
    || parsed.parser === 'ocr'
    || /ocr/i.test(String(parsed.parseMode || ''))
    || (parsed.ocrConfidence != null && Number.isFinite(Number(parsed.ocrConfidence)))
  );
  const normalizeContext = {
    ...
    sourceFile,
    processedAt,
    ocrConfidence: parsed.ocrConfidence != null ? parsed.ocrConfidence : null,
    fromOcr: fromOcrParsed
  };
  ...
  // Always skip addresses already published to The Vault (Home + Land catalog).
  // Not opt-in — product rule so operators never re-work vaulted leads.
  ...
  try {
    importIndex = loadVaultAddressIndex({ force: true });
    if (importIndex.addresses && importIndex.addresses.size) {
      importFiltered = filterAlreadyImported(deduped.rows, importIndex.addresses);
    }
  } catch (err) {
    console.warn('[Bridge] Vault skip failed (continuing without vault filter):', err.message);
  }
  ...
  if (!kept.length && !fnAll.length) {
    ...
    err.details = {
      ...
      processingMeta: partialProcessingMetaFromParsed(parsed, started)
    };
    throw err;
  }
  ...
  // Full discard arrays can be huge; client only needs counts (stats) + a small sample.
  const DISCARD_SAMPLE_MAX = 40;
  const discardedSample = nonReviewDiscarded.slice(0, DISCARD_SAMPLE_MAX);
  ...
  return {
    ...
    discardedTotal: nonReviewDiscarded.length,
    brainMeta,
    processingMeta: {
...
function mergeProcessResults(results, { city, uploadType } = {}) {
  ...
  if (!list.length) {
    ...
    err.code = 'NO_USABLE_ROWS';
    // No per-file parse ran — cannot attach OCR honesty meta
    err.details = { discarded: [], stats: emptyProcessingStats(), processingMeta: null };
    throw err;
  }
  if (list.length === 1) {
    const only = list[0];
  ...
  return {
    ...
    processingMeta: {
      ...
      // Batch OCR honesty: any-file truncated wins; page counts + redactedSkipped summed
      ...mergeOcrProcessingMeta(list),
      importIndexCount,
      importIndexSources,
      brainVersion,
...
function buildConfirmMaps(context) {
  ...
  // When confirmedFormats[] is present, never apply the bare '*' wildcard —
  // that falsely marks other sheet formats as confirmed after the first
  // dialog, so process fails mid-batch with TYPE_COLUMN_CONFIRM_REQUIRED
  // instead of opening format 2 of N.
  const hasMultiList = Array.isArray(list) && list.length > 0;
...
async function scanBatchFileInfos(entries, context) {
  ...
  for (const file of entries) {
    ...
    const sampleRows = (parsed.rows || []).slice(0, 80);
    const ranked = scoreTypeColumns(headers, sampleRows);
    const fingerprint = computeFormatFingerprint(headers);
    const memory =
...
async function processUploadBatch(fileEntries, context) {
  ...
  for (const file of entries) {
    try {
      ...
      // Opt-in only — never default-on (IND-04 / TEST-01 v2.0)
      // Vault skip is always on inside processUpload — no opt-in flag needed
  ...
  if (!successes.length) {
    ...
    err.details = {
      ...
      // OCR honesty: merge any per-file partial meta so 422 still warns
      processingMeta: (() => {
        const withMeta = failures
          .map((f) => f.details?.processingMeta)
          .filter((m) => m && typeof m === 'object');
        if (!withMeta.length) return first.details?.processingMeta || null;
        if (withMeta.length === 1) return withMeta[0];
        const ocr = mergeOcrProcessingMeta(withMeta.map((m) => ({ processingMeta: m })));
        return {
          parser: withMeta[0].parser || null,
          parseMode: withMeta[0].parseMode || null,
          ...ocr,
          multiFile: true,
