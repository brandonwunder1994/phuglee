...
/**
 * Build fingerprint → confirmed Type header from batch context.
 * Supports multi-format confirmedFormats[] and legacy single confirm fields.
 * @returns {Map<string, string|null>}
 */
function buildConfirmMap(context) {
  const map = new Map();
  if (!context || typeof context !== 'object') return map;

  const list = context.confirmedFormats;
  if (Array.isArray(list)) {
    for (const item of list) {
      if (!item || item.formatFingerprint == null || item.formatFingerprint === '') continue;
      if (!Object.prototype.hasOwnProperty.call(item, 'confirmedTypeHeader')) continue;
      map.set(String(item.formatFingerprint), resolveConfirmedHeader(item.confirmedTypeHeader));
    }
  }
  ...
  if (Object.prototype.hasOwnProperty.call(context, 'confirmedTypeHeader')) {
    ...
    if (context.formatFingerprint) {
      map.set(String(context.formatFingerprint), header);
    } else {
      // Same-fingerprint batch: apply to all files when only one format is present
      map.set('*', header);
    }
  }
  return map;
...
async function preScanBatchTypeGate(entries, context) {
  ...
  const confirmMap = buildConfirmMap(context);
  const fingerprints = [...new Set(fileInfos.map((f) => f.formatFingerprint))];
  const allSameFp = fingerprints.length === 1;

  // Resolve needConfirm against multi-format map + legacy single header for whole batch
  for (const f of fileInfos) {
    if (confirmMap.has(f.formatFingerprint)) {
      f.needsConfirm = false;
    } else if (confirmMap.has('*')) {
      // Legacy: confirmedTypeHeader without fingerprint applies to every sheet
      // (column name must exist on each file or INVALID_TYPE_COLUMN per file).
      f.needsConfirm = false;
    }
  }
  ...
  if (needing.length) {
    ...
    err.details = {
      city,
      uploadType,
      multiFormat: needing.length > 1 || fingerprints.length > 1,
  ...
  return { fileInfos, formats, confirmMap, fingerprints, allSameFp };
}
...
async function processUploadBatch(fileEntries, context) {
  ...
  const confirmMap = (scan && scan.confirmMap) || buildConfirmMap(ctx);
  const fingerprints =
  ...
  for (const file of entries) {
    try {
      ...
      // Per-format confirm first; legacy * applies one header name to every sheet
      if (fp && confirmMap.has(fp)) {
        uploadArgs.confirmedTypeHeader = confirmMap.get(fp);
        uploadArgs.formatFingerprint = fp;
      } else if (confirmMap.has('*')) {
        uploadArgs.confirmedTypeHeader = confirmMap.get('*');
