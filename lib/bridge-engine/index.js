...
function noUsableRowsMessage({
  ...
  if (breakdown) {
    return `No usable records found. Breakdown: ${breakdown}.`;
  }
  return 'No usable records found — every row was missing a valid street address, not a distress signal, or duplicated.';
}
...
function resolveTypeColumnGate({
  ...
  const memory = cityId ? loadCityFormat(cityId, uploadType) : null;
  const memoryMatch =
    Boolean(memory) &&
 ...
 * Mixed fingerprints or shared need-confirm → hard throw (never partial Train merge).
 */
async function preScanBatchTypeGate(entries, context) {
  if (!context || context.uploadType !== 'code_violation') return;
  if (!entries || entries.length <= 1) return;

  const hasConfirm = Object.prototype.hasOwnProperty.call(context, 'confirmedTypeHeader');
  // With admin confirm, each file processes with override; skip aggregate pre-scan
  // (per-file INVALID_TYPE_COLUMN still hard-fails if header missing).
  ...
  for (const file of entries) {
    ...
    const fingerprint = computeFormatFingerprint(headers);
    const memory = city && city.id ? loadCityFormat(city.id, uploadType) : null;
  ...
  if (fingerprints.length > 1) {
    const err = new Error(
      'Mixed file formats in batch — process files separately or confirm each format'
    );
  ...
  // Same fingerprint, need confirm → single 409 payload (shared candidates from first)
  if (fileInfos.some((f) => f.needsConfirm)) {
 ...
 * TYPE_COLUMN_CONFIRM_REQUIRED / FORMAT_MISMATCH / ADMIN_REQUIRED / INVALID_TYPE_COLUMN hard-fail.
 */
async function processUploadBatch(fileEntries, context) {
  ...
  // GATE-06: pre-scan mixed / need-confirm before any full process (no partial Train)
  await preScanBatchTypeGate(entries, context || {});
  ...
  for (const file of entries) {
    try {
      ...
      if (hasConfirm) {
        uploadArgs.confirmedTypeHeader = context.confirmedTypeHeader;
      }
      if (
        context &&
        Object.prototype.hasOwnProperty.call(context, 'formatFingerprint')
      ) {
        uploadArgs.formatFingerprint = context.formatFingerprint;
      }
      ...
      // Hard errors (confirm, format mismatch, unsupported, parse fail, OCR, etc.)
      err.failedFile = file.filename;
      throw err;
    }
  }
