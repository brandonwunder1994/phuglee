...
function rowMatchKeys(row) {
  ...
  // Street-only keys are only used when the row has no city — avoids cross-city false drops.
...
function bucketByHouseNumber(indexList) {
  const buckets = new Map();
  for (const existing of indexList) {
    const num = leadingStreetNumber(existing) || '_';
    if (!buckets.has(num)) buckets.set(num, []);
    buckets.get(num).push(existing);
  }
  return buckets;
}

function matchesImported(row, importedAddresses, indexList, threshold, buckets) {
  ...
  // Fuzzy only within same house-number bucket (or tiny index).
  let candidates = indexList;
  if (buckets && streetNum && buckets.has(streetNum)) {
    candidates = buckets.get(streetNum);
  } else if (buckets && streetNum) {
    candidates = buckets.get('_') || [];
  }
  // Cap fuzzy work per row
...
function filterAlreadyImported(rows, importedAddresses, options = {}) {
  ...
  const buckets = bucketByHouseNumber(indexList);

  for (const row of rows) {
    const matched = matchesImported(row, importedAddresses, indexList, threshold, buckets);
