...
const { needsPdfOcr, ocrPageCapMessage, MAX_OCR_PAGES } = pdfOcr;
...
function parseAoaAsSpreadsheet(aoa, meta = {}) {
  ...
  const parseMode = meta.parseMode || 'table-to-xlsx';
  const ocrConfidence = meta.ocrConfidence != null ? meta.ocrConfidence : null;
  // Family rebuilds drop per-row _meta; surface OCR origin so normalizer
  // can apply DATA-STANDARDS confidence bands instead of street-column high.
  const fromOcr = Boolean(
    meta.fromOcr
    || /ocr/i.test(String(parseMode))
    || (ocrConfidence != null && Number.isFinite(Number(ocrConfidence)))
  );
  return {
    ...parsed,
    parser: 'pdf-xlsx',
...
/**
 * Build structured OCR hard-failure when image-like PDF has no usable embedded text.
 * @param {Error|null} ocrCaught
 * @param {object|null} ocrResult
 * @returns {Error}
 */
function buildOcrHardFailure(ocrCaught, ocrResult) {
  if (ocrCaught) {
    if (ocrCaught.code === 'OCR_UNAVAILABLE' || ocrCaught.code === 'OCR_FAILED') {
      return ocrCaught;
    }
    if (/tesseract|worker|wasm|unavailable/i.test(String(ocrCaught.message || ''))) {
      const err = new Error(
        `OCR is unavailable in this environment. Upload Excel/CSV, or ensure Tesseract can run locally. ${ocrPageCapMessage(MAX_OCR_PAGES)}`
      );
    ...
    const err = new Error(
      `OCR failed on this scanned PDF. Try a clearer scan, or upload Excel/CSV. ${ocrPageCapMessage(MAX_OCR_PAGES)}`
    );
    err.code = 'OCR_FAILED';
    err.cause = ocrCaught;
    return err;
  }
  // null / empty OCR result on image-like packet
  const emptyText = ocrResult && String(ocrResult.text || '').trim() === '';
  const err = new Error(
    emptyText
      ? `OCR ran but could not read usable text from this scanned PDF. Try a clearer scan, or upload Excel/CSV. ${ocrPageCapMessage(MAX_OCR_PAGES)}`
      : `This looks like a scanned (image) PDF and OCR could not run or extract text. Upload Excel/CSV, or ensure Tesseract OCR can run. ${ocrPageCapMessage(MAX_OCR_PAGES)}`
  );
  err.code = emptyText ? 'OCR_FAILED' : 'OCR_UNAVAILABLE';
  return err;
}

/**
 * Snapshot OCR honesty fields from an ocrPdfBuffer result.
 * Stored in outer scope so every return path after OCR can attach them —
 * not only when OCR wins the street-count contest.
 * @param {object|null} ocr
 * @returns {object|null}
 */
function captureOcrHonesty(ocr) {
  if (!ocr || typeof ocr !== 'object') return null;
  const honesty = {
    ocrTruncated: Boolean(ocr.ocrTruncated),
    ocrMaxPages: ocr.ocrMaxPages,
    ocrTotalPages: ocr.ocrTotalPages,
    ocrPageCapNote: ocr.ocrPageCapNote || null
  };
  if (ocr.rotatedBy != null) honesty.rotatedBy = ocr.rotatedBy;
  return honesty;
}

/**
 * Attach OCR honesty fields onto a parse result (mutates when result is object).
 * Call on every return path after OCR may have run so truncation is never silent.
 * @param {object|null} result
 * @param {object|null} honesty
 * @returns {object|null}
 */
function attachOcrHonestyFields(result, honesty) {
  if (!result || !honesty || typeof result !== 'object') return result;
  result.ocrTruncated = Boolean(honesty.ocrTruncated);
  if (honesty.ocrMaxPages != null) result.ocrMaxPages = honesty.ocrMaxPages;
  if (honesty.ocrTotalPages != null) result.ocrTotalPages = honesty.ocrTotalPages;
  if (honesty.ocrPageCapNote != null) result.ocrPageCapNote = honesty.ocrPageCapNote;
  if (honesty.rotatedBy != null) result.rotatedBy = honesty.rotatedBy;
  return result;
}
...
async function parsePdf(buffer, filename) {
  ...
  let fromOcr = false;
  /** @type {ReturnType<typeof captureOcrHonesty>} */
  let ocrHonesty = null;
  const textPages =
  ...
  if (shouldOcr) {
    // Hard-fail only for image-like packets with no usable embedded streets.
    // When embedded text already has streets, OCR remains a soft enhancement.
    const hasUsableEmbedded =
      fromText && countUsableStreets(fromText) >= 1;
    const imageLikeRequired = needsPdfOcr(text, tableAoa) && !hasUsableEmbedded;

    let ocr = null;
    let ocrCaught = null;
    try {
      ocr = await pdfOcr.ocrPdfBuffer(buffer, {
    ...
    const ocrText = ocr && String(ocr.text || '').trim();
    if (!ocrCaught && ocrText) {
      // Capture honesty even if OCR later loses the street contest
      ocrHonesty = captureOcrHonesty(ocr);
      fromOcr = true;
      ...
      if (fromOcrParsed) {
        ...
        if (
          ...
          return attachOcrHonestyFields(fromOcrParsed, ocrHonesty);
        }
      }
    ...
    } else if (imageLikeRequired) {
      // Do not swallow OCR failure into a false empty success for pure image PDFs
      throw buildOcrHardFailure(ocrCaught, ocr);
    }
    // else: soft continue with embedded text (OCR optional enhancement)
  }

  // Prefer text path, but never drop OCR truncation/rotation honesty
  if (fromText && countUsableStreets(fromText) >= 1) {
    return attachOcrHonestyFields(fromText, ocrHonesty);
  }
  if (fromText) return attachOcrHonestyFields(fromText, ocrHonesty);
  ...
  if (text) {
    ...
    if (again) return attachOcrHonestyFields(again, ocrHonesty);
  }
  ...
  if (extractedAoa) {
    try {
      ...
      return attachOcrHonestyFields(parsed, ocrHonesty);
    } catch {
      // fall through
    }
  }

  return attachOcrHonestyFields({
    ...extracted,
    pageCount,
    ocrConfidence,
