...
/**
 * Operator-facing note for the OCR page ceiling.
 * @param {number} [maxPages]
 * @returns {string}
 */
function ocrPageCapMessage(maxPages = MAX_OCR_PAGES) {
  const n = Number(maxPages) > 0 ? Number(maxPages) : MAX_OCR_PAGES;
  return `OCR processes up to ${n} pages per PDF; split longer scans or upload Excel/CSV.`;
}
...
async function ocrPdfBuffer(buffer, opts = {}) {
  ...
  try {
    ...
    const truncated = totalPages > 0 && totalPages > maxPages;
    ...
    return {
      ...
      ocrTotalPages: totalPages || pages.length,
      ocrTruncated: Boolean(truncated),
      ocrPageCapNote: truncated ? ocrPageCapMessage(maxPages) : null
    };
  } finally {
    if (worker) await worker.terminate().catch(() => {});
