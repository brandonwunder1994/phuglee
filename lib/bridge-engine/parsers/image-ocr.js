...
/** Raster formats OCR can ingest (plus HEIC/HEIF for clear convert guidance). */
const IMAGE_EXT_RE = /\.(jpg|jpeg|png|webp|gif|tif|tiff|bmp|heic|heif)$/i;
...
function isHeicFile(filename) {
  return HEIC_EXT_RE.test(String(filename || ''));
}
...
async function parseImageOcr(buffer, filename) {
  ...
  // No sharp/heic-convert in dependencies — accept extension at intake, guide operator.
  if (isHeicFile(filename)) {
    const err = new Error(
      'HEIC/HEIF photos cannot be decoded on the server. Convert to JPG or PNG and upload again.'
    );
  ...
  try {
    ...
    if (err.code === 'OCR_UNAVAILABLE' || err.code === 'UNSUPPORTED_FILE') throw err;
