...
/** Plans a self-serve register may claim. `max` is Vault entitlement — admin-assign only. */
const REGISTERABLE_PLANS = new Set(['lite', 'pro']);

function isRegisterablePlan(plan) {
  return REGISTERABLE_PLANS.has(sanitizePlan(plan));
}

/**
 * open | invite | closed
 * Production defaults to invite when PHUGLEE_REGISTRATION_MODE unset.
 */
function registrationMode() {
  const raw = String(process.env.PHUGLEE_REGISTRATION_MODE || '').trim().toLowerCase();
  if (raw === 'open' || raw === 'invite' || raw === 'closed') return raw;
  const isProd = String(process.env.NODE_ENV || '').toLowerCase() === 'production';
  return isProd ? 'invite' : 'open';
}

function inviteCodeConfigured() {
  return String(process.env.PHUGLEE_INVITE_CODE || '').trim();
}

function inviteCodeMatches(provided) {
  const expected = inviteCodeConfigured();
...
/** Vault-only partner password from PHUGLEE_VAULT_ONLY_PASSWORD (Railway var). */
function vaultOnlyBootstrapPassword() {
  return String(process.env.PHUGLEE_VAULT_ONLY_PASSWORD || '').trim();
}

function isBootstrapVaultOnly(username, password) {
  const key = sanitizeUsername(username);
  const expected = vaultOnlyBootstrapPassword();
  if (!expected || !password || !isVaultOnlyUsername(key)) return false;
  const a = Buffer.from(String(password));
  const b = Buffer.from(expected);
  if (a.length !== b.length) return false;
  return crypto.timingSafeEqual(a, b);
...
/**
 * Persist matt (vault-only) into users.json when PHUGLEE_VAULT_ONLY_PASSWORD is set.
 * Safe to call on every boot — refreshes the hash to match the env password.
 */
function ensureVaultOnlyUser() {
  const password = vaultOnlyBootstrapPassword();
  if (!password) return { ok: false, reason: 'PHUGLEE_VAULT_ONLY_PASSWORD unset' };
  const users = loadUsers();
  const role = roleForUsername(VAULT_ONLY_USERNAME);
  const prev = users[VAULT_ONLY_USERNAME] || {};
  users[VAULT_ONLY_USERNAME] = {
    username: VAULT_ONLY_USERNAME,
    password: hashPassword(password),
    fullName: prev.fullName || role.fullName || 'Matt',
    email: prev.email || 'matt@phuglee.com',
    plan: 'max',
    createdAt: prev.createdAt || Date.now(),
    updatedAt: Date.now()
  };
  saveUsers(users);
  return { ok: true, username: VAULT_ONLY_USERNAME, file: credentialsPath() };
}
...
function authenticateUser(username, password) {
  ...
  if (isBootstrapVaultOnly(username, password)) {
    const role = roleForUsername(VAULT_ONLY_USERNAME);
    return {
      ok: true,
      username: VAULT_ONLY_USERNAME,
      plan: role.plan,
      fullName: role.fullName || 'Matt',
      email: role.email || '',
      role: role.id
    };
  }
  ...
  const resolvedUser = sanitizeUsername(record.username || key);
  const role = roleForUsername(resolvedUser);
  return {
    ok: true,
    username: resolvedUser,
    plan: role.plan || sanitizePlan(record.plan || 'lite'),
    fullName: record.fullName || role.fullName || '',
    email: record.email || role.email || '',
    role: role.id
  };
}
 ...
 */
function registerUser({ username, password, plan, email, fullName, inviteCode } = {}) {
  const key = sanitizeUsername(username);
  ...
  if (mode === 'closed') {
    return { ok: false, error: 'Registration is closed', code: 'REGISTRATION_CLOSED' };
  }
  if (mode === 'invite') {
    if (!inviteCodeConfigured()) {
      return {
        ok: false,
        error: 'Registration requires an invite code (server not configured)',
        code: 'INVITE_NOT_CONFIGURED'
      };
    }
    if (!inviteCodeMatches(inviteCode)) {
      return { ok: false, error: 'Invalid invite code', code: 'INVITE_INVALID' };
    }
  }
  ...
  if (key === ADMIN_USERNAME || key === DISPOS_USERNAME || key === VAULT_ONLY_USERNAME) {
  ...
  if (!isRegisterablePlan(tier)) {
    return {
      ok: false,
      error: 'Choose Lite or Pro. Max is assigned by an admin.',
...
module.exports = {
  ...
  registerUser,
  ensureVaultOnlyUser,
  vaultOnlyBootstrapPassword,
  isBootstrapVaultOnly,
  loadUsers,
  sanitizeUsername,
  sanitizePlan,
  isRegisterablePlan,
