...
function ruleTypeKey(rule) {
  if (rule.violationTypeKey && String(rule.violationTypeKey).trim()) {
    return stableTypeKey(rule.violationTypeKey);
  }
  if (rule.violationTypeLabel) {
    return stableTypeKey(rule.violationTypeLabel);
  }
  return null;
}
...
function typeRuleMatches(rule, row) {
  ...
  if (!key || key === '__unknown__') return false;
  return stableTypeKey(row.violationIssueType) === key;
}
 ...
 * Other upload types: return shallow copy unchanged (list type is the signal).
 */
