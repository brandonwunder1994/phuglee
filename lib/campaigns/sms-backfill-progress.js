...
/**
 * Recovery floor when volume/checkpoint were wiped but CRM was already tagged.
 * Override with SMS_BACKFILL_SEED_FLOOR (set 0 to disable). Default 10000 from
 * the July 2026 vault→GHL phuglee backfill (10k+ contacts tagged).
 */
function progressSeedFloor() {
  if (process.env.SMS_BACKFILL_SEED_FLOOR === '0') return 0;
  if (process.env.SMS_BACKFILL_SEED_FLOOR != null && process.env.SMS_BACKFILL_SEED_FLOOR !== '') {
    const n = Number(process.env.SMS_BACKFILL_SEED_FLOOR);
 ...
 *
 * Important: GHL total of 0 / rate-limit failures must NOT clear the bar.
 */
function getBackfillProgress({ ghlPhugleeTotal = null, ghlError = null } = {}) {
  ...
  // Only positive totals count as live GHL evidence (0 is often a rate-limit lie)
  const ghlRaw = ghlPhugleeTotal != null && Number.isFinite(Number(ghlPhugleeTotal))
    ? Number(ghlPhugleeTotal)
    : null;
  const ghlN = ghlRaw != null && ghlRaw > 0 ? ghlRaw : null;

  const seed = progressSeedFloor();
  ...
  // Never lose progress: best known processed across durable sources + positive GHL + seed
  ...
  let tagged = Math.max(
    ...
    nOr0(hw && hw.tagged),
    ghlN != null ? ghlN : 0,
    // Keep tagged near processed when only the seed floor is available
    seed
  );
  ...
  let source = 'none';
  if (fromCp && fromCp.processed > 0 && fromCp.processed === processed) source = 'checkpoint';
  else if (ghlN != null && ghlN === processed) source = 'ghl-live';
  else if (hw && nOr0(hw.processed) === processed) source = 'highwater';
  else if (slim && nOr0(slim.processed) === processed) source = 'progress-file';
  else if (seed > 0 && processed === seed && !fromCp && !ghlN) source = 'seed-recovery';
  else if (processed > 0) source = 'merged';

  if (ghlN != null && ghlN > 0 && (!fromCp || fromCp.processed < ghlN) && processed === ghlN) {
    source = fromCp ? 'ghl-live+checkpoint' : 'ghl-live';
  }
  ...
  if (total <= 0 && ghlN != null && ghlN > 0) {
    percent = 100;
    processed = Math.max(processed, ghlN);
  ...
  const lastAt =
    ...
    || (slim && slim.updatedAt)
    || (hw && hw.updatedAt)
    || (ghlN != null || processed > 0 ? new Date().toISOString() : null);

  // Persist high-water only when we have real positive progress (never write zeros)
  if (processed > 0 || tagged > 0) {
    persistProgressHighWater({
      ...
      total,
      vaultTotal: total,
      // only store positive GHL totals
