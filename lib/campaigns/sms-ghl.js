...
function readSearchTotal(data) {
  if (!data || typeof data !== 'object') return null;
  const candidates = [
    data.total,
    data.meta && data.meta.total,
    data.meta && data.meta.count,
    data.count
  ];
...
/**
 * Cheap single-page tag count for progress bars.
 * Throws after limited 429 retries — never fabricates total:0 on failure.
 */
async function countContactsByTag(tag, d = {}) {
  const { ghl } = deps(d);
  ...
  for (let attempt = 0; attempt < 5; attempt++) {
    try {
      ...
      const data = await ghl.api('POST', '/contacts/search', {
        locationId,
        pageLimit: 1,
        page: 1,
        filters: [{ field: 'tags', operator: 'contains', value: tag }]
      });
      const total = readSearchTotal(data);
      ...
      lastErr = err;
      if (err && err.status === 429) continue;
      throw err;
    }
  }
  const err = lastErr || new Error('GHL tag count failed after retries');
  err.code = err.code || 'GHL_TAG_COUNT_FAILED';
  throw err;
}
...
async function searchContactsByTag(tag, { pageLimit = 100, searchAfter = null, maxPages = 40 } = {}, d = {}) {
  ...
  while (pages < maxPages) {
    ...
    try {
      ...
      if (err.status === 429) {
        consecutive429 += 1;
        if (consecutive429 >= 4) {
          // Do not return total:0 after rate-limit thrash — callers treat 0 as “none tagged”
          const e = new Error('GHL rate limited while listing tagged contacts');
          e.code = 'GHL_RATE_LIMIT';
          e.status = 429;
          throw e;
        }
        await sleep(Math.min(30000, 3000 * consecutive429));
        pages -= 1; // retry same logical page
        continue;
      }
      throw err;
    }
    if (total == null) {
      const t = readSearchTotal(data);
      total = t != null ? t : null;
    }
    const contacts = data.contacts || [];
    all.push(...contacts);
  ...
  return { contacts: all, total: total != null ? total : all.length };
}
