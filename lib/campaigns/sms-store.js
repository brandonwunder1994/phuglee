...
/**
 * Durable campaigns SMS root (checkpoint, progress, KPI snapshot, runs).
 * Prefer explicit CAMPAIGNS_SMS_DATA_ROOT, else nest under PDA_DATA_ROOT (Railway
 * volume) so redeploys never wipe 10k+ backfill progress.
 */
function dataRoot() {
  ...
  if (process.env.PDA_DATA_ROOT) {
    return path.join(path.resolve(process.env.PDA_DATA_ROOT), 'campaigns', 'sms');
  }
