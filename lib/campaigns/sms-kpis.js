...
async function fetchOverviewKpis(d = {}, opts = {}) {
  ...
  if (snap && snap.outcomes) {
    ...
    // Do NOT auto-crawl GHL on every stale load — that rate-limits the API and
    // zeros the progress bar. Force refresh only via ?force=1 / explicit action.
    const sent = sentFromRuns();
    return {
      scope: snap.scope || PHUGLEE_TAG,
  ...
  // Cold start: instant placeholder. Full crawl only when force=true.
  return placeholderKpis({
    note: 'KPIs not computed yet. Use Refresh KPIs (slow GHL walk) when rate limit allows.'
  });
}
