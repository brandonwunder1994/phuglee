...
const { countContactsByTag } = require('./sms-ghl');
const { retagPhugleeDncSplit, handleGhlSmsTagWebhook } = require('./sms-auto-tag');

/** One cheap GHL page for phuglee total — never treats rate-limit as zero tags. */
async function fetchGhlPhugleeTotalSafe() {
  try {
    const snap = await countContactsByTag(PHUGLEE_TAG);
    if (snap.unknownTotal || snap.total == null) {
      return { total: null, error: null };
    }
    // Positive only; 0 may be real empty account — still ok to report
    return { total: Number(snap.total), error: null };
  } catch (err) {
    return {
      total: null,
      error: err && err.message ? String(err.message).slice(0, 180) : 'GHL tag count failed'
    };
  }
}
...
async function handle(req, res, pathname, url) {
  ...
  try {
    ...
    if (req.method === 'GET' && pathname === '/api/admin/campaigns/sms/overview') {
      ...
      const forceKpis = url.searchParams.get('force') === '1';
      // Live GHL phuglee total (one page). Rate-limit → null, not 0.
      const ghlSnap = await fetchGhlPhugleeTotalSafe();
      const backfillProgress = getBackfillProgress({
        ghlPhugleeTotal: ghlSnap.total,
        ghlError: ghlSnap.error
      });
      // KPIs: snapshot / memory first (never block the desk on full GHL crawl)
      const kpis = await fetchOverviewKpis({}, { force: forceKpis }).catch((err) => ({
        error: err.message,
        outcomes: {},
        funnel: {},
        cached: true,
    ...
    // Lightweight progress poll without full KPI recompute
