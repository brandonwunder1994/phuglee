...
async function handle(req, res, pathname, url) {
  ...
  try {
    ...
    if (req.method === 'GET' && pathname === '/api/admin/campaigns/sms/overview') {
      ...
      // Live GHL phuglee total for progress bar (first page only — cheap).
      // Failures are surfaced; high-water still recovers last known counts.
      ...
      try {
        ...
        ghlError = err && err.message ? String(err.message).slice(0, 180) : 'GHL search failed';
      }
      const backfillProgress = getBackfillProgress({ ghlPhugleeTotal, ghlError });
    ...
    if (req.method === 'GET' && pathname === '/api/admin/campaigns/sms/backfill-progress') {
      ...
      sendJson(res, 200, {
        ok: true,
        backfillProgress: getBackfillProgress({ ghlPhugleeTotal, ghlError })
      });
      return true;
    }
