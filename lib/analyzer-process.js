...
function startAnalyzerProcess() {
  ...
  // Force the child to use the same bearer the shell proxy injects.
  try {
    const { getPdaAuthToken, clearPdaAuthTokenCache } = require('./pda-auth-token');
    clearPdaAuthTokenCache();
    const tok = getPdaAuthToken();
    if (tok) env.PDA_AUTH_TOKEN = tok;
  } catch (_) {}

  // Detached + unref so analyzer survives when the parent shell/Job Object ends.
  // (Agent shells often kill non-detached children when the spawn command finishes.)
  ...
  analyzerChild.on('exit', (code, signal) => {
    console.warn('[Property Analyzer] exited', { code, signal });
    analyzerChild = null;
    // Auto-restart with backoff (mirrors Forge) unless external entrypoint owns lifecycle.
    if (process.env.FORGE_EXTERNAL_BOOT === '1') return;
    const delayMs = 3000;
    setTimeout(() => {
      try {
        if (!analyzerChild) {
          console.log('[Property Analyzer] restarting after exit…');
          startAnalyzerProcess();
        }
      } catch (err) {
        console.error('[Property Analyzer] restart failed:', err.message);
      }
    }, delayMs);
  });
  try { analyzerChild.unref(); } catch (_) {}
