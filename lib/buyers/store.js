...
function writeCatalog(catalog) {
  ...
  // Keep public seed mirror in sync for local static fallback / git
  try {
    const publicMirror = path.join(config.PUBLIC, 'data', 'buyers', 'catalog.json');
    const repoMirror = path.join(config.ROOT, 'data', 'buyers', 'catalog.json');
    writeJsonAtomic(publicMirror, next);
    // Only rewrite repo seed when BUYERS_ROOT is the default data/buyers
    if (path.resolve(rootDir()) === path.resolve(path.join(config.ROOT, 'data', 'buyers'))) {
      // already wrote via catalogPath
    } else if (fs.existsSync(path.dirname(repoMirror))) {
      writeJsonAtomic(repoMirror, next);
    }
  } catch (err) {
    console.warn('[buyers] mirror sync failed:', err.message);
  }
  return next;
}
