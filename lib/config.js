...
module.exports = {
  ...
  /**
   * Geocodio clean jobs + usage ledger.
   * Prefer GEOCODIO_ROOT; else nest under PDA_DATA_ROOT; else data/geocodio.
   */
  GEOCODIO_ROOT: process.env.GEOCODIO_ROOT
    ? path.resolve(process.env.GEOCODIO_ROOT)
    : (process.env.PDA_DATA_ROOT
      ? path.join(path.resolve(process.env.PDA_DATA_ROOT), 'geocodio')
      : path.join(ROOT, 'data', 'geocodio')),
