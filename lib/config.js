...
const PHUGLEE_ROUTES = {
  ...
  '/buyers': 'buyers.html',
  '/trust-funds': 'buyers.html', // legacy alias → Buyers desk
...
module.exports = {
  loopbackHost,
  // Prefer PHUGLEE_*; fall back to legacy DISTRESS_OS_* then Railway PORT.
  PORT: Number(process.env.PHUGLEE_PORT || process.env.DISTRESS_OS_PORT || process.env.PORT || 3000),
  // Local default: omit bind host so Node accepts IPv4 + IPv6 (Windows "localhost" often hits ::1 first).
  // Production/Railway: PORT is set → bind 0.0.0.0. Override with PHUGLEE_HOST (or legacy DISTRESS_OS_HOST).
  HOST: process.env.PHUGLEE_HOST || process.env.DISTRESS_OS_HOST || (process.env.PORT ? '0.0.0.0' : ''),
  ROOT,
  ...
  PHUGLEE_ROUTES,
  /** @deprecated Use PHUGLEE_ROUTES — kept as alias during rename transition. */
  DISTRESS_ROUTES: PHUGLEE_ROUTES,
