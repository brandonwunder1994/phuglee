...
const { UPLOAD_TYPES, usesDistressPhraseFilter } = require('./bridge-intake-schema');
const { classifyViolationRow } = require('./bridge-violation-categories');
const {
  detectLandScrubTags,
  preferLandPath
} = require('./leads-platform/land/filter-route');
...
const INDICATOR_CATEGORIES = Object.freeze([
  ...
  },
  {
    ...
    patterns: [
      // junk + junked (cities often label "Junked Lot" — \bjunk\b alone misses "junked")
      /\b(trash|trashy|junk|junked|debris|rubbish|solid\s+waste|refuse|litter)\b/i,
      /\b(accumulation|accumulated|excessive|improper\s+storage)\b.{0,40}\b(trash|junk|junked|debris|rubbish|waste)\b/i,
      ...
      // City labels: "Junked Lot", "Junked-Lot", "Weedy Junked Lot" (weedy also hits vegetation)
      /\bjunked[-\s]?lots?\b/i,
...
function withLandRouteFields(base, row, uploadType) {
  const scrubTags = detectLandScrubTags({ uploadType, row });
  return {
    ...base,
    landScrubTags: scrubTags,
    preferLandPath: preferLandPath({ uploadType, scrubTags, row })
  };
}
...
function tagRow(row, uploadType, rawRow) {
  ...
  // List-type signal lists (water, pre-lien, tax, etc.): tag once, no phrase filter
  if (!usesDistressPhraseFilter(uploadType)) {
    return withLandRouteFields({
 ...
 * Other upload types (water, pre-lien, tax, …) pass through unchanged.
 */
