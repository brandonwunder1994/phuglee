...
function wrapResponseForRewrite(res) {
  ...
  const flush = () => {
    ...
    if (isText && rewriter) {
      ...
      delete headers['Content-Length'];
      // Keep rewritten JS/HTML out of long browser caches (rewrite can change without file mtime).
      if (/javascript|text\/html/i.test(String(contentType))) {
        for (const k of Object.keys(headers)) {
          if (/^(cache-control|etag|last-modified|expires)$/i.test(k)) delete headers[k];
        }
