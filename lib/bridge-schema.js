...
function findColumn(headers, aliases) {
  const normalized = headers.map((h) => ({ original: h, lower: normalizeHeader(h) }));
  for (const alias of aliases) {
    const match = normalized.find((h) => h.lower === alias);
    if (match) return match.original;
  }
  for (const alias of aliases) {
    const match = normalized.find((h) => h.lower.includes(alias) || alias.includes(h.lower));
    if (match) return match.original;
  }
  return null;
}
...
function detectColumnMap(headers) {
  ...
  for (const key of COLUMN_KEYS) {
    map[key] = findColumn(headers, PDA_COLUMNS[key].aliases);
  }
  return map;
}
