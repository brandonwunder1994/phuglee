...
function resolveForgeTarget(pathname) {
  ...
  return new URL(`http://${config.FORGE_HOST}:${config.FORGE_PORT}${pathname}`);
}
...
function canUseBundledFallback() {
  return runtime.isVercel() || process.env.FORGE_BUNDLED_FALLBACK === '1';
}
...
async function fetchForgeJson(pathname) {
  try {
    return await requestForge('GET', pathname);
  } catch (err) {
