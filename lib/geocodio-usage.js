...
/**
 * Mask an API key for client display (last 4 only). Never return the full secret.
 * @param {string} key
 * @returns {string}
 */
...
/**
 * Modal payload for operators. Full API keys stay server-side only.
 * @param {{ root?: string, env?: NodeJS.ProcessEnv, now?: Date }} [opts]
 */
