function createRewriter({ prefix, targetHost, targetPort }) {
  // Distress OS shell APIs live at site root. Prefixing them under /analyzer
  ...
  function rewriteTextBody(body, contentType = '') {
    ...
    // Keep shell APIs unprefixed (Vault/health/auth live on Distress OS root).
    const rewriteTemplateCall = (fnName) => {
      const re = new RegExp(`\\b${fnName}\\(\`\\/(?!\\/)`, 'g');
      out = out.replace(re, (match, offset, full) => {
        // Peek at the path start after the opening `/
    ...
    if (type.includes('text/html')) {
      ...
      const shellHead = prefix === '/analyzer'
        ? [
          ...
          '<link rel="stylesheet" href="/css/shell-nav.css?v=14-logo-center">',
      ...
      // Analyzer gets a stronger grit plate; forge and other modules stay subtle.
      const premiumBgClass = prefix === '/analyzer'
        ? 'premium-bg premium-bg--medium analyze-premium-bg'
        : 'premium-bg premium-bg--subtle';
      const shellBody = [
        `<div class="${premiumBgClass}" aria-hidden="true"><div class="premium-bg-photo"></div><div class="premium-bg-grain"></div><div class="premium-bg-wear"></div></div>`,
        ...
        '<script src="/js/shell-nav.js?v=29-logo-center" defer></script>',
