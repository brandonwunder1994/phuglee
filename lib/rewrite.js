/** Shell-owned API paths that must stay on Distress OS root when proxying /analyzer. */
const SHELL_API_PREFIXES = ['/api/leads', '/api/health', '/api/auth', '/api/me'];

function isShellApiPath(pathWithOptionalQuery) {
  const pathOnly = String(pathWithOptionalQuery || '').split('?')[0].split('${')[0];
...
function createRewriter({ prefix, targetHost, targetPort }) {
  ...
  /** Analyzer only: leave shell vault/auth/health fetches unprefixed. */
  function shouldSkipShellApi(pathValue) {
    return prefix === '/analyzer' && isShellApiPath(pathValue);
  }
  ...
  function rewriteTextBody(body, contentType = '') {
    ...
    out = out.replace(
      ...
      (match, quote, rest) => {
        if (alreadyPrefixed(`/${rest}`)) return match;
        if (shouldSkipShellApi(`/${rest}`)) return match;
    ...
    out = out.replace(
      /\bfetch\(`(\/(?!\/)[^`]*)/g,
      (match, pathPart) => {
        const pathOnly = String(pathPart).split('?')[0].split('${')[0];
        if (alreadyPrefixed(pathOnly)) return match;
        if (shouldSkipShellApi(pathOnly)) return match;
    ...
    out = out.replace(
      ...
      (match, pathPart) => {
        ...
        if (alreadyPrefixed(pathOnly)) return match;
        if (shouldSkipShellApi(pathOnly)) return match;
        return `apiFetch(\`${prefix}${pathPart.startsWith('/') ? pathPart : `/${pathPart}`}`;
      }
    );
    ...
    out = out.replace(
      ...
      (match, pathPart) => {
        ...
        if (alreadyPrefixed(pathOnly)) return match;
        if (shouldSkipShellApi(pathOnly)) return match;
        return `postJson(\`${prefix}${pathPart.startsWith('/') ? pathPart : `/${pathPart}`}`;
      }
    );
    ...
    out = out.replace(
      ...
      (match, pathPart) => {
        ...
        if (alreadyPrefixed(pathOnly)) return match;
        if (shouldSkipShellApi(pathOnly)) return match;
        return `postForm(\`${prefix}${pathPart.startsWith('/') ? pathPart : `/${pathPart}`}`;
      }
    );

    // return `/api/...` — skip shell APIs under analyzer
    out = out.replace(
      /return `(\/api\/[^`]*)/g,
      (match, pathPart) => {
        const pathOnly = String(pathPart).split('?')[0].split('${')[0];
        if (alreadyPrefixed(pathOnly)) return match;
        if (shouldSkipShellApi(pathOnly)) return match;
        return `return \`${prefix}${pathPart.startsWith('/') ? pathPart : `/${pathPart}`}`;
      }
    );
    ...
    if (type.includes('text/html')) {
      ...
      const shellHead = prefix === '/analyzer'
        ? [
          ...
          '<link rel="stylesheet" href="/css/shell-bundle.css?v=15-polish">',
          '<link rel="stylesheet" href="/css/phuglee-shell.css?v=2-analyze-bg">',
          '<link rel="stylesheet" href="/css/distress-analyzer-os.css?v=distill2-shell">'
        ].join('')
        : [
          '<script src="/js/theme.js?v=distress2"></script>',
      ...
      const shellBody = prefix === '/analyzer'
        ? [
          '<div class="phuglee-shell-bg phuglee-shell-bg--medium analyze-shell-bg" aria-hidden="true"></div>',
          ...
          '<script src="/js/settings-menu.js?v=5" defer></script>',
          '<script src="/js/command-palette.js?v=4" defer></script>',
          '<script src="/js/distress-status.js?v=distress3" defer></script>',
          '<script src="/js/shell-nav.js?v=36-clarify-ia" defer></script>',
      ...
      out = out.replace(
        /<head([^>]*)>/i,
        `<head$1><script src="/js/auth-session.js" defer></script><script src="/js/auth-config.js" defer></script><script src="/js/auth-guard.js" defer></script><script src="/js/phuglee-session-headers.js" defer></script><script>window.__DISTRESS_OS_MODULE_PREFIX__=${JSON.stringify(prefix)};</script>${shellHead}`
      );
