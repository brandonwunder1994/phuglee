  ...
  function rewriteTextBody(body, contentType = '') {
    ...
    if (type.includes('text/html')) {
      const shellHead = [
        ...
        '<link rel="stylesheet" href="/css/shell-nav.css?v=distress2">',
      ...
      const shellBody = [
        ...
        '<script src="/js/shell-nav.js?v=distress2" defer></script>',
        '<script src="/js/shell.js" defer></script>'
      ].join('');

      out = out.replace(
        /<head([^>]*)>/i,
        `<head$1><script src="/js/auth-config.js"></script><script src="/js/auth-guard.js"></script><script>window.__DISTRESS_OS_MODULE_PREFIX__=${JSON.stringify(prefix)};</script>${shellHead}`
      );
