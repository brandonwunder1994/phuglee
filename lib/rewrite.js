  ...
  function rewriteTextBody(body, contentType = '') {
    ...
    if (type.includes('text/html')) {
      const shellHead = [
        ...
        '<link rel="stylesheet" href="/css/shell-nav.css">'
      ].join('');
      ...
      const shellBody = [
        ...
        '<script src="/js/shell-nav.js" defer></script>',
        '<script src="/js/shell.js" defer></script>'
      ].join('');

      out = out.replace(
        /<head([^>]*)>/i,
        `<head$1><script>window.__DISTRESS_OS_MODULE_PREFIX__=${JSON.stringify(prefix)};</script>${shellHead}`
      );

      out = out.replace(/<body([^>]*)>/i, (match, attrs) => {
        const embedClass = 'distress-os-embedded';
