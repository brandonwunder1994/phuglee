...
async function handleProcess(req, res) {
  ...
  for (const file of fileList) {
    ...
    if (!isAcceptedFile(file.filename)) {
      sendJson(res, 400, {
        error: `Unsupported file type: ${file.filename}. Upload Excel, CSV, PDF, Word, TXT, or list images (JPG, PNG, WebP, GIF, TIF, BMP; HEIC may need convert to JPG/PNG).`,
  ...
  try {
    ...
    // Vault address skip is always applied inside processUpload (no client flag).
    const batchArgs = {
      city: cityPayload,
      uploadType,
      username,
...
function extractNoUsableListOutcomes(requests) {
  ...
  const out = [];
  // Forge stores water_shutoff; Filter surfaces water_shut_off
  for (const [forgeType, filterType] of [
    ['code_violation', 'code_violation'],
    ['water_shutoff', 'water_shut_off']
  ]) {
    const req = reqs[forgeType];
...
// Filter canonical id is water_shut_off (matches uploadType). Legacy water_shutoff accepted.
const CITY_OUTCOME_REQUEST_TYPES = new Set(['code_violation', 'water_shut_off', 'water_shutoff']);

/** Map Filter outcome request_type → Form Forge request_type key. */
function toForgeOutcomeRequestType(requestType) {
  if (requestType === 'water_shut_off') return 'water_shutoff';
  return requestType;
}

/** Normalize legacy forge/spelling to Filter surface id. */
function toFilterOutcomeRequestType(requestType) {
  if (requestType === 'water_shutoff') return 'water_shut_off';
  return requestType;
}
...
async function handleCityOutcome(req, res) {
  ...
  const filterRequestType = toFilterOutcomeRequestType(requestType);
  const forgeRequestType = toForgeOutcomeRequestType(filterRequestType);
  ...
  try {
    ...
    sendJson(res, 200, {
      ...
      response_status: responseStatus,
      request_type: filterRequestType
    });
  } catch (err) {
