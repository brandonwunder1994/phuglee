...
const { pushRowsToAnalyzer } = require('./bridge-analyzer-push');
...
function fetchForgeJson(pathname) {
  return new Promise((resolve, reject) => {
      ...
      (res) => {
        ...
        res.on('end', () => {
          ...
          if (res.statusCode < 200 || res.statusCode >= 300) {
            reject(new Error(`Form Forge request failed (${res.statusCode})`));
            return;
          }
          try {
...
function postForgeJson(pathname, payload) {
    ...
    const req = http.request(
      ...
      (res) => {
        ...
        res.on('end', () => {
          ...
          if (res.statusCode < 200 || res.statusCode >= 300) {
            reject(new Error(data.error || `Form Forge request failed (${res.statusCode})`));
            return;
          }
          resolve(data);
        });
...
async function handleProcess(req, res) {
  ...
  try {
    ...
    let analyzerPush = { ok: false, added: 0, skipped: 0, mode: 'none' };
    if (payload.rows?.length) {
      try {
        analyzerPush = await pushRowsToAnalyzer(payload.rows, {
          uploadType,
          sourceFile: payload.sourceFile
        });
      } catch (err) {
        console.warn('[Bridge] Analyzer push failed:', err.message);
        analyzerPush = { ok: false, error: err.message, added: 0, skipped: 0, mode: 'failed' };
      }
    }

    sendJson(res, 200, { ...payload, analyzerPush });
  } catch (err) {
