...
const CITY_OUTCOME_STATUSES = new Set(['needs_clarification', 'no', 'other_source']);
...
async function handleCityOutcome(req, res) {
  ...
  if (!CITY_OUTCOME_STATUSES.has(responseStatus)) {
    sendJson(res, 400, {
      error: 'response_status must be needs_clarification, no, or other_source',
      code: 'INVALID_STATUS'
    });
    return;
  }
...
async function handleBrainMetrics(req, res) {
  ...
  const brain = loadBrain();
  sendJson(res, 200, recomputeMetrics(brain));
}
