...
function rowIdsPresentInList(rows, rowIds) {
  ...
  const set = new Set((rows || []).map((r) => r && r.rowId).filter(Boolean));
  return rowIds.some((id) => set.has(id));
}
...
async function handleBrainDecision(req, res) {
  ...
  // Mutating paths require at least one matching rowId in the target list
...
function recountBrainMetrics(brain) {
  ...
  brain.metrics.typeRulesActive = typeRules.filter((r) => r && r.status === 'active').length;
  brain.metrics.phraseRulesActive = phraseRules.filter((r) => r && r.status === 'active').length;
  brain.metrics.phraseRulesProposed = phraseRules.filter((r) => r && r.status === 'proposed').length;
...
async function handleBrainRuleStatus(req, res, ruleId) {
  ...
  brain.updatedAt = rule.reviewedAt;
  recountBrainMetrics(brain);
  saveBrain(brain);
