...
function readAnalyzerAuthToken() {
  const file = path.join(config.ANALYZER_PATH, AUTH_FILE);
...
function readSessionFromDisk() {
  const file = path.join(config.ANALYZER_PATH, SESSION_FILE);
...
function appendRecordsLocally(session, records, meta) {
  ...
  const merged = {
    ...
    savedAt: Date.now(),
    fileName: meta.sourceFile || base.fileName || 'Data Bridge import',
...
function postAnalyzerJson(pathname, payload) {
...
async function pushRowsToAnalyzer(rows, meta = {}) {
  ...
  try {
    const result = await postAnalyzerJson('/api/bridge-import-records', payload);
    await loadImportAddressIndex({ force: true });
