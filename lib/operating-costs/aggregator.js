...
async function buildOperatingCostsSnapshot(periodArg) {
  ...
  const geminiPrepaidCreditUsd =
    rateCard.geminiPrepaidCreditUsd != null ? money(rateCard.geminiPrepaidCreditUsd) : null;
  const geminiPrepaidCredit = {
    source: 'manual',
    note:
      'AI Studio prepaid Credit balance (aistudio.google.com/billing). Separate from Google Cloud promo credits and from HighLevel wallet balance. Paste the Credit balance number when you top up.',
    balanceUsd: geminiPrepaidCreditUsd,
  ...
  const services = {
    ...
    gemini: {
      id: 'gemini',
      label: 'Google Gemini',
      provider: 'Google AI Studio (prepaid)',
      status: analyzer.ok ? 'live' : 'error',
      amountUsd: geminiUsd,
      prepaidCreditUsd: geminiPrepaidCreditUsd,
      detail: analyzer.ok
        ? geminiPrepaidCreditUsd != null
          ? `${label} · ${geminiCalls} calls · AI Studio credit left $${geminiPrepaidCreditUsd.toFixed(2)}`
          : `${label} · ${geminiCalls} calls · set AI Studio credit balance below`
        : analyzer.error || 'Ledger unavailable',
      billing: portals.gemini,
      meta: analyzer.ok
        ? { ...analyzer.gemini, prepaidCredit: geminiPrepaidCredit }
        : { prepaidCredit: geminiPrepaidCredit }
    }
  };
