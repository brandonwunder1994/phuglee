...
function load() {
  try {
    ...
    const state = { ...emptyState(), ...raw, watermark: { ...emptyState().watermark, ...(raw.watermark || {}) } };
...
function rebuildWatermarkFromCharges(state) {
  ...
  state.watermark.pickUpHint =
    `Last charge on file: ${when}` +
    (last.description ? ` — ${String(last.description).slice(0, 80)}` : '') +
    `. Next export: include ${last.date} onward. Charges already stored are skipped; new ones that day are added.`;
}

/**
 * Import one or more GHL export buffers (max 5). Dedupes by chargeKey so re-exports
 * of the same day only add *new* charges.
 * @param {{ data: Buffer, filename: string }[]} files
 */
function importGhlExports(files) {
  ...
  for (const file of list) {
    ...
    let parsed;
    try {
      parsed = parseGhlExport(file.data, filename);
    } catch (err) {
      fileResults.push({
        ok: false,
        filename,
...
function summarizeByKind(charges) {
  ...
  return Object.values(out)
    .map((x) => ({
      ...x,
      label: x.kind === 'invoice' ? 'Invoices' : x.kind === 'transactions' ? 'Transactions' : 'Other',
