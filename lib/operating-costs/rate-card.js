...
const DEFAULTS = {
  ...
  // Google AI Studio / Gemini API prepaid credit balance (aistudio.google.com/billing).
  // Separate from Cloud promo credits — paste from AI Studio → Billing → Credit balance.
...
function readRateCard() {
  try {
    ...
    return {
      ...
      geminiPrepaidCreditUsd: optionalMoney(raw.geminiPrepaidCreditUsd, null)
    };
  } catch (_) {
    return { ...DEFAULTS };
  }
...
function writeRateCard(patch = {}) {
  ...
  const next = {
    ...
    geminiPrepaidCreditUsd:
      patch.geminiPrepaidCreditUsd !== undefined
        ? optionalMoney(patch.geminiPrepaidCreditUsd, null)
        : current.geminiPrepaidCreditUsd,
    ghlPlanName:
