...
async function handle(req, res, pathname, url) {
  ...
  if (pathname === '/api/admin/operating-costs/rate-card' && req.method === 'PATCH') {
    ...
    const rateCard = writeRateCard({
      ...
      geminiPrepaidCreditUsd: body.geminiPrepaidCreditUsd,
      notes: body.notes
    });
    ...
    try {
      ...
      if (contentType.includes('application/json')) {
        ...
        try {
          ...
          sendJson(res, 400, { ok: false, error: 'Invalid JSON body', code: 'INVALID_BODY' });
          return true;
        }
        const list = Array.isArray(body?.files) ? body.files : body?.file ? [body.file] : [];
        uploads = list
          .map((f) => {
            if (!f) return null;
            const filename = String(f.filename || f.name || 'ghl-export.csv');
            const b64 = String(f.dataBase64 || f.base64 || '').replace(/\s/g, '');
            if (!b64) return null;
            try {
              return { filename, data: Buffer.from(b64, 'base64') };
            } catch (_) {
              return null;
            }
          })
          .filter((f) => f && f.data && f.data.length);
      } else if (contentType.includes('multipart/form-data')) {
        const { files } = parseMultipart(buffer, contentType);
        uploads = collectUploadFiles(files).map((f) => ({
          data: f.data,
          filename: f.filename || 'ghl-export.csv'
        }));
      } else {
        sendJson(res, 400, {
          ok: false,
          error: 'Expected multipart/form-data or JSON upload',
      ...
      const result = await importGhlExports(uploads);
      sendJson(res, 200, {
        ...result,
        receivedFilenames: uploads.map((u) => u.filename)
      });
    } catch (err) {
      const status =
        err.code === 'GHL_PARSE_COLUMNS' ||
        err.code === 'GHL_PDF_EMPTY' ||
        err.code === 'GHL_PARSE_EMPTY' ||
