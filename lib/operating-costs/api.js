...
const { importGhlExport, listCharges, periodBounds } = require('./ghl-import-store');
...
async function handle(req, res, pathname, url) {
  ...
  if (pathname === '/api/admin/operating-costs/ghl-import' && req.method === 'POST') {
    try {
      ...
      const result = importGhlExport(file.data, file.filename || 'ghl-export.csv');
      sendJson(res, 200, result);
    } catch (err) {
      const status = err.code === 'GHL_PARSE_COLUMNS' ? 400 : err.code === 'PAYLOAD_TOO_LARGE' ? 413 : 500;
      sendJson(res, status, {
        ok: false,
        error: err.message || String(err),
