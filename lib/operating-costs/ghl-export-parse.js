...
const CATEGORY_RULES = [
  ...
  { id: 'tax', re: /\b(sales\s*tax|wallet\s*sales\s*tax|tax\s*invoice|final\s*tax|vat|gst)\b/i },
...
// Prefer charge Amount — never match "Wallet Balance…" columns via /balance/.
const AMOUNT_HEADERS = [/^(?!.*balance).*amount/i, /^amount$/i, /charge/i, /cost/i, /price/i];
...
const BALANCE_AFTER_HEADERS = [/wallet\s*balance\s*after/i, /balance\s*after\s*transaction/i];
const BALANCE_TOTAL_HEADERS = [/total\s*wallet\s*balance/i, /including\s*wallet\s*credits/i];
...
function parseDate(raw) {
  ...
  // Strip ordinal suffixes: "Jul 14th 2026, 01:14:33 PM" → "Jul 14 2026, …"
  const cleaned = s.replace(/\b(\d{1,2})(st|nd|rd|th)\b/gi, '$1');

  // HighLevel / wallet exports: "Jul 14 2026, 01:14:33 PM" or "Jun 27, 2026"
  const mon = cleaned.match(
    /^(Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)[a-z]*\.?\s+(\d{1,2}),?\s+(\d{4})\b/i
  );
  if (mon) {
    const mo = MONTH_INDEX[mon[1].slice(0, 3).toLowerCase()];
    const day = Number(mon[2]);
    const year = Number(mon[3]);
    if (mo != null && day >= 1 && day <= 31 && year >= 2000) {
      return new Date(Date.UTC(year, mo, day)).toISOString().slice(0, 10);
    }
  }
...
function parseTime(raw) {
  ...
  const s = String(raw).trim();
  if (!s) return null;
  // Only accept an explicit clock — don't invent times from date-only strings via Date().
...
const KIND_LABELS = {
  usage: 'Wallet usage (SMS / calls / etc.)',
  topup: 'Wallet top-ups (funding)',
  tax: 'Wallet sales tax',
  invoice: 'Invoice / plan bill',
  transactions: 'Transaction / usage charges'
};

/**
 * Explicit HighLevel export families (filename wins over heuristics).
 * These are stored as charge.kind so UI can keep them in separate areas.
 */
function detectDocumentKind({ filename, headers, charges }) {
  const name = String(filename || '').toLowerCase();
  const headerBlob = (headers || []).map(normalizeHeader).join(' | ');

  if (/wallet[_-]?sales[_-]?tax|sales[_-]?tax[_-]?invoice/.test(name)) {
    return { kind: 'tax', confidence: 'high', scores: {}, label: KIND_LABELS.tax };
  }
  if (/wallet[_-]?top[_-]?up|top[_-]?up[_-]?receipt/.test(name)) {
    return { kind: 'topup', confidence: 'high', scores: {}, label: KIND_LABELS.topup };
  }
  if (/wallet[_-]?transactions|transaction\s*id/.test(name) || /transaction\s*id/.test(headerBlob)) {
    return { kind: 'usage', confidence: 'high', scores: {}, label: KIND_LABELS.usage };
  }

  const score = { invoice: 0, transactions: 0 };
  if (/\binvoice\b/.test(name)) score.invoice += 2;
  ...
  const meterShare =
    total
      ? ((cats.sms || 0) + (cats.phone || 0) + (cats.email || 0) + (cats.ai || 0) + (cats.numbers || 0)) /
        total
      : 0;
  const walletShare = total ? (cats.wallet || 0) / total : 0;

  if (total <= 5 && subShare >= 0.4) score.invoice += 2;
  if (total >= 8 && meterShare >= 0.35) score.transactions += 2;
  if (total >= 3 && walletShare >= 0.6) {
    return { kind: 'topup', confidence: 'medium', scores: score, label: KIND_LABELS.topup };
  }
  if (total === 1 && (cats.tax || 0) === 1) {
    return { kind: 'tax', confidence: 'medium', scores: score, label: KIND_LABELS.tax };
  }
  if (total === 1) score.invoice += 1;

  const kind = score.invoice > score.transactions ? 'invoice' : 'usage';
  const confidence =
...
function stampDate(x) {
  return (x && (x.date || x.asOfDate)) || '';
}

function stampTime(x) {
  return (x && (x.time || x.asOfTime)) || '';
}

function compareChargeStamp(a, b) {
  const da = stampDate(a);
  const db = stampDate(b);
...
function chargesFromTable(headers, rows, filename) {
  ...
  const balanceAfterIdx = findCol(headers, BALANCE_AFTER_HEADERS);
  const balanceTotalIdx = findCol(headers, BALANCE_TOTAL_HEADERS);
  ...
  for (const row of rows) {
    const dateRaw = cellAt(row, dateIdx);
    const date = parseDate(dateRaw);
    const amount = parseAmount(cellAt(row, amountIdx));
    if (!date || amount == null) continue;
    // HighLevel puts date+time in one cell ("Jul 14th 2026, 01:14:33 PM").
    let time = timeIdx >= 0 ? parseTime(cellAt(row, timeIdx)) : null;
    if (!time) time = parseTime(dateRaw);
    ...
    const balanceAfter =
      balanceAfterIdx >= 0 ? parseAmount(cellAt(row, balanceAfterIdx)) : null;
    const balanceTotal =
      balanceTotalIdx >= 0 ? parseAmount(cellAt(row, balanceTotalIdx)) : null;
    const key = chargeKey({
      date,
      time,
      amountCents,
    ...
    charges.push({
      ...
      walletBalanceTotalUsd: balanceTotal
    });
    if (
      (balanceAfter != null || balanceTotal != null) &&
      (!walletBalance || compareChargeStamp({ date, time }, walletBalance) >= 0)
    ) {
      walletBalance = {
        balanceAfterUsd: balanceAfter != null ? Number(balanceAfter.toFixed(2)) : null,
        totalIncludingCreditsUsd: balanceTotal != null ? Number(balanceTotal.toFixed(2)) : null,
        asOfDate: date,
        asOfTime: time || null
      };
    }
  ...
  for (const c of charges) {
    c.kind = doc.kind;
    if (doc.kind === 'tax' && c.category === 'other') {
      c.category = 'tax';
      c.categoryLabel = CATEGORY_LABELS.tax;
    }
    if (doc.kind === 'topup' && c.category === 'other') {
      c.category = 'wallet';
      c.categoryLabel = CATEGORY_LABELS.wallet;
    }
    // Prefer stable Transaction IDs for usage ledger dedupe (not date cutoffs).
  ...
  return {
    ...
    detectedColumns: {
      ...
      balanceAfter: balanceAfterIdx >= 0 ? headers[balanceAfterIdx] : null,
      balanceTotal: balanceTotalIdx >= 0 ? headers[balanceTotalIdx] : null
    },
    headers,
