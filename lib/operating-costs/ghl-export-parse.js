...
/**
 * Detect whether a file looks like an invoice summary vs wallet/transaction ledger.
 */
function detectDocumentKind({ filename, headers, charges }) {
  ...
  if (/\binvoice\b/.test(name)) score.invoice += 3;
...
function parseGhlExport(buffer, filename) {
  ...
  for (const row of rows) {
    ...
    // kind filled after detect — provisional transactions
    const key = chargeKey({
      date,
      time,
      amountCents,
  ...
  for (const c of charges) {
    c.kind = doc.kind;
    // Re-hash with kind so invoice vs wallet copies of the same line stay distinct if needed
