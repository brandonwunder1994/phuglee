...
function getSessionSecret() {
  ...
  if (isProd) {
    // Never derive from bootstrap password (that would let password-holders mint any-user cookies).
    // Prefer env; else a durable random secret on the data volume (generated once).
    try {
      const fs = require('fs');
      const path = require('path');
      const root = String(process.env.PDA_DATA_ROOT || '').trim() || path.join(process.cwd(), 'data');
      const secretFile = path.join(root, '.phuglee-session-secret');
      if (fs.existsSync(secretFile)) {
        const disk = String(fs.readFileSync(secretFile, 'utf8') || '').trim();
      ...
      console.warn(
        '[Auth] PHUGLEE_SESSION_SECRET unset — generated durable secret at PDA_DATA_ROOT/.phuglee-session-secret. Set PHUGLEE_SESSION_SECRET in Railway.'
      );
      return generated;
    } catch (err) {
      const e = new Error(
        'PHUGLEE_SESSION_SECRET must be set in production (could not create durable secret file)'
      );
      e.code = 'SESSION_SECRET_REQUIRED';
      e.cause = err;
      throw e;
    }
  }
...
/** True when product APIs must require a verified session cookie (no header spoofing). */
function isAuthRequired() {
  try {
    ...
    const isProd = String(process.env.NODE_ENV || '').toLowerCase() === 'production';
    if (isProd && !flag('PHUGLEE_AUTH_OPEN')) return true;
    return !flag('PHUGLEE_AUTH_DISABLED');
  }
}

/** Allow X-Phuglee-* headers when auth is off, explicitly opted in, or outside production. */
function allowHeaderIdentityFallback() {
  if (!isAuthRequired()) return true;
  if (['1', 'true', 'yes'].includes(
    String(process.env.PHUGLEE_ALLOW_HEADER_AUTH || '').trim().toLowerCase()
  )) {
    return true;
  }
  // Local/dev tooling may still send identity headers; production requires a real cookie.
