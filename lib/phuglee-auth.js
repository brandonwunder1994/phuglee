...
function getSessionSecret() {
  ...
  if (isProd) {
    // Refuse weak defaults in production. Bootstrap-derived secret is allowed as a
    // temporary bridge, but prefer an explicit PHUGLEE_SESSION_SECRET.
    if (bootstrap) {
      console.warn(
        '[Auth] PHUGLEE_SESSION_SECRET unset — deriving session HMAC from bootstrap password. Set PHUGLEE_SESSION_SECRET.'
      );
      return crypto.createHash('sha256').update(`phuglee-session:${bootstrap}`).digest('hex');
    }
    const err = new Error(
      'PHUGLEE_SESSION_SECRET (or PHUGLEE_BOOTSTRAP_ADMIN_PASSWORD) must be set in production'
    );
