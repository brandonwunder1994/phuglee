...
/**
 * Remove one city+uploadType format memory (all fingerprints), or the whole city.
 * Used to force Type-column re-confirm after a bad auto-map.
 * @param {string} cityId
 * @param {string} [uploadType] when omitted, deletes the entire city entry
 * @returns {boolean} true if something was removed
 */
function deleteCityFormat(cityId, uploadType) {
  ...
  if (!uploadType) {
    delete doc.cities[cityId];
    writeJsonAtomic(cityFormatsPath(), doc);
    return true;
  }

  if (!Object.prototype.hasOwnProperty.call(city, uploadType)) return false;
  delete city[uploadType];
  if (!Object.keys(city).length) {
    delete doc.cities[cityId];
  } else {
    doc.cities[cityId] = city;
  }
  writeJsonAtomic(cityFormatsPath(), doc);
  return true;
}
