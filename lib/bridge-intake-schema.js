...
const UPLOAD_TYPES = Object.freeze({
  ...
  /**
   * Water shut-off lists: always high-value, never phrase/brain type-suppress.
   * Desk UI may hide this option; engine + API still accept it for imports.
   */
  water_shut_off: {
    id: 'water_shut_off',
...
/**
 * Separate house/street number columns (city exports often split number + name).
 * Never use bare "number" — it steals Case Number / Permit Number.
 */
const STREET_NUMBER_ALIASES = Object.freeze([
  'issue street number',
  'street number',
  'house number',
  'address number',
...
const INTAKE_FIELD_ALIASES = Object.freeze({
  streetAddress: [
    ...
    // (paired with streetNumber column via composeStreetAddress)
...
/**
 * Join house number + street name when city sheets split them into two columns.
 * Does not double-prefix when the street cell already starts with a house number.
 */
function composeStreetAddress(numberPart, streetPart) {
  const n = String(numberPart ?? '')
    .replace(/\s+/g, ' ')
    .trim();
  const s = String(streetPart ?? '')
    .replace(/\s+/g, ' ')
    .trim();
  if (!n && !s) return '';
  if (!n) return s;
  if (!s) return n;
  // Street already has a leading house number (full address in one cell)
  if (/^\d{1,6}[A-Za-z]?\b/.test(s)) return s;
  // Number cell accidentally holds the full address
  if (hasUsableStreetAddress(n) && /\d{1,6}\s+[A-Za-z#]/.test(n)) return n;
  return `${n} ${s}`.replace(/\s+/g, ' ').trim();
}

/**
 * Resolve streetAddress from a raw row + column map, composing number + name when needed.
 */
function resolveStreetAddressFromRaw(rawRow, columnMap) {
  const map = columnMap || {};
  const streetHeader = map.streetAddress;
  const numberHeader = map.streetNumber;
  const street = streetHeader ? String(rawRow?.[streetHeader] ?? '').trim() : '';
  const number = numberHeader ? String(rawRow?.[numberHeader] ?? '').trim() : '';
  return composeStreetAddress(number, street);
}

function detectIntakeColumnMap(headers) {
  const map = {};
  const used = new Set();
  // Claim split house/street NUMBER columns first so alias "street" does not
  // steal "Street Number" as the full streetAddress field.
  const streetNumber = findColumn(headers, STREET_NUMBER_ALIASES, used);
  if (streetNumber) used.add(streetNumber);
...
function mapRawRow(rawRow, columnMap) {
  ...
  // Split Address Number + Street Name → one usable streetAddress
  mapped.streetAddress = resolveStreetAddressFromRaw(rawRow, columnMap);
  return mapped;
}
