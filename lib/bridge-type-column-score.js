...
const DEFAULTS = Object.freeze({
  ...
  minScore: 45,
  minMargin: 8,
  /** Confirm UI only suggests a default when pick clears these bars. */
  suggestMinScore: 58,
  suggestMinMargin: 12
});

/**
 * City categorical / distress vocabulary — boost columns whose samples look
 * like violation codes rather than prose, addresses, or dates.
 */
const DISTRESS_VOCAB_RE =
  /\b(weeds?|grass|overgrown|trash|junk|debris|blight|nuisance|abandon|inoperable|vehicle|dilapidat|property\s+maint|care\s+of\s+premises?|illegal\s+dump|vegetation|rubbish|litter|boarding|vacant|sanitation|high\s+grass|tall\s+grass)\b/i;
...
function computeValueStats(samples) {
  ...
  for (const s of samples) {
    ...
    if (DISTRESS_VOCAB_RE.test(s)) distressN += 1;
  }
  ...
  return {
    ...
    distressVocabFrac: distressN / count
  };
}
...
function headerFeatureScore(header, opts = {}) {
  ...
  // Address-like headers almost never hold violation codes
  if (/\b(address|street|location|property\s+addr|site\s+addr|parcel)\b/i.test(header || '')) {
    score -= 40;
    reasons.push('address_header:-40');
  }

  // Strong categorical header tokens cities commonly use
  if (/\b(vio\s*cat|case\s*type|issue\s*type|violation\s*type|complaint\s*type|action\s*form)\b/i.test(header || '')) {
    score += 12;
    reasons.push('strong_type_header:+12');
  }
...
function valueShapeScore(stats) {
  ...
  // Median length bands — prefer short city code labels
  if (stats.medianLen <= 24) {
    score += 26;
    reasons.push('median_len_code:+26');
  } else if (stats.medianLen <= 40) {
    score += 18;
  ...
  // Samples look like weeds/trash/vehicle codes
  if (stats.distressVocabFrac >= 0.35) {
    score += 22;
    reasons.push('distress_vocab_frac:+22');
  } else if (stats.distressVocabFrac >= 0.15) {
    score += 10;
    reasons.push('distress_vocab_frac_mild:+10');
  }
...
/**
 * Whether the confirm UI may pre-select a suggestion (stricter than pick).
 * @param {Array} ranked
 * @param {object} [opts]
 * @returns {{ header: string, score: number, ... }|null}
 */
function pickSuggestedTypeColumn(ranked, opts = {}) {
  return pickTypeColumn(ranked, {
    minScore: opts.suggestMinScore != null ? opts.suggestMinScore : DEFAULTS.suggestMinScore,
    minMargin: opts.suggestMinMargin != null ? opts.suggestMinMargin : DEFAULTS.suggestMinMargin
  });
}
