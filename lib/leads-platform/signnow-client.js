...
async function api(method, urlPath, body) {
  ...
  if (!res.ok) {
    const snippet = String(text || '').slice(0, 700);
    let message = `SignNow ${method} ${urlPath} → ${res.status}: ${snippet}`;
    if (res.status === 429 || /rate limit/i.test(snippet)) {
      message = 'SignNow rate limit exceeded (500 API requests per hour). Wait until the next hour, then try sending again.';
    }
    const err = new Error(message);
    err.code = res.status === 429 ? 'SIGNNOW_RATE_LIMIT' : 'SIGNNOW_API_ERROR';
...
/**
 * Upload a PDF buffer as a new SignNow document. Returns document id.
 */
async function uploadDocumentPdf(pdfBuffer, fileName = 'document.pdf') {
  const { token, base } = getConfig();
  if (!token) {
    const err = new Error('SignNow is not configured (missing SIGNNOW_ACCESS_TOKEN)');
    err.code = 'SIGNNOW_NOT_CONFIGURED';
  ...
  if (!bytes.length) {
    const err = new Error('Empty PDF upload');
    err.code = 'EMPTY_PDF';
    throw err;
  }
  ...
  form.append(
    ...
    String(fileName || 'document.pdf').replace(/[^\w.\- ]+/g, '_').slice(0, 120)
  );
  ...
  if (!res.ok) {
    const err = new Error(`SignNow POST /document → ${res.status}: ${String(text).slice(0, 700)}`);
    err.code = 'SIGNNOW_API_ERROR';
    err.status = res.status;
    err.body = json;
    throw err;
  }
  const id = json && (json.id || json.document_id);
  if (!id) {
    const err = new Error('SignNow upload did not return a document id');
    err.code = 'SIGNNOW_UPLOAD_FAILED';
    err.body = json;
    throw err;
  }
  return id;
...
/**
 * @param {object} doc
 * @param {{ emailIncludes?: string, kind?: 'signature'|'initial'|null }} [opts]
 *   kind=signature → drawn signatures only (excludes initials)
 *   kind=initial → initials only
 *   emailIncludes → keep signatures whose email contains this substring
 */
function ownerArtifactsFromDoc(doc, opts = {}) {
  const emailIncludes = opts.emailIncludes
    ? String(opts.emailIncludes).trim().toLowerCase()
    : '';
  const kind = opts.kind ? String(opts.kind).trim().toLowerCase() : '';
  ...
  const signatures = (doc?.signatures || [])
    .filter((s) => {
      ...
      if (emailIncludes) {
        ...
        if (!email.includes(emailIncludes)) return false;
      }
      const subtype = String(s.subtype || '').toLowerCase();
      const isInitial = subtype === 'initial' || subtype === 'initials';
      if (kind === 'signature' && isInitial) return false;
      if (kind === 'initial' && !isInitial) return false;
      return true;
    })
    .map((s) => ({
 ...
 */
async function loadOwnerArtifactsFromTemplate(templateId, opts = {}) {
  const doc = await getDocument(templateId);
  return ownerArtifactsFromDoc(doc, opts);
}

/**
 * Load artifacts from any document id (e.g. a prior signed JV for Brad's sig/initials).
 */
async function loadOwnerArtifactsFromDocument(documentId, opts = {}) {
  const doc = await getDocument(documentId);
  return ownerArtifactsFromDoc(doc, opts);
}

/**
 * Fully complete a document as the owner:
 * - Stamp named text values as owner texts
 * - Clear ALL live fields (no invites / no remaining signers)
 * - Re-apply signature images + identity texts in the same write
 *
 * @param {string} documentId
 * @param {object} valuesByName
 * @param {{ artifacts?: { signatures?: object[], texts?: object[] } }} [opts]
 */
async function applyCompleteOwnerStamp(documentId, valuesByName = {}, opts = {}) {
  const doc = await getDocument(documentId);
  const stampedTexts = [];
...
async function applyPrefillStampingRole(documentId, valuesByName = {}, opts = {}) {
  ...
  const skipRoles = new Set(opts.skipRoles || []);
  const dropStampSignatures = opts.dropStampSignatures !== false;
  const doc = await getDocument(documentId);
  const keepFields = [];
  const stampedTexts = [];
  ...
  for (const f of doc.fields || []) {
    ...
    // Drop stamp-role signature/initial boxes when we re-apply a real signature image.
    if (
      dropStampSignatures
      && f.role === stampRole
...
function isDocumentFullySigned(doc) {
  ...
  // Do NOT treat "has stamped signatures + no invites yet" as signed.
  // Amendment/Cash/AOC stamp Buyer first, then invite sellers — the old
  // signatures-without-invites heuristic marked those docs signed too early.
  const status = String(doc.status || doc.document_status || '').toLowerCase();
  return status === 'completed' || status === 'complete' || status === 'fulfilled';
}
