...
/**
 * Recent documents from the SignNow account (newest first).
 * Used to re-attach packages that lost their local signNowPending row.
 */
async function listRecentDocuments(limit = 50) {
  const n = Math.max(1, Math.min(100, Number(limit) || 50));
  const data = await api('GET', `/user/documentsv2?limit=${n}`);
