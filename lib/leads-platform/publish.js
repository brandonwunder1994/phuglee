...
function publishLead(raw, opts = {}) {
  ...
  // Preserve Vault hide status unless explicitly provided
  if (!opts.catalogStatus && !raw?.catalogStatus) {
    const existing = require('./store').getLead(lead.leadId);
    if (existing?.catalogStatus && existing.catalogStatus !== 'active') {
      lead.catalogStatus = existing.catalogStatus;
    }
  }
  ...
  // Campaigns SMS: enqueue GHL sync when lead is vault-approved (non-blocking).
  try {
    if (saved && saved.reviewStatus === 'approved' && saved.leadId) {
      const { enqueueLeadSync } = require('../campaigns/sms-sync');
      enqueueLeadSync(saved.leadId);
    }
  } catch (_) { /* campaigns module optional at boot */ }
