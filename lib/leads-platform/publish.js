...
function mapAnalyzeResultToLead(row = {}, meta = {}) {
  const address = row.address || row.streetAddress || row.normalizedAddress || '';
  const leadType = meta.leadType || row.leadType || row.bucket || 'distressed';
  const signalTags = [];
  ...
  return normalizeLeadRecord({
    ...
    leadType,
    reviewStatus: meta.reviewStatus || (leadType === 'distressed' ? 'pending' : 'approved'),
    ...
    sourceListId: meta.sourceListId || '',
...
function publishLead(raw, opts = {}) {
  const lead = typeof raw.leadId === 'string'
    ? normalizeLeadRecord(raw)
    : mapAnalyzeResultToLead(raw, opts);
