 ...
 *   📮 AOC Sent / ✅ AOC Signed    → buyer_signed_aoc
...
const STAGE_RULES = [
  ...
  // EMD / close-line before AOC rules so "AOC … close" phrases still rank correctly
  { re: /in line to close|buyer\s*submitted\s*emd|emd\s*submitted/i, stage: 'buyer_found' },
  // AOC sent/signed = Buyer Signed AOC column (not EMD)
  { re: /aoc\s*signed|buyer\s*signed\s*aoc/i, stage: 'buyer_signed_aoc' },
  { re: /aoc\s*sent/i, stage: 'buyer_signed_aoc' },
...
async function upsertDealFromOpportunity(opp, pipeline, contactSummary) {
  ...
  // Desk money wins when already set. Never use opportunity monetaryValue as purchase
  // (often assignment fee / deal value, not PSA price). Never overwrite PSA-locked agreement date.
  const purchasePrice = existing?.purchasePrice != null
    ? existing.purchasePrice
    : (deskKeep.purchasePrice != null
      ? deskKeep.purchasePrice
      : (normalizeMoney(fromContact.contractPrice) ?? null));
  const assignmentFee = existing?.assignmentFee != null
    ? existing.assignmentFee
    : (deskKeep.assignmentFee != null
      ? deskKeep.assignmentFee
      : (normalizeMoney(fromContact.assignmentFee) ?? null));
  const emdDeposit = existing?.emdDeposit != null
    ? existing.emdDeposit
    : (deskKeep.emdDeposit != null
      ? deskKeep.emdDeposit
      : (normalizeMoney(fromContact.emdDeposit) ?? null));
  const originalAgreementDate = existing?.originalAgreementDate
    || deskKeep.originalAgreementDate
    || formatClosingSlash(fromContact.contractSignedDate)
    || '';
  ...
  const deal = upsertDeal({
    ...
    // Otherwise GHL CRM contact is source of truth (refresh email/phone every sync).
    phone: contactLocked
    ...
    // ownerEmail drives Overview + amendments — must track CRM, not freeze at first fill
    ownerEmail: contactLocked
      ? (existing?.ownerEmail || existing?.email || fromContact.email || '')
      : (fromContact.email || existing?.ownerEmail || existing?.email || ''),
  ...
  // New Under Contract (or first dispo landing) → SMS/email Brad + Brandon
  fireDealTransitionAlerts(existing, deal).catch((err) => {
    console.warn('[team-notify] GHL sync alerts failed:', err.message);
  });

  // Seed APN + legal for AOC when deal is (or becomes) under contract / buyer found.
  if (stage === 'under_contract' || stage === 'buyer_found') {
    try {
      const { ensureDealContractParcel } = require('./contracts');
      const ensured = await ensureDealContractParcel(deal.dealId);
      if (ensured?.deal) return ensured.deal;
    } catch (_) {
      /* REAPI optional — desk can ensure on AOC open */
    }
  }
