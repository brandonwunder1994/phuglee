...
async function alertSignNowOpened({ deal, kind, openedByEmail }) {
  const addr = dealLabel(deal);
  const link = dealDeepLink(deal.dealId);
  const label = docKindLabel(kind);
  const who = openedByEmail ? ` (${openedByEmail})` : '';
  const sms = `${label} opened for ${addr}${who}.\n${link}`;
  return notifyBoth(sms, `${label} opened — ${addr}`,
    `<p><strong>${label}</strong> was opened on <strong>${addr}</strong>${who ? ` by ${openedByEmail}` : ''}.</p><p><a href="${link}">Open deal</a></p>`);
}

async function alertSignNowSigned({ deal, kind }) {
  const addr = dealLabel(deal);
  const link = dealDeepLink(deal.dealId);
  const label = docKindLabel(kind);
