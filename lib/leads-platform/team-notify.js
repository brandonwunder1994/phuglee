...
/**
 * Parties who are auto-stamped (not invited) on Cash PSA / AOC / Amendment.
 * Never list these as "awaiting signatures" in team texts.
 * SubTo is the exception where Buyer may still need a live invite — callers
 * should only pass actual SignNow invitees for those flows.
 */
function isAutoStampedAwaitingParty(label) {
  const s = String(label || '').trim().toLowerCase();
  if (!s) return true;
  // Wunderhaus / Brandon already signed as Buyer or Assignor on cash/AOC/amendment
  if (/\bwunderhaus\b/.test(s)) return true;
  if (/\bbrandon\s+wunder\b/.test(s)) return true;
  if (/\bbrandon@wunderhausgroup\.com\b/.test(s)) return true;
  if (/\bassignor\b/.test(s) && !/\b(assignee|seller)\b/.test(s)) return true;
  if (/\b(buyer)\b/.test(s) && /\b(wunderhaus|brandon)\b/.test(s)) return true;
  return false;
}

/** Keep only counterparties who still need to sign. */
function filterAwaitingSigners(awaiting = []) {
  const list = Array.isArray(awaiting) ? awaiting : [awaiting];
  return list
    .map((x) => String(x || '').trim())
    .filter((x) => x && !isAutoStampedAwaitingParty(x));
}

/**
 * Build human labels for invitees still expected to sign.
 * Prefer explicit invitee roles/emails from the SignNow send result.
 */
function awaitingLabelsFromInvitees(invitees = [], fallback = []) {
  const fromInvitees = (Array.isArray(invitees) ? invitees : [])
    .map((i) => {
      if (!i || typeof i !== 'object') return String(i || '').trim();
      const role = String(i.role || i.name || '').trim();
      const email = String(i.email || '').trim();
      if (role && email) return `${role} (${email})`;
      return role || email;
    })
    .filter(Boolean);
  return filterAwaitingSigners(fromInvitees.length ? fromInvitees : fallback);
}

async function alertAmendmentSent({ deal, awaiting = [] }) {
  const addr = dealLabel(deal);
  const link = dealDeepLink(deal.dealId);
  // Buyer (Wunderhaus) is pre-stamped on amendments — only list real invitees.
  const pending = filterAwaitingSigners(awaiting);
  const wait = pending.length ? pending.join(', ') : 'the counterparty';
...
module.exports = {
  ...
  alertJvSent,
  alertAmendmentSent,
  isAutoStampedAwaitingParty,
  filterAwaitingSigners,
  awaitingLabelsFromInvitees,
  alertTitleOpened,
  alertSellerEmd,
  alertDeskReady,
  alertPhotosReady,
