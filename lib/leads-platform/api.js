  ...
  if (mediaListMatch && req.method === 'POST') {
    ...
    try {
      const items = Array.isArray(body.items) ? body.items
        : (body.url || body.sourceUrl ? [body] : []);
      if (!items.length) {
        sendJson(res, 400, { ok: false, error: 'Provide url or items[]', code: 'MISSING_MEDIA_URL' });
        return true;
      }
      const out = items.length === 1
        ? await (async () => {
          const one = await saveSellerMediaFromUrl(mediaListMatch[1], items[0]);
          return {
            deal: one.deal,
            sellerMedia: enrichSellerMediaForDisplay(one.deal),
            results: [{ ok: true, skipped: !!one.skipped, item: one.item, url: items[0].url || items[0].sourceUrl }],
            saved: one.skipped ? 0 : 1,
            skipped: one.skipped ? 1 : 0,
            failed: 0
          };
        })()
        : await saveSellerMediaMany(mediaListMatch[1], items);
      const deskUser = normalizeTeamUser(readPhugleeUser(req)) || (isAdmin(req) ? 'admin' : 'brad');
      sendJson(res, 200, {
        ok: true,
        ...out,
