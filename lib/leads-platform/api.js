...
const { matchLandFunds } = require('./land/fund-match');
const { normalizeLandUnderwriting } = require('./land/lao');
const { runLandAutoComp } = require('./land/run-land-comp');
const { buildManualLandCompReport } = require('./land/manual-land-comp');
const { promoteLeadToTeardown } = require('./land/teardown');
const { getTaxDirtScript } = require('./land/tax-dirt-script');
const { buildBuilderPacket } = require('./land/builder-packet');
const { mergeParcelIntoLead, extractParcelFields } = require('./land/parcel');
const {
  normalizeLandDisposition,
  applyLandDispositionPatch,
  LAND_DISPO_LABELS
} = require('./land/disposition');
const { enrichLandLeadFromReapi, enrichLandLeadsFromReapi } = require('./land/enrich-from-reapi');
...
/** Short in-memory board list cache — silent polls hit this; interactive uses fresh=1. */
const BOARD_LIST_CACHE_MS = 8000;
const boardListCache = new Map();

function boardListCacheKey(board, teamUser) {
  return `${board || 'contracts'}:${String(teamUser || '').toLowerCase()}`;
}

function getBoardListCache(board, teamUser) {
  const key = boardListCacheKey(board, teamUser);
  const hit = boardListCache.get(key);
  if (!hit) return null;
  if (Date.now() - hit.at > BOARD_LIST_CACHE_MS) {
    boardListCache.delete(key);
    return null;
  }
  return hit.body;
}

function setBoardListCache(board, teamUser, body) {
  boardListCache.set(boardListCacheKey(board, teamUser), {
    at: Date.now(),
    body
  });
}
...
/**
 * Brad may not open sales-stage dossiers, including Waiting for Signatures
 * (contract_sent) — address+photo glance only on the tracker.
 */
function denyBradSalesRead(req, res, deal) {
  const user = readPhugleeUser(req);
  if (!isDisposUsername(user)) return false;
  if (!deal || !isSalesStage(deal.stage)) return false;
  const waiting = String(deal.stage || '').trim() === 'contract_sent';
  sendJson(res, 403, {
    ok: false,
    error: waiting
      ? 'Waiting for Signatures is admin-only — you can see the address on the tracker'
      : 'Sales-stage deals are view-only (address and photo) for disposition',
...
/** Contract nested routes: auth + sales-stage isolation for Brad. */
function gateContractDealRead(req, res, dealId) {
  if (!requireContractDesk(req, res)) return null;
  const deal = getDeal(dealId);
  if (!deal) {
    sendJson(res, 404, { ok: false, error: 'Deal not found', code: 'NOT_FOUND' });
    return null;
  }
  if (denyBradSalesRead(req, res, deal)) return null;
  return deal;
}

function gateContractDealWrite(req, res, dealId) {
  if (!requireContractDesk(req, res)) return null;
  if (denyBradSalesWrite(req, res, dealId)) return null;
  return true;
}
...
function parseListQuery(url) {
  ...
  return normalizeListQuery({
    ...
    landVerdict: url.searchParams.get('landVerdict') || 'all',
    landDispo: url.searchParams.get('landDispo') || 'all',
...
function normalizeListQuery(raw = {}) {
  ...
  return {
    ...
    surface: ['home', 'land', 'all'].includes(String(raw.surface || '').toLowerCase())
      ...
      : 'all',
    landVerdict: ['pending', 'keep', 'toss', 'fund-shaped', 'all'].includes(String(raw.landVerdict || '').toLowerCase())
      ? String(raw.landVerdict).toLowerCase()
      : 'all',
    landDispo: ['new', 'screening', 'ready', 'pitched', 'waiting', 'won', 'passed', 'dead', 'all']
      .includes(String(raw.landDispo || '').toLowerCase())
      ? String(raw.landDispo).toLowerCase()
      : 'all',
    assetClass: ['teardown', 'vacant_lot', 'all'].includes(String(raw.assetClass || '').toLowerCase())
      ? String(raw.assetClass).toLowerCase()
      : 'all',
    state: raw.state || '',
    city: raw.city || '',
    minScore: raw.minScore != null && raw.minScore !== '' ? raw.minScore : null,
    maxScore: raw.maxScore != null && raw.maxScore !== '' ? raw.maxScore : null,
    minDistressTier: raw.minDistressTier != null && raw.minDistressTier !== ''
      ? raw.minDistressTier
      : null,
    q: raw.q || '',
    page: raw.page || '1',
...
function buildListResponse(user, query, req) {
  ...
  // Land Desk falls back to meta.states / meta.citiesByState — skip redundant facet pass.
  // House Vault still needs byTypeFiltered / statesFiltered on list responses.
  if (query.surface === 'land') query.skipFacets = true;
...
function buildExportFilename({ format = 'xlsx', filters = {}, label = '' } = {}) {
  ...
  if (labelSlug) parts.push(labelSlug);
    ...
    if (filters.minDistressTier) parts.push(`tier${Number(filters.minDistressTier) || ''}`);
  }
  parts.push(stamp);
...
function exportRateKey(user, now = new Date()) {
  const day = now.toISOString().slice(0, 10);
  return `${user}:${day}`;
}

/** Drop export rate keys not for today (Wave 3 — Map must not grow forever). */
function pruneExportCounts(now = new Date()) {
  const today = now.toISOString().slice(0, 10);
  for (const key of exportCounts.keys()) {
    if (!String(key).endsWith(`:${today}`)) exportCounts.delete(key);
  }
}
...
function landCompSummaryFromLead(lead) {
  return {
    landCompingReport: lead.landCompingReport || null,
    landComps: Array.isArray(lead.landComps) ? lead.landComps : [],
    landCompedAt: lead.landCompedAt || null,
    landCompConfidence: lead.landCompConfidence || null,
    landCompSource: lead.landCompSource || null,
    landUnderwriting: lead.landUnderwriting || null
  };
}
...
async function handle(req, res, pathname, url) {
  ...
  if (pathname === '/api/leads/bootstrap' && req.method === 'GET') {
    ...
    sendJson(res, 200, {
      ok: true,
      meta: getMeta({ surface: query.surface }),
  ...
  if (pathname === '/api/leads/land/tax-dirt-script' && req.method === 'GET') {
    if (!requireMax(req, res)) return true;
    sendJson(res, 200, { ok: true, script: getTaxDirtScript() });
    return true;
  }

  const builderPacketMatch = /^\/api\/leads\/([a-zA-Z0-9_-]+)\/builder-packet$/.exec(pathname);
  if (builderPacketMatch && req.method === 'GET') {
    const user = requireMax(req, res);
    if (!user) return true;
    const lead = getLead(builderPacketMatch[1]);
    if (!lead || lead.leadType !== 'land') {
      sendJson(res, 404, { ok: false, error: 'Land lead not found', code: 'NOT_FOUND' });
      return true;
    }
    const overlays = readOverlays(user);
    const note = overlays?.notes?.[lead.leadId] || '';
    const packet = await buildBuilderPacket(lead, { note });
    const format = String(url.searchParams.get('format') || '').toLowerCase();
    const accept = String(req.headers.accept || '');
    const wantPdf = format === 'pdf'
      || (format !== 'json' && format !== 'txt' && accept.includes('application/pdf') && !accept.includes('application/json'));
    if (wantPdf) {
      res.writeHead(200, {
        'Content-Type': 'application/pdf',
        'Content-Disposition': `attachment; filename="${String(packet.pdfFilename || 'builder-packet.pdf').replace(/"/g, '')}"`,
        'Cache-Control': 'no-store',
        'Content-Length': String(packet.pdfBuffer.length)
      });
      res.end(packet.pdfBuffer);
      return true;
    }
    sendJson(res, 200, {
      ok: true,
      ...
      acres: packet.acres,
      zoning: packet.zoning
    });
    return true;
  }

  const landParcelMatch = /^\/api\/leads\/([a-zA-Z0-9_-]+)\/parcel$/.exec(pathname);
  if (landParcelMatch && req.method === 'PUT') {
    const user = requireMax(req, res);
    if (!user) return true;
    const lead = getLead(landParcelMatch[1]);
    if (!lead || lead.leadType !== 'land') {
      sendJson(res, 404, { ok: false, error: 'Land lead not found', code: 'NOT_FOUND' });
      return true;
    }
    ...
    const patch = body.parcel || body.propertyDetails || body;
    const merged = mergeParcelIntoLead(lead, patch);
    const saved = upsertLead(merged);
    sendJson(res, 200, {
      ok: true,
      lead: saved,
      parcel: extractParcelFields(saved)
    });
    return true;
  }

  const landEnrichParcelMatch = /^\/api\/leads\/([a-zA-Z0-9_-]+)\/enrich-parcel-reapi$/.exec(pathname);
  if (landEnrichParcelMatch && req.method === 'POST') {
    const user = requireMax(req, res);
    if (!user) return true;
    const lead = getLead(landEnrichParcelMatch[1]);
    if (!lead || lead.leadType !== 'land') {
      sendJson(res, 404, { ok: false, error: 'Land lead not found', code: 'NOT_FOUND' });
      return true;
    }
    ...
    try {
      const out = await enrichLandLeadFromReapi(lead, reapi, {
        force: body.force === true,
        skipComplete: body.skipComplete !== false
      });
      if (!out.ok) {
        sendJson(res, out.code === 'REAPI_EMPTY' ? 404 : 502, {
          ok: false,
          error: out.error || 'Parcel enrich failed',
          code: out.code || 'REAPI_ENRICH_FAILED'
        });
        return true;
      }
      if (out.skipped || (!(out.filled || []).length && !(out.coordsFilled || []).length)) {
        sendJson(res, 200, {
          ok: true,
          skipped: true,
          reason: out.reason || 'already_complete',
          lead,
          parcel: extractParcelFields(lead),
          filled: [],
          coordsFilled: []
        });
        return true;
      ...
      sendJson(res, 500, {
        ok: false,
        error: err.message || 'Parcel enrich failed',
        code: 'REAPI_ENRICH_FAILED'
      });
    }
    return true;
  }

  const landDispoMatch = /^\/api\/leads\/([a-zA-Z0-9_-]+)\/land-disposition$/.exec(pathname);
  if (landDispoMatch && req.method === 'GET') {
    if (!requireMax(req, res)) return true;
    const lead = getLead(landDispoMatch[1]);
    if (!lead || lead.leadType !== 'land') {
      sendJson(res, 404, { ok: false, error: 'Land lead not found', code: 'NOT_FOUND' });
      return true;
    }
    sendJson(res, 200, {
      ok: true,
      landDisposition: normalizeLandDisposition(lead.landDisposition || {}),
      labels: LAND_DISPO_LABELS
    });
    return true;
  }

  if (landDispoMatch && req.method === 'PUT') {
    const user = requireMax(req, res);
    if (!user) return true;
    const lead = getLead(landDispoMatch[1]);
    if (!lead || lead.leadType !== 'land') {
    ...
    lead.landDisposition = applyLandDispositionPatch(
      lead.landDisposition,
      body.landDisposition || body,
      user
    );
    ...
    sendJson(res, 200, {
      ok: true,
      lead: saved,
      landDisposition: saved.landDisposition
    });
    return true;
  }
  ...
  if (pathname === '/api/leads/map' && req.method === 'GET') {
    ...
    if (query.includeHidden && !isAdmin(req)) query.includeHidden = false;
    if (!isAdmin(req)) query.includeHidden = false;
    // Map payloads use markers; type/state facets come from meta — skip second index scan.
    if (query.skipFacets == null) query.skipFacets = true;
  ...
  if (pathname === '/api/leads/admin/contracts' && req.method === 'GET') {
    ...
    const board = String(listUrl.searchParams.get('board') || 'contracts')
      .trim()
      .toLowerCase() === 'pipeline'
      ? 'pipeline'
      : 'contracts';
    // Only silent board polls opt into the short cache (poll=1). Interactive /
    // test loads always re-read disk so saves are visible immediately.
    const useCache = listUrl.searchParams.get('poll') === '1';
    // Board must stay snappy: return stored deals first. GHL SMS peek + SignNow
    // import run after the response (throttled; also covered by bg SignNow timer).
    setImmediate(() => {
      Promise.resolve()
        .then(() => peekSellerSmsForOpenDeals())
        .catch((err) => console.warn('[seller-sms] bg peek failed:', err && err.message));
      Promise.resolve()
        .then(() => syncPendingSignNowAcrossDeals())
        .catch((err) => console.warn('[signnow] bg board sync failed:', err && err.message));
    });
    if (useCache) {
      const cached = getBoardListCache(board, teamUser);
      if (cached) {
    ...
    const rawDeals = filterDealsForBoard(allDeals, board);
    const deals = rawDeals.map((d) => projectDealForBoard(d, teamUser));
    const body = {
      ok: true,
      board,
      deals,
      totals: proofTotals(board === 'pipeline' ? allDeals : rawDeals),
      goal: computeFundedGoal(allDeals),
      ghlConfigured: isGhlConfigured(),
      payoutSettings: readPayoutSettings(),
      unreadTeam: listUnreadTeamForUser(teamUser),
      unreadSellerSms: listUnreadSellerSmsForUser(teamUser)
    };
    // Warm cache for upcoming silent polls (also after interactive loads).
    setBoardListCache(board, teamUser, body);
  ...
  if (pathname === '/api/leads/admin/contracts/vault-search' && req.method === 'GET') {
    if (!requireContractDesk(req, res)) return true;
    const q = String(url.searchParams.get('q') || '').trim();
    ...
    // Address-ranked typeahead (not priority-sorted Vault list) so house numbers match streets.
    // Includes under_contract catalog status (GHL pipeline / pending PSA), not sold/excluded.
    const rows = searchActiveVaultLeads(q, { limit });
    const leads = rows.map((l) => {
      // Index list rows omit email — hydrate from full lead record.
      const full = getLead(l.leadId) || l;
      return {
        leadId: full.leadId || l.leadId,
        address: full.address || l.address || '',
        city: full.city || l.city || '',
        state: full.state || l.state || '',
        zip: full.zip || l.zip || '',
        ownerName: full.ownerName || l.ownerName || '',
        email: full.email || l.email || '',
        phones: Array.isArray(full.phones) && full.phones.length
          ? full.phones
          : (Array.isArray(l.phones) ? l.phones : []),
        catalogStatus: full.catalogStatus || l.catalogStatus || 'active'
      };
    });
    sendJson(res, 200, { ok: true, leads, total: leads.length });
    return true;
  }

  if (pathname === '/api/leads/admin/contracts/send-new-psa' && req.method === 'POST') {
    ...
    try {
      const out = await requestSendNewPsa(body || {}, user);
      sendJson(res, 200, {
        ok: true,
      ...
      const status = err.code === 'NOT_FOUND' ? 404
        : err.code === 'SIGNNOW_NOT_CONFIGURED' ? 503
          : err.code === 'SIGNNOW_API_ERROR' || err.code === 'SIGNNOW_SEND_FAILED' ? 502
            : err.code === 'DEAL_ALREADY_UNDER_CONTRACT' ? 409
              : 400;
      sendJson(res, status, {
        ok: false,
        error: err.message,
  ...
  if (dealGetMatch && req.method === 'GET') {
    ...
    if (profile.ghlContactId && isGhlConfigured()) {
      try {
        ...
        // GHL CRM email/phone are source of truth (unless SMS contact remapped/locked).
        // Always refresh when CRM has values so stale PSA/first-fill data does not stick.
        if (contact && !profile.ghlContactLocked) {
          const contactPatch = {};
          const crmEmail = String(contact.email || '').trim().toLowerCase();
          const crmPhone = String(contact.phone || '').trim();
          if (crmEmail) {
            if (String(profile.email || '').trim().toLowerCase() !== crmEmail) {
              contactPatch.email = crmEmail;
            }
            if (String(profile.ownerEmail || '').trim().toLowerCase() !== crmEmail) {
              contactPatch.ownerEmail = crmEmail;
            }
          }
          if (crmPhone) {
            const dealDigits = String(profile.phone || '').replace(/\D/g, '');
            const crmDigits = crmPhone.replace(/\D/g, '');
            if (crmDigits && dealDigits !== crmDigits) contactPatch.phone = crmPhone;
          }
          if (Object.keys(contactPatch).length) {
            try {
              patchDeal(profile.dealId, contactPatch);
              profile = getDealProfile(profile.dealId) || profile;
            } catch (_) { /* optional */ }
          }
        }
    ...
    // PSA already in Documents (common for SubTo uploads) also unlocks amendments
    if (!profile.originalAgreementDate) {
      try {
        ensureOriginalAgreementDate(profile.dealId, contact);
        profile = getDealProfile(profile.dealId) || profile;
      } catch (_) { /* optional */ }
    }
    if (isSignNowConfigured()) {
      try {
        const sn = await syncSignedSignNowDocuments(profile.dealId, { scanLibrary: true });
        if (sn.ingested || (sn.added && sn.added.length) || (sn.deal && sn.deal.signNowPending)) {
  ...
  if (docsListMatch && req.method === 'GET') {
    if (!gateContractDealRead(req, res, docsListMatch[1])) return true;
  ...
  if (docsListMatch && req.method === 'POST') {
    if (!gateContractDealWrite(req, res, docsListMatch[1])) return true;
    // Docs allow up to ~18MB binary; base64 JSON is ~4/3 + wrapper → 26MB (same as media/comp).
    let body;
    try {
      body = await readBody(req, { maxBytes: 26 * 1024 * 1024 });
    } catch (err) {
      if (err && err.code === 'PAYLOAD_TOO_LARGE') {
        sendJson(res, 413, {
          ok: false,
          error: err.message || 'Document too large to upload',
          code: 'PAYLOAD_TOO_LARGE'
        });
        return true;
      }
      sendJson(res, 400, { ok: false, error: err.message || 'Could not read upload', code: err.code || 'BAD_BODY' });
      return true;
    }
    if (body === null) {
      sendJson(res, 400, { ok: false, error: 'Invalid JSON', code: 'INVALID_JSON' });
  ...
  if (docFileMatch && req.method === 'GET') {
    if (!gateContractDealRead(req, res, docFileMatch[1])) return true;
  ...
  if (docFileMatch && req.method === 'DELETE') {
    if (!gateContractDealWrite(req, res, docFileMatch[1])) return true;
    try {
  ...
  if (mediaProxyMatch && req.method === 'GET') {
    if (!gateContractDealRead(req, res, mediaProxyMatch[1])) return true;
    try {
  ...
  if (photoSchedMatch && req.method === 'POST') {
    const user = requireContractDesk(req, res);
    if (!user) return true;
    if (denyBradSalesWrite(req, res, photoSchedMatch[1])) return true;
    const body = await readBody(req);
    if (body === null) {
      sendJson(res, 400, { ok: false, error: 'Invalid JSON', code: 'INVALID_JSON' });
      return true;
  ...
  if (photoTokenMatch && req.method === 'POST') {
    if (!gateContractDealWrite(req, res, photoTokenMatch[1])) return true;
    try {
  ...
  if (photoMsgMatch && req.method === 'GET') {
    if (!gateContractDealRead(req, res, photoMsgMatch[1])) return true;
    try {
  ...
  if (photoMsgMatch && req.method === 'POST') {
    const user = requireContractDesk(req, res);
    if (!user) return true;
    if (denyBradSalesWrite(req, res, photoMsgMatch[1])) return true;
    const body = await readBody(req);
    if (body === null) {
      sendJson(res, 400, { ok: false, error: 'Invalid JSON', code: 'INVALID_JSON' });
      return true;
  ...
  if (rehabScanMatch && req.method === 'POST') {
    if (!gateContractDealWrite(req, res, rehabScanMatch[1])) return true;
    const body = await readBody(req);
    if (body === null) {
      sendJson(res, 400, { ok: false, error: 'Invalid JSON', code: 'INVALID_JSON' });
      return true;
  ...
  if (rehabVoidMatch && req.method === 'POST') {
    const user = requireContractDesk(req, res);
    if (!user) return true;
    if (denyBradSalesWrite(req, res, rehabVoidMatch[1])) return true;
    const body = await readBody(req);
    if (body === null) {
      sendJson(res, 400, { ok: false, error: 'Invalid JSON', code: 'INVALID_JSON' });
      return true;
  ...
  if (rehabOptsMatch && req.method === 'POST') {
    if (!gateContractDealWrite(req, res, rehabOptsMatch[1])) return true;
    const body = await readBody(req);
    if (body === null) {
      sendJson(res, 400, { ok: false, error: 'Invalid JSON', code: 'INVALID_JSON' });
      return true;
  ...
  if (mediaZipMatch && req.method === 'GET') {
    if (!gateContractDealRead(req, res, mediaZipMatch[1])) return true;
    try {
  ...
  if (mediaListMatch && req.method === 'GET') {
    if (!gateContractDealRead(req, res, mediaListMatch[1])) return true;
  ...
  if (mediaListMatch && req.method === 'POST') {
    if (!gateContractDealWrite(req, res, mediaListMatch[1])) return true;
    let body;
    try {
      body = await readBody(req, { maxBytes: 26 * 1024 * 1024 });
    } catch (err) {
      if (err && err.code === 'PAYLOAD_TOO_LARGE') {
        sendJson(res, 413, {
          ok: false,
          error: err.message || 'Media upload too large',
          code: 'PAYLOAD_TOO_LARGE'
        });
        return true;
      }
  ...
  if (jvSendMatch && req.method === 'POST') {
    ...
    try {
      ...
      // JV auto-signs Brandon + Brad — never text/email "awaiting signatures".
      sendJson(res, 200, {
        ok: true,
        deal: enrichDealForDisplay(out.deal),
        jv: out.jv,
  ...
  if (amendmentMatch && req.method === 'POST') {
    ...
    try {
      ...
      // Wunderhaus/Buyer is already stamped on amendments — only text who still must sign.
      const party = out.amendment?.partyType === 'end_buyer' ? 'End buyer' : 'Seller';
      const invitees = Array.isArray(out.amendment?.invitees)
        ? out.amendment.invitees
        : Array.isArray(out.amendment?.sellers)
          ? out.amendment.sellers.map((s, i) => ({
            role: s.role || (i === 0 ? (party === 'End buyer' ? 'End buyer' : 'Seller 1') : 'Seller 2'),
            email: s.email,
            name: s.name
          }))
          : [];
      const awaiting = teamNotify.awaitingLabelsFromInvitees(invitees, [
        `${party} (${body.sellerEmail || body.counterpartyEmail || body.sellers?.[0]?.email || 'counterparty'})`
      ]);
      ...
      const status = err.code === 'NOT_FOUND' ? 404
        : err.code === 'SIGNNOW_NOT_CONFIGURED' ? 503
          : err.code === 'SIGNNOW_RATE_LIMIT' || err.status === 429 ? 429
            : err.code === 'SIGNNOW_API_ERROR' || err.code === 'SIGNNOW_SEND_FAILED' ? 502
              : 400;
      sendJson(res, status, {
        ok: false,
  ...
  if (sendDocMatch && req.method === 'POST') {
    ...
    try {
      ...
      sendJson(res, 200, {
        ok: true,
        deal: enrichDealForDisplay(out.deal),
        result: out.aoc || out.jv || out.amendment || out.psa || out,
        signNowConfigured: isSignNowConfigured()
      });
    } catch (err) {
      const status = err.code === 'NOT_FOUND' ? 404
  ...
  if (syncSnMatch && req.method === 'POST') {
    ...
    if (!user) return true;
    try {
      const out = await syncSignedSignNowDocuments(syncSnMatch[1], { scanLibrary: true });
      sendJson(res, 200, {
        ok: true,
        deal: enrichDealForDisplay(out.deal),
  ...
  if (dealPatchMatch && (req.method === 'PATCH' || req.method === 'POST')) {
    ...
    try {
      ...
      // Push desk seller contact edits back to GHL CRM (best effort)
      ...
      if (touchedContact && deal.ghlContactId && !deal.ghlContactLocked && isGhlConfigured()) {
        try {
          const emailOut = String(deal.ownerEmail || deal.email || '').trim();
          const phoneOut = String(deal.phone || '').trim();
          await updateContact(deal.ghlContactId, {
            email: emailOut || undefined,
            phone: phoneOut || undefined
          });
        } catch (ghlErr) {
          console.warn('[ghl] desk contact push failed:', ghlErr.message);
        }
      }
      sendJson(res, 200, { ok: true, deal: enrichDealForDisplay(deal) });
    } catch (err) {
  ...
  // GIF picker (Giphy proxy — Discord-like, rating=r). Desk-only.
  if (pathname === '/api/leads/admin/gifs/search' && req.method === 'GET') {
    if (!requireContractDesk(req, res)) return true;
    try {
      if (!giphyClient.isConfigured()) {
        sendJson(res, 503, {
          ok: false,
          error: 'GIF search needs GIPHY_API_KEY in server env. Free key: https://developers.giphy.com/',
      ...
      const out = await giphyClient.searchGifs(q, { limit, offset });
      sendJson(res, 200, { ok: true, configured: true, ...out });
    } catch (err) {
      const status = err.code === 'GIPHY_NOT_CONFIGURED' ? 503 : 502;
      sendJson(res, status, {
        ok: false,
        error: err.message || 'GIF search failed',
        code: err.code || 'GIPHY_ERROR',
        configured: giphyClient.isConfigured()
      });
    }
    return true;
  }

  if (pathname === '/api/leads/admin/gifs/trending' && req.method === 'GET') {
    if (!requireContractDesk(req, res)) return true;
    try {
      if (!giphyClient.isConfigured()) {
        sendJson(res, 503, {
      ...
      const limit = url.searchParams.get('limit') || '24';
      const offset = url.searchParams.get('offset') || '0';
      const out = await giphyClient.trendingGifs({ limit, offset });
      sendJson(res, 200, { ok: true, configured: true, ...out });
    } catch (err) {
      const status = err.code === 'GIPHY_NOT_CONFIGURED' ? 503 : 502;
      sendJson(res, status, {
        ok: false,
        error: err.message || 'GIF trending failed',
        code: err.code || 'GIPHY_ERROR',
        configured: giphyClient.isConfigured()
      });
    }
  ...
  if (pathname === '/api/leads/admin/gifs/status' && req.method === 'GET') {
    if (!requireContractDesk(req, res)) return true;
    sendJson(res, 200, {
      ok: true,
      configured: giphyClient.isConfigured(),
  ...
  if (teamMsgMatch && req.method === 'POST') {
    ...
    try {
      ...
      const out = addTeamMessage(teamMsgMatch[1], {
        ...
        gif: body.gif || body.attachment || null
      });
      const notifyBody = out.message?.gif && !(body.body || body.message || body.text)
        ? `GIF${out.message.gif.title ? `: ${out.message.gif.title}` : ''}`
        : out.message?.body;
  ...
  if (compManualMatch && req.method === 'POST') {
    ...
    const reportFiles = Array.isArray(lead.compReportFiles) ? lead.compReportFiles : [];
    if (!reportFiles.length) {
      sendJson(res, 400, {
        ok: false,
        error: 'Upload a Propelio / CMA report file before saving manual Comp ARV',
  ...
  const promoteToLandMatch = /^\/api\/leads\/([a-zA-Z0-9_-]+)\/promote-to-land$/.exec(pathname);
  if (promoteToLandMatch && req.method === 'POST') {
    const user = requireMax(req, res);
    if (!user) return true;
    const lead = getLead(promoteToLandMatch[1]);
    if (!lead) {
      sendJson(res, 404, { ok: false, error: 'Lead not found', code: 'NOT_FOUND' });
      return true;
    }
    ...
    try {
      const promoted = promoteLeadToTeardown(lead, {
        demoEstimate: body.demoEstimate,
        structureNote: body.structureNote,
        reason: body.reason || 'operator'
      });
      promoted.priorityScore = computePriorityScore(promoted);
      const saved = upsertLead(promoted);
      sendJson(res, 200, {
        ok: true,
        lead: saved,
        redirect: `/land-vault?lead=${encodeURIComponent(saved.leadId)}`
      });
    } catch (err) {
      const status = err.code === 'NOT_FOUND' ? 404
        : (err.code === 'ALREADY_LAND' || err.code === 'NOT_PROMOTABLE') ? 400
          : 500;
      sendJson(res, status, {
        ok: false,
  ...
  const landScreenMatch = /^\/api\/leads\/([a-zA-Z0-9_-]+)\/land-screen$/.exec(pathname);
  if (landScreenMatch && req.method === 'GET') {
    if (!requireMax(req, res)) return true;
    const lead = getLead(landScreenMatch[1]);
    if (!lead || lead.leadType !== 'land') {
      sendJson(res, 404, { ok: false, error: 'Land lead not found', code: 'NOT_FOUND' });
      return true;
    }
    sendJson(res, 200, {
      ok: true,
      landScreen: normalizeLandScreen(lead.landScreen || {}),
      fundMatches: Array.isArray(lead.fundMatches) ? lead.fundMatches : [],
      fundMatchedAt: lead.fundMatchedAt || null
    });
    return true;
  }

  if (landScreenMatch && req.method === 'PUT') {
    const user = requireMax(req, res);
    if (!user) return true;
    const lead = getLead(landScreenMatch[1]);
    if (!lead || lead.leadType !== 'land') {
    ...
    const screen = normalizeLandScreen(body.landScreen || body);
    screen.recommendedVerdict = recommendLandVerdict(screen);
    if (screen.verdict === 'keep' && screen.recommendedVerdict === 'toss' && !screen.verdictNote) {
      sendJson(res, 400, {
        ok: false,
        error: 'verdictNote required when keeping a lead that fails the 7-check screen',
        code: 'NOTE_REQUIRED'
      });
      return true;
    }
    screen.screenedAt = new Date().toISOString();
    screen.screenedBy = user;
    lead.landScreen = screen;
    lead.priorityScore = computePriorityScore(lead);
    const saved = upsertLead(lead);
    sendJson(res, 200, { ok: true, lead: saved, landScreen: saved.landScreen });
    return true;
  }

  const fundMatchRoute = /^\/api\/leads\/([a-zA-Z0-9_-]+)\/fund-match$/.exec(pathname);
  if (fundMatchRoute && req.method === 'POST') {
    if (!requireMax(req, res)) return true;
    const lead = getLead(fundMatchRoute[1]);
    if (!lead || lead.leadType !== 'land') {
      sendJson(res, 404, { ok: false, error: 'Land lead not found', code: 'NOT_FOUND' });
      return true;
    }
    const matches = matchLandFunds(lead);
    lead.fundMatches = matches;
    lead.fundMatchedAt = new Date().toISOString();
    const saved = upsertLead(lead);
    sendJson(res, 200, {
      ok: true,
      fundMatches: saved.fundMatches,
      fundMatchedAt: saved.fundMatchedAt,
      lead: saved
    });
    return true;
  }

  const landUwMatch = /^\/api\/leads\/([a-zA-Z0-9_-]+)\/land-underwriting$/.exec(pathname);
  if (landUwMatch && req.method === 'GET') {
    if (!requireMax(req, res)) return true;
    const lead = getLead(landUwMatch[1]);
    if (!lead || lead.leadType !== 'land') {
      sendJson(res, 404, { ok: false, error: 'Land lead not found', code: 'NOT_FOUND' });
      return true;
    }
    sendJson(res, 200, {
      ok: true,
      landUnderwriting: normalizeLandUnderwriting(lead.landUnderwriting || {})
    });
    return true;
  }

  if (landUwMatch && req.method === 'PUT') {
    const user = requireMax(req, res);
    if (!user) return true;
    const lead = getLead(landUwMatch[1]);
    if (!lead || lead.leadType !== 'land') {
    ...
    const uw = normalizeLandUnderwriting(body.landUnderwriting || body);
    uw.method = 'manual';
    uw.updatedAt = new Date().toISOString();
    lead.landUnderwriting = uw;
    lead.priorityScore = computePriorityScore(lead);
    const saved = upsertLead(lead);
    sendJson(res, 200, {
      ok: true,
      landUnderwriting: saved.landUnderwriting,
      lead: saved
    });
    return true;
  }

  const landCompManualMatch = /^\/api\/leads\/([a-zA-Z0-9_-]+)\/land-comp\/manual$/.exec(pathname);
  if (landCompManualMatch && req.method === 'POST') {
    if (!requireMax(req, res)) return true;
    const lead = getLead(landCompManualMatch[1]);
    if (!lead || lead.leadType !== 'land') {
      sendJson(res, 404, { ok: false, error: 'Land lead not found', code: 'NOT_FOUND' });
      return true;
    }
    ...
    try {
      const { leadPatch, report } = buildManualLandCompReport({
        lead,
        landFmv: body.landFmv,
        comps: body.comps,
        note: body.note,
        sanity: body.sanity
      });
      const saved = mergeCompOntoLead(lead, leadPatch);
      sendJson(res, 200, { ok: true, lead: saved, report, ...landCompSummaryFromLead(saved) });
    } catch (err) {
      const status = err.code === 'FMV_REQUIRED' || err.code === 'COMPS_REQUIRED'
        || err.code === 'COMP_PRICE_REQUIRED' ? 400 : 500;
      sendJson(res, status, { ok: false, error: err.message, code: err.code || 'ERROR' });
    }
    return true;
  ...
  const landCompMatch = /^\/api\/leads\/([a-zA-Z0-9_-]+)\/land-comp$/.exec(pathname);
  if (landCompMatch && req.method === 'GET') {
    if (!requireMax(req, res)) return true;
    const lead = getLead(landCompMatch[1]);
    if (!lead || lead.leadType !== 'land') {
      sendJson(res, 404, { ok: false, error: 'Land lead not found', code: 'NOT_FOUND' });
      return true;
    }
    sendJson(res, 200, { ok: true, ...landCompSummaryFromLead(lead) });
    return true;
  }

  if (landCompMatch && req.method === 'POST') {
    if (!requireMax(req, res)) return true;
    const lead = getLead(landCompMatch[1]);
    if (!lead || lead.leadType !== 'land') {
      sendJson(res, 404, { ok: false, error: 'Land lead not found', code: 'NOT_FOUND' });
    ...
    const replace = body.replace === true || url.searchParams.get('replace') === '1';
    if (lead.landCompedAt && !replace) {
      sendJson(res, 200, {
        ok: true,
        confirmReplace: true,
        lead,
        report: lead.landCompingReport || null
      });
      return true;
    }
    if (isNonDisclosureState(lead.state)) {
    ...
    try {
      const out = await runLandAutoComp(lead, { reapi });
      if (out.needsManual) {
        if (out.leadPatch) mergeCompOntoLead(lead, out.leadPatch);
        sendJson(res, 200, {
          ok: false,
          needsManual: true,
          reason: out.reason || 'thin_market',
          report: out.report || null,
          lead: getLead(lead.leadId)
        });
        return true;
      }
      if (!out.ok) {
        sendJson(res, 400, {
          ok: false,
          error: out.error || 'Land Comp failed',
          code: out.code || 'LAND_COMP_FAILED'
        });
        return true;
      }
      const saved = mergeCompOntoLead(lead, out.leadPatch);
      sendJson(res, 200, { ok: true, lead: saved, report: out.report, ...landCompSummaryFromLead(saved) });
    } catch (err) {
      sendJson(res, 500, {
        ok: false,
        error: err.message || 'Land Comp failed',
        code: err.code || 'LAND_COMP_FAILED'
      });
    }
    return true;
  }
  ...
  if (detailMatch && req.method === 'GET') {
    ...
    const user = readPhugleeUser(req);
    const overlays = readOverlays(user);
    const enriched = enrichLeadProof(lead);
    sendJson(res, 200, {
      ok: true,
      lead: enriched,
      scoreExplain: explainPriorityScore(enriched),
  ...
  const skipTraceMatch = /^\/api\/leads\/([a-zA-Z0-9_-]+)\/skip-trace$/.exec(pathname);
  if (skipTraceMatch && req.method === 'POST') {
    if (!requireMax(req, res)) return true;
    const lead = getLead(skipTraceMatch[1]);
    if (!lead) {
      sendJson(res, 404, { ok: false, error: 'Lead not found', code: 'NOT_FOUND' });
      return true;
    }
    ...
    const existing = lead.skipTrace && typeof lead.skipTrace === 'object' ? lead.skipTrace : null;
    if (existing?.tracedAt && (phones.length || existing.status === 'miss' || existing.status === 'hit')) {
      sendJson(res, 200, {
        ok: true,
        cached: true,
        lead: enrichLeadProof(lead),
        skipTrace: {
          ...existing,
          cached: true,
          peopleFound: existing.peopleFound != null ? existing.peopleFound : (phones.length ? 1 : 0)
        }
      });
      return true;
    }

    // Cache whatever contacts already live on the lead (Analyze import / CSV enrich).
    // No third-party skip provider in-app — reload must not re-spend.
    const skipTrace = phones.length
      ? {
        status: 'hit',
        tracedAt: new Date().toISOString(),
        source: lead.enrichmentSource || 'catalog',
        peopleFound: 1,
        cached: false
      }
      : {
    ...
    const saved = upsertLead({ ...lead, skipTrace });
    sendJson(res, 200, {
      ok: true,
      cached: false,
      lead: enrichLeadProof(saved),
      skipTrace,
      message: phones.length
        ? 'Contacts loaded from this lead and cached.'
        : 'No phone on file yet. Import skip data or admin-enrich, then lookup again.'
    });
    return true;
  }
  ...
  // Fill land parcel fields from RealEstateAPI PropertyDetail (fill-blanks).
  if (pathname === '/api/leads/admin/enrich-land-reapi' && req.method === 'POST') {
    if (!requireAdmin(req, res)) return true;
    const body = (await readBody(req)) || {};
    const reapi = createReapiClientFromConfig();
    if (!reapi) {
    ...
    const concurrency = Math.min(5, Math.max(1, Number(body.concurrency) || 2));
    const delayMs = Math.max(0, Number(body.delayMs) || 120);

    let landIds = Array.isArray(body.leadIds)
      ? body.leadIds.map(String).filter(Boolean)
      : readIndex()
        .filter((e) => e.leadType === 'land' && (e.catalogStatus || 'active') === 'active')
        .map((e) => e.leadId);

    const totalLand = landIds.length;
    landIds = landIds.slice(offset, offset + limit);
    const leads = landIds.map((id) => getLead(id)).filter((l) => l && l.leadType === 'land');
    ...
    try {
      ...
      if (!dryRun) {
        for (const r of results) {
          ...
          const check = validateLeadRecord(normalizeLeadRecord(r.lead));
          if (!check.ok) {
            summary.errors += 1;
            continue;
          }
      ...
      const batch = dryRun ? { published: 0, unchanged: 0 } : upsertLeadsBatch(toUpsert);
      ...
      sendJson(res, 200, {
        ...
        nextOffset: offset + leads.length < totalLand ? offset + leads.length : null,
        summary,
        updated: batch.published || 0,
        unchanged: batch.unchanged || 0,
      ...
      sendJson(res, 500, {
        ok: false,
        error: err.message || 'Land REAPI enrich failed',
        code: 'REAPI_ENRICH_FAILED'
      });
    }
    return true;
