  ...
  if (dealGetMatch && req.method === 'GET') {
    ...
    if (profile.ghlContactId && isGhlConfigured()) {
      try {
        ...
        // Persist seller email from GHL contact page so Amendment form auto-fills
        if (contact?.email) {
          const emailPatch = {};
          if (!profile.email) emailPatch.email = contact.email;
          if (!profile.ownerEmail) emailPatch.ownerEmail = contact.email;
          if (Object.keys(emailPatch).length) {
            try {
              patchDeal(profile.dealId, emailPatch);
              profile = getDealProfile(profile.dealId) || profile;
            } catch (_) { /* optional */ }
          }
        }
