...
async function handle(req, res, pathname, url) {
  ...
  if (pathname === '/api/leads/admin/contracts/from-vault' && req.method === 'POST') {
    ...
    try {
      ...
      let parcel = null;
      try {
        parcel = await ensureDealContractParcel(deal.dealId);
      } catch (ensErr) {
        parcel = { ok: false, error: ensErr.message, fields: null };
      }
      sendJson(res, 200, {
        ok: true,
        deal: enrichDealForDisplay(parcel?.deal || deal),
        parcel: parcel
          ? {
            fields: parcel.fields,
            filled: parcel.filled,
            pulled: parcel.pulled,
            skipped: parcel.skipped,
  ...
  const ensureParcelMatch = /^\/api\/leads\/admin\/contracts\/([a-zA-Z0-9_-]+)\/ensure-parcel$/.exec(pathname);
  if (ensureParcelMatch && req.method === 'POST') {
    const user = requireContractDesk(req, res);
    if (!user) return true;
    if (denyBradSalesWrite(req, res, ensureParcelMatch[1])) return true;
    const body = (await readBody(req)) || {};
    try {
      const out = await ensureDealContractParcel(ensureParcelMatch[1], {
        force: body.force === true
      });
      sendJson(res, 200, {
        ok: true,
