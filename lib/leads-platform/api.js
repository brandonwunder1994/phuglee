...
const { readOverlays, toggleFavorite, upsertNote, savePresets } = require('./user-overlays');
const { publishLead, publishBatch } = require('./publish');
const { ensureAnalyzerSync } = require('./analyzer-sync');
...
async function handle(req, res, pathname, url) {
  ...
  if (pathname === '/api/leads/meta' && req.method === 'GET') {
    ...
    sendJson(res, 200, { ok: true, meta: getMeta() });
    return true;
  }

  if (pathname === '/api/leads' && req.method === 'GET') {
    if (!requireMax(req, res)) return true;
    const result = queryLeads(parseListQuery(url));
