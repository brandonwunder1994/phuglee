...
function warmCatalogIndex() {
  ...
  return { leads: index.length, ms: Date.now() - t0 };
}
...
/**
 * Active Vault leads for address typeahead — ranked by address relevance.
 */
function searchActiveVaultLeads(q, opts = {}) {
  ...
  for (const entry of readIndex()) {
    const status = String(entry.catalogStatus || 'active').toLowerCase();
