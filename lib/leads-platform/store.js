...
function indexEntryFromLead(lead) {
  ...
  return {
    ...
    hasPhone: Array.isArray(lead.phones) && lead.phones.length > 0,
...
function queryLeads(query = {}) {
  ...
  for (const entry of index) {
    ...
    filtered.push({
      ...
      thumbUrl: entry.thumbUrl || lead.streetViewUrl || (lead.photos && lead.photos[0]) || '',
      distressTier: entry.distressTier != null ? entry.distressTier : lead.distressTier
    });
  }
