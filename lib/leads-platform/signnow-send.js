...
async function sendAocForDeal(deal, input = {}) {
  ...
  // Reject zero / non-positive money fields that still stringify as present ("$0.00").
  const priceNum = Number(String(values['Assignee Purchase Price'] || '').replace(/[^0-9.-]/g, ''));
  const emdNum = Number(String(values['Assignee EMD'] || '').replace(/[^0-9.-]/g, ''));
  if (!Number.isFinite(priceNum) || priceNum <= 0) {
    const err = new Error('Assignee Purchase Price must be greater than $0');
    err.code = 'MISSING_AOC_FIELDS';
    throw err;
  }
  if (!Number.isFinite(emdNum) || emdNum <= 0) {
    const err = new Error('Assignee EMD must be greater than $0');
    err.code = 'MISSING_AOC_FIELDS';
    throw err;
  }
...
function markKindSignedOnDeal(deal, kind, documentId, signedAt) {
  ...
  if (k === 'purchase_contract' || k === 'cash' || k === 'psa') {
    const prev = deal.cashPsaSend && typeof deal.cashPsaSend === 'object' ? deal.cashPsaSend : {};
    const signedIso = signedAt || new Date().toISOString();
    let originalAgreementDate = slug(deal.originalAgreementDate);
    if (!originalAgreementDate) {
      // Format in America/Chicago so late US signatures aren't pushed to the next UTC day.
      try {
        const parts = new Intl.DateTimeFormat('en-US', {
          ...
          month: '2-digit',
          day: '2-digit'
        }).formatToParts(new Date(signedIso));
        const get = (t) => parts.find((p) => p.type === t)?.value;
        const m = get('month');
        const day = get('day');
        const y = get('year');
        if (m && day && y) originalAgreementDate = `${m}/${day}/${y}`;
      } catch (_) {
        const d = new Date(signedIso);
        if (!Number.isNaN(d.getTime())) {
          const m = String(d.getMonth() + 1).padStart(2, '0');
          const day = String(d.getDate()).padStart(2, '0');
          originalAgreementDate = `${m}/${day}/${d.getFullYear()}`;
        }
      }
    }
    const stage = slug(deal.stage) === 'contract_sent' ? 'under_contract' : deal.stage;
    return {
      ...deal,
      stage,
      originalAgreementDate: originalAgreementDate || deal.originalAgreementDate || '',
      cashPsaSend: {
        ...prev,
        status: 'signed',
        signedAt: signedIso,
...
function isSubjectToDeal(deal, input = {}) {
  const dealType = slug(deal?.dealType || input.dealType).toLowerCase().replace(/[\s-]+/g, '_');
  return dealType === 'subject_to' || dealType === 'subto' || dealType === 'sub_to';
}

function isCashDeal(deal, input = {}) {
  const dealType = slug(deal?.dealType || input.dealType).toLowerCase().replace(/[\s-]+/g, '_');
  return dealType === 'cash' || dealType === 'cash_deal' || dealType === 'purchase_cash';
}

/** Invite email / toast copy for amendments. */
function amendmentInviteCopy(deal, prop) {
  const isSubto = isSubjectToDeal(deal);
  const agreementLabel = isSubto ? 'Purchase Contract' : 'Purchase Agreement';
  return {
    agreementLabel,
    subject: `Amendment to ${agreementLabel} — ${prop || 'Property'}`,
    message: isSubto
      ? `${SENDER.entity} has sent an amendment to the subject-to purchase contract for your review and signature.`
      : `${SENDER.entity} has sent an amendment for your review and signature.`
  };
}

/**
 * Interim SubTo field map (SignNow Text Field N) from the no-seller-carry PSA layout.
 */
const SUBTO_FIELD_MAP = {
  street: 'Text Field 1',
  cityStateZip: 'Text Field 2',
  apn: 'Text Field 3',
  legal: 'Text Field 4',
  purchasePrice: 'Text Field 5',
  emd: 'Text Field 6',
  existingFirst: 'Text Field 7',
  existingSecond: 'Text Field 8',
  newLoan: 'Text Field 9',
  sellerCarry: 'Text Field 10',
  cureDefaults: 'Text Field 11',
  other: 'Text Field 12',
  otherNote: 'Text Field 44',
  cashAtCoe: 'Text Field 13',
  inspectionDays: 'Text Field 15',
  closingLocation: 'Text Field 17',
  sellerName: 'Text Field 45',
  buyerEntity: 'Text Field 20'
};
...
function requirePsaTemplate(templateKey) {
  const t = TEMPLATES[templateKey];
  if (!t || !slug(t.templateId)) {
    const err = new Error(
      templateKey === 'subto'
        ? 'Subject-to SignNow template is not registered yet. Convert your blank SubTo to a Template in SignNow, then add its templateId to signnow-templates.json (key: subto).'
        : `SignNow template not configured: ${templateKey}`
    );
    err.code = 'SIGNNOW_TEMPLATE_MISSING';
    throw err;
  }
  return t;
}
 ...
 * Buyer (Brandon) real signature + dates are stamped — sellers are the only invitees.
 * Amendment template has no baked signature; reuse Brandon's AOC signature image (same as Cash PSA).
 */
async function sendAmendmentForDeal(deal, input = {}) {
  ...
  // Prefer deal PSA-locked date; ignore client override to prevent spoofed original dates.
  const originalAgreementDate = slug(
    deal.originalAgreementDate
      || input.originalAgreementDate
      || deal.agreementDate
      || deal.contractDate
  );
  if (!originalAgreementDate) {
  ...
  const aocTpl = getTemplate('aoc');
  const ownerArtifacts = await loadOwnerArtifactsFromTemplate(aocTpl.templateId, {
    emailIncludes: 'brandon',
    kind: 'signature'
  });
  if (!ownerArtifacts.signatures.length) {
    const err = new Error(
      'Could not load Brandon\'s signature from the AOC template. Open the AOC template in SignNow, place your signature, then try again.'
    );
    err.code = 'AMENDMENT_MISSING_BUYER_SIGNATURE';
    throw err;
  }
  ...
  let buyerSigBox = null;
  try {
    const doc = await getDocument(documentId);
    termCaps = capacitiesForNamedFields(doc, termFields);
    const sigField = (doc.fields || []).find((f) => {
      const n = String(f.json_attributes?.name || '');
      return f.role === 'Buyer' && (f.type === 'signature' || /buyer_signature/i.test(n));
    });
    if (sigField?.json_attributes) buyerSigBox = sigField.json_attributes;
  } catch (_) { /* fall back */ }
  ...
  const srcSig = ownerArtifacts.signatures[0];
  const placedSig = buyerSigBox
    ? {
      page_number: Number(buyerSigBox.page_number),
      x: Number(buyerSigBox.x),
      y: Number(buyerSigBox.y),
      width: Number(buyerSigBox.width) || Number(srcSig.width),
      height: Number(buyerSigBox.height) || Number(srcSig.height),
      data: srcSig.data
    }
    : srcSig;
  ...
  await applyPrefillStampingRole(documentId, values, {
    ...
    dropStampSignatures: true,
    artifacts: {
      signatures: [placedSig]
    }
  });

  const { subject, message, agreementLabel } = amendmentInviteCopy(deal, prop);
  const { fromEmail } = getConfig();
  ...
  return {
    ...
    sellers,
    invitees: to,
    message: `Amendment (${agreementLabel}) sent via SignNow to ${sellers.map((s) => s.email).join(', ')}. Buyer signature + ${sendDate} applied automatically.`
  };
}

/**
 * Send Cash Purchase Agreement via SignNow.
 * Buyer (Brandon) real signature + dates are stamped — sellers are the only invitees.
 * Cash template has no baked signature; we reuse Brandon's AOC signature image.
 */
async function sendCashForDeal(deal, input = {}) {
  requireConfigured();
  ...
  const purchasePrice = input.purchasePrice != null && input.purchasePrice !== ''
    ? input.purchasePrice
    : deal.purchasePrice;
  const purchaseNum = Number(String(purchasePrice == null ? '' : purchasePrice).replace(/[^0-9.-]/g, ''));
  if (purchasePrice == null || purchasePrice === '' || !Number.isFinite(purchaseNum) || purchaseNum <= 0) {
    const err = new Error('Purchase price must be greater than $0 to send a cash purchase agreement');
    err.code = 'MISSING_PURCHASE_PRICE';
    throw err;
  }

  const sendDate = todayUs();
  const agreementDate = slug(input.agreementDate) || sendDate;
  const { street, cityLine } = propertyLines(deal);
  const prop = propertyLine(deal);
  const legalDescription = slug(
    input.legalDescription
      || deal.aocSend?.legalDescription
      || loadLinkedLead(deal)?.propertyDetails?.legalDescription
  );
  // Default $100 EMD — template has no dollar sign printed.
  const emdRaw = input.emdDeposit != null && input.emdDeposit !== ''
    ? input.emdDeposit
    : (deal.emdDeposit != null && deal.emdDeposit !== '' ? deal.emdDeposit : 100);
  const closingLocation = slug(input.closingLocation) || "Buyer's Choice";
  const inspectionDays = slug(input.inspectionDays) || '10';
  const additionalTerms = slug(input.additionalTerms || input.terms);

  const tpl = getTemplate('cash');
  const aocTpl = getTemplate('aoc');
  const ownerArtifacts = await loadOwnerArtifactsFromTemplate(aocTpl.templateId, {
    emailIncludes: 'brandon',
    kind: 'signature'
  ...
  if (!ownerArtifacts.signatures.length) {
    ...
    err.code = 'CASH_MISSING_BUYER_SIGNATURE';
    throw err;
  }

  const docName = `Cash PSA — ${prop || deal.dealId}`;
  const documentId = await copyTemplate(tpl.templateId, docName);

  const termFields = ['additional_terms_1', 'additional_terms_2'];
  let termCaps = termFields.map(() => DEFAULT_TERM_FIELD_CHARS);
  let buyerSigBox = null;
  try {
    const doc = await getDocument(documentId);
    ...
    if (sigField?.json_attributes) {
      buyerSigBox = sigField.json_attributes;
    }
  } catch (_) { /* fall back */ }
  const termLines = splitAcrossLines(additionalTerms, termCaps);

  const values = {
    agreement_date: agreementDate,
    seller_names: sellers.map((s) => s.name).join(' / '),
    street_address: street || '',
    city_state_zip: cityLine || '',
    legal_description: legalDescription || '',
    emd_deposit: formatMoney(emdRaw),
    purchase_price: formatMoney(purchasePrice),
    closing_location: closingLocation,
    inspection_days: inspectionDays,
    additional_terms_1: termLines[0] || '',
    additional_terms_2: termLines[1] || '',
    buyer_date: sendDate,
    seller_1_name: sellers[0]?.name || '',
    seller_2_name: sellers[1]?.name || ''
  };
  ...
  await sendInvite({
    documentId,
    from: fromEmail,
    subject: `Cash Purchase Agreement — ${prop || 'Property'}`,
    message: `${SENDER.entity} has sent a cash purchase agreement for your review and signature.`,
    to
  });
  ...
  return {
    ...
    sellers,
    invitees: to,
    message: `Cash PSA sent via SignNow to ${sellers.map((s) => s.email).join(', ')}. Buyer (${SENDER.name}) already stamped.`
  };
}

/**
 * Send Subject-To Purchase Agreement (no seller carry) via SignNow.
 */
async function sendSubtoForDeal(deal, input = {}) {
  requireConfigured();
  const tpl = requirePsaTemplate('subto');
  ...
  if (!sellers.length) {
    const err = new Error('Seller name + email required to send a subject-to purchase agreement');
    err.code = 'MISSING_SELLERS';
    throw err;
  }
  ...
  const { street, cityLine } = propertyLines(deal);
  const prop = propertyLine(deal);
  if (!street && !cityLine && !prop) {
    const err = new Error('Deal needs a property address before sending a subject-to purchase contract');
    err.code = 'MISSING_PROPERTY_ADDRESS';
    throw err;
  }

  const purchasePrice = input.purchasePrice != null ? input.purchasePrice : deal.purchasePrice;
  const emdDeposit = input.emdDeposit != null ? input.emdDeposit : deal.emdDeposit;
  const legal = slug(input.legalDescription || deal.aocSend?.legalDescription);
  const apn = slug(input.apn || deal.aocSend?.apn);

  const docName = `SubTo PSA — ${prop || deal.dealId}`;
  const documentId = await copyTemplate(tpl.templateId, docName);

  const values = {
    [SUBTO_FIELD_MAP.street]: street || prop,
    [SUBTO_FIELD_MAP.cityStateZip]: cityLine,
    [SUBTO_FIELD_MAP.apn]: apn,
    [SUBTO_FIELD_MAP.legal]: legal,
    [SUBTO_FIELD_MAP.purchasePrice]: moneyPlain(purchasePrice),
    [SUBTO_FIELD_MAP.emd]: moneyPlain(emdDeposit),
    [SUBTO_FIELD_MAP.existingFirst]: moneyPlain(input.existingFirst != null ? input.existingFirst : ''),
    [SUBTO_FIELD_MAP.existingSecond]: moneyPlain(input.existingSecond != null ? input.existingSecond : 0),
    [SUBTO_FIELD_MAP.newLoan]: moneyPlain(input.newLoan != null ? input.newLoan : 0),
    [SUBTO_FIELD_MAP.sellerCarry]: moneyPlain(input.sellerCarry != null ? input.sellerCarry : 0),
    [SUBTO_FIELD_MAP.cureDefaults]: moneyPlain(input.cureDefaults != null ? input.cureDefaults : 0),
    [SUBTO_FIELD_MAP.other]: moneyPlain(input.otherAmount != null ? input.otherAmount : 0),
    [SUBTO_FIELD_MAP.otherNote]: slug(input.otherNote) || 'NA',
    [SUBTO_FIELD_MAP.cashAtCoe]: moneyPlain(input.cashAtCoe != null ? input.cashAtCoe : ''),
    [SUBTO_FIELD_MAP.inspectionDays]: slug(input.inspectionDays) || '10',
    [SUBTO_FIELD_MAP.closingLocation]: slug(input.closingLocation) || "Buyer's Choice",
    [SUBTO_FIELD_MAP.sellerName]: sellers[0].name,
    [SUBTO_FIELD_MAP.buyerEntity]: SENDER.entity
  };

  await applyPrefill(documentId, values, { skipRoles: [] });

  const subject = `SubTo PSA — ${prop || 'Property'}`;
  const message = `${SENDER.entity} has sent a subject-to purchase contract for ${prop || 'this property'}.`;
  const { fromEmail } = getConfig();
  const to = [
  ...
  return {
    ...
    sellers,
    invitees: to,
    message: `SubTo PSA sent via SignNow to ${SENDER.email} (Buyer) and ${sellers.map((s) => s.email).join(', ')} (Seller).`
  };
}

/**
 * Send purchase contract by deal type (cash vs subject-to).
 */
async function sendPsaForDeal(deal, input = {}) {
  if (isSubjectToDeal(deal, input) || slug(input.kind).toLowerCase() === 'subto') {
    return sendSubtoForDeal(deal, input);
  }
  if (isCashDeal(deal, input) || slug(input.kind).toLowerCase() === 'cash') {
    return sendCashForDeal(deal, input);
  }
  // Default psa/purchase_contract without dealType → cash (legacy Send New PSA)
  if (!slug(deal?.dealType || input.dealType)) {
    return sendCashForDeal(deal, input);
  }
  const err = new Error('Set deal type to Cash or Subject-to before sending a purchase contract');
...
async function sendDocumentForDeal(deal, kind, input = {}) {
  ...
  if (k === 'subto' || k === 'subject_to') return sendSubtoForDeal(deal, input);
  if (k === 'cash') return sendCashForDeal(deal, input);
  if (k === 'purchase_contract' || k === 'psa') return sendPsaForDeal(deal, input);
...
function kindFromTemplateKey(key) {
  ...
  if (key === 'cash' || key === 'subto') return 'purchase_contract';
  return 'other';
}
...
function inviteLooksSigned(status) {
  const s = String(status || '').toLowerCase();
  return s === 'fulfilled' || s === 'signed' || s === 'completed' || s === 'finished';
}

function inviteLooksOpened(status) {
  const s = String(status || '').toLowerCase();
  if (!s) return false;
...
/**
 * Per-seller open/sign alerts for Cash PSA (and generic docs as fallback).
 * Two sellers: alert each open + first signer; final signer only gets the
 * single "fully signed" completion text (no duplicate signed+complete).
 */
async function maybeAlertSignNowLifecycle(deal, item, {
  opened,
  signed,
  openedByEmail,
  invites = [],
  ...
  if (!shouldNotifySignNowKind(item.kind, item.templateKey)) {
    return {
      openedAlertedAt: item.openedAlertedAt || null,
      signedAlertedAt: item.signedAlertedAt || null,
      signerAlerts: item.signerAlerts || {}
    };
  }

  let openedAlertedAt = item.openedAlertedAt || null;
  let signedAlertedAt = item.signedAlertedAt || null;
  const signerAlerts = { ...(item.signerAlerts && typeof item.signerAlerts === 'object' ? item.signerAlerts : {}) };
  const kind = kindFromTemplateKey(item.templateKey) || item.kind || 'other';
  const isCashPsa = kind === 'purchase_contract' || item.templateKey === 'cash';
  ...
  try {
    ...
    const team = new Set(teamSignerEmails());
    const sellerInvites = (Array.isArray(invites) ? invites : [])
      .map((inv) => ({
        email: slug(inv.email).toLowerCase(),
        role: slug(inv.role),
        status: slug(inv.status || inv.status_code)
      }))
      .filter((inv) => inv.email && !team.has(inv.email));

    if (isCashPsa && sellerInvites.length) {
      const signedCount = sellerInvites.filter((inv) => inviteLooksSigned(inv.status)).length;
      const totalSellers = sellerInvites.length;

      for (const inv of sellerInvites) {
        const prev = signerAlerts[inv.email] || {};
        const roleLabel = inv.role || (inv.email === openedByEmail ? 'Seller' : 'Seller');
        if (inviteLooksOpened(inv.status) && !prev.openedAlertedAt) {
          const out = await teamNotify.alertSignNowOpened({
            deal,
            kind,
            openedByEmail: inv.email,
            roleLabel
          });
          if (alertLanded(out)) {
            signerAlerts[inv.email] = {
              ...prev,
              role: roleLabel,
              openedAlertedAt: new Date().toISOString(),
              signedAlertedAt: prev.signedAlertedAt || null
            };
            openedAlertedAt = openedAlertedAt || signerAlerts[inv.email].openedAlertedAt;
          }
        }

        const cur = signerAlerts[inv.email] || prev;
        if (inviteLooksSigned(inv.status) && !cur.signedAlertedAt) {
          const isLastSigner = fullySigned || signedCount >= totalSellers;
          if (isLastSigner) {
            // Completion alert is sent once below — mark this signer so we do not
            // also fire a separate "seller signed" text.
            signerAlerts[inv.email] = {
              ...cur,
              role: roleLabel,
              openedAlertedAt: cur.openedAlertedAt || new Date().toISOString(),
              signedAlertedAt: 'pending_complete'
            };
          } else {
            const out = await teamNotify.alertSignNowSigned({
              deal,
              kind,
              roleLabel,
              openedByEmail: inv.email,
      ...
      if ((fullySigned || signed) && !signedAlertedAt) {
        const out = await teamNotify.alertSignNowSigned({ deal, kind, complete: true });
        if (alertLanded(out)) {
          signedAlertedAt = new Date().toISOString();
          for (const inv of sellerInvites) {
            const cur = signerAlerts[inv.email] || {};
            if (cur.signedAlertedAt === 'pending_complete' || inviteLooksSigned(inv.status)) {
              signerAlerts[inv.email] = {
                ...cur,
                signedAlertedAt: signedAlertedAt
              };
            }
          }
        }
      ...
      // AOC / Amendment / other: one open + one fully-signed alert.
  ...
  return { openedAlertedAt, signedAlertedAt, signerAlerts };
}
 ...
 * Cash PSA: per-seller open/sign alerts; one completion text when fully signed.
...
async function ingestSignedSignNowDocuments(deal, { addDealDocument, upsertDeal, patchPending }) {
  ...
  for (const item of pending) {
    ...
    try {
      ...
      // Cash PSA keeps polling invite status every sync so Seller 2 open/sign can alert.
      if (notify && (isCashPsa || !item.openedAlertedAt)) {
        let history = [];
        try {
      ...
      if (!fullySigned) {
        ...
        remaining.push({
          ...
          signerAlerts: alertMarks.signerAlerts
        });
        continue;
      }
