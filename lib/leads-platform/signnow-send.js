 ...
 * Prefills Assignor-side deal/form fields; Assignee identity fields stay blank for the buyer to complete.
 * Invites: Assignor → Brandon, Assignee → buyer email from the form.
 */
async function sendAocForDeal(deal, input = {}) {
  ...
  return {
    ...
    templateKey: 'aoc',
    invitees: [
      { role: 'Assignor', email: SENDER.email },
...
/** Normalize for fuzzy address matching against SignNow document names. */
...
/**
 * Infer template key from a SignNow document_name.
 * @returns {'amendment'|'aoc'|'jv'|null}
 */
function inferTemplateKeyFromDocName(documentName) {
  const n = normalizeMatchText(documentName);
  if (!n) return null;
  if (/\bamendment\b|\baddendum\b/.test(n)) return 'amendment';
  if (/\baoc\b|\bassignment of contract\b|\bassignment\b/.test(n)) return 'aoc';
  if (/\bjv\b|\bjoint venture\b/.test(n)) return 'jv';
  return null;
}

/**
 * True when a SignNow library document name belongs to this deal's property.
 */
function signNowDocNameMatchesDeal(documentName, deal) {
  const street = normalizeMatchText(deal?.address);
  if (!street || street.length < 5) return false;
  const name = normalizeMatchText(documentName);
  if (!name.includes(street)) return false;
  const city = normalizeMatchText(deal?.city);
  const state = normalizeMatchText(deal?.state);
  // Prefer city/state when present so two "Main St" deals don't collide.
  if (city && city.length >= 3 && !name.includes(city)) return false;
  if (state && state.length === 2 && !name.includes(state)) return false;
  return true;
}
...
async function maybeAlertSignNowLifecycle(deal, item, { opened, signed, openedByEmail }) {
  ...
  try {
    ...
    // Fire "opened/viewed" even when we first notice the package already fully signed —
    // otherwise a late poll (or no Contract Tracker open) silently drops the viewed text.
    if (opened && !openedAlertedAt) {
