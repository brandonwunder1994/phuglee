 ...
 * flattened onto Party A's signature line. Wipe that band so we can stamp hand-drawn only.
 */
async function whiteoutJvPartyASignature(pdfBuffer) {
  ...
  // SignNow coords are top-left origin in PDF points on letter pages.
  // Cover Party A signature ink only — keep the "Signature:" label visible.
  const snX = 128;
  const snY = 118;
  const snW = 220;
  const snH = 42;
...
async function sendJvForDeal(deal) {
  ...
  // Template PDF already has typed+hand-drawn baked on Party A — wipe that band, upload clean.
