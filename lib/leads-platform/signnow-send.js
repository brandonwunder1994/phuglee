...
async function sendAocForDeal(deal, input = {}) {
  ...
  await applyPrefillStampingRole(documentId, values, {
    ...
    artifacts: {
      ...
      texts: [...(ownerArtifacts.texts || []), dateBesideSig]
    }
  });
...
/**
 * Complete JV in one click: stamp property + both dates, re-apply Brandon + Brad
 ...
 * signature + identity texts come from the JV template.
 */
async function sendJvForDeal(deal) {
  ...
  // Brandon: signature image + identity texts from the template (copy drops these).
  const tplDoc = await getDocument(tpl.templateId);
  const brandonArts = ownerArtifactsFromDoc(tplDoc, { kind: 'signature' });
  const identityTexts = ownerArtifactsFromDoc(tplDoc).texts || [];
  if (!brandonArts.signatures.length) {
    const err = new Error(
      'JV template is missing Brandon\'s Wunderhaus signature. Open the JV template in SignNow and place your signature, then try again.'
    );
  ...
  await applyCompleteOwnerStamp(
    ...
    },
    {
      artifacts: {
        texts: identityTexts,
        signatures: [...brandonArts.signatures, ...bradArts.signatures]
      }
    }
  );
