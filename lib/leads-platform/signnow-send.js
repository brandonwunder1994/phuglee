...
function propertyLine(deal) {
  const street = slug(deal.address);
  const cityLine = [slug(deal.city), [slug(deal.state), slug(deal.zip)].filter(Boolean).join(' ')]
    .filter(Boolean)
    .join(', ');
...
/**
 * Send JV between hardwired Brandon (Party A) + Brad (Party B).
 */
async function sendJvForDeal(deal) {
  requireConfigured();
  const tpl = getTemplate('jv');
  const prop = propertyLine(deal);
  const docName = `JV Agreement — ${prop || deal.dealId}`;
  const documentId = await copyTemplate(tpl.templateId, docName);

  const values = {
    property_line_1: prop,
    property_line_2: '',
    party_a_name: `${SENDER.entity} / ${SENDER.name}`,
    party_a_email: SENDER.email,
    party_b_name: `${BRAD.company} / ${BRAD.name}`,
    party_b_email: BRAD.email,
    party_a_expense_pct: '70',
    party_b_expense_pct: '30',
    party_a_add_pct: '70',
    party_b_add_pct: '30',
    party_a_signer_name: SENDER.name,
    party_a_title: SENDER.title,
    party_b_signer_name: BRAD.name,
    title_holder: SENDER.entity
  };

  await applyPrefill(documentId, values, { skipRoles: ['Party C'] });

  const subject = `JV Agreement — ${prop || 'Property'}`;
  ...
  await sendInvite({
    ...
    message,
    to: [
      { email: SENDER.email, role: 'Party A', order: 1 },
      { email: BRAD.email, role: 'Party B', order: 2 }
    ]
  });
  ...
  return {
    ...
    message: `JV sent via SignNow to ${SENDER.email} and ${BRAD.email}.`
  };
}
...
async function sendAmendmentForDeal(deal, input = {}) {
  ...
  const originalAgreementDate = slug(
    input.originalAgreementDate || deal.originalAgreementDate || deal.agreementDate || deal.contractDate
  );
  if (!originalAgreementDate) {
    const err = new Error('Original agreement date is required');
