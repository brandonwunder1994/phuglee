...
async function sendAmendmentForDeal(deal, input = {}) {
  ...
  await sendInvite({
    documentId,
    from: fromEmail,
    subject: `Amendment to Purchase Agreement — ${prop || 'Property'}`,
    message: `${SENDER.entity} has sent an amendment for your review and signature.`,
    to
  });
  ...
  return {
    ...
    sellers,
    invitees: to,
    message: `Amendment sent via SignNow to ${sellers.map((s) => s.email).join(', ')}. Buyer signature + ${sendDate} applied automatically.`
  };
}
...
async function sendDocumentForDeal(deal, kind, input = {}) {
  ...
  if (k === 'cash' || k === 'purchase_contract' || k === 'psa') return sendCashForDeal(deal, input);
