...
async function sendEmail({ contactId, subject, html, message, fromNumber }) {
...
async function ensureContactByEmail({ email, phone, name, firstName, lastName }) {
  ...
  const found = await searchContacts(em, { limit: 10 });
  const list = Array.isArray(found) ? found : [];
  const match = list.find((c) => String(c.email || '').trim().toLowerCase() === em) || list[0];
  if (match?.id) {
    // Optionally refresh phone if missing
    if (phone && !match.phone) {
      try {
        await api('PUT', `/contacts/${match.id}`, { phone: String(phone) });
      } catch (_) { /* best effort */ }
    }
    return match;
  }
