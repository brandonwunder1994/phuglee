...
function findLatestInboundMessage(messages = []) {
  ...
  for (let i = list.length - 1; i >= 0; i--) {
    ...
    if (!id && !body) continue;
    return {
      id: id || `inbound_${m.dateAdded || Date.now()}`,
      body,
      dateAdded: slugPart(m.dateAdded) || new Date().toISOString()
    };
  }
  return null;
}
...
function applySellerSmsInbound(existingSms, inbound, { checkedAt } = {}) {
  ...
  const next = { ...prev };
  if (checkedAt) next.lastCheckedAt = slugPart(checkedAt) || new Date().toISOString();
  if (inbound && inbound.id) {
    const same = prev.lastInboundId && prev.lastInboundId === inbound.id;
    if (!same) {
      next.lastInboundId = inbound.id;
      next.lastInboundAt = inbound.dateAdded || new Date().toISOString();
      next.lastInboundPreview = String(inbound.body || '').slice(0, 80);
    } else if (inbound.dateAdded && !prev.lastInboundAt) {
      next.lastInboundAt = inbound.dateAdded;
    }
  }
...
function recordSellerSmsFromMessages(dealId, messages) {
  ...
  return upsertDeal({ ...existing, sellerSms });
}
...
async function peekSellerSmsForOpenDeals() {
  ...
  await mapPool(candidates, SELLER_SMS_PEEK_CONCURRENCY, async (deal) => {
    ...
    try {
      ...
      human.sort((a, b) => new Date(a.dateAdded || 0) - new Date(b.dateAdded || 0));
...
function enrichDealForDisplay(deal, opts = {}) {
  ...
  return {
    ...
    buyerFound: (() => {
      ...
      if (stage === 'buyer_found' || stage === 'closing' || stage === 'funded') return 'yes';
