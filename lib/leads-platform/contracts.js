...
const { catalogRoot, getLead, setLeadCatalogStatus, upsertLead } = require('./store');
const {
  DEAL_STAGES,
  SALES_STAGES,
  DISPO_STAGES,
...
function normalizeSignerAlerts(raw) {
  if (!raw || typeof raw !== 'object' || Array.isArray(raw)) return {};
  const out = {};
  for (const [key, val] of Object.entries(raw)) {
    const email = slugPart(key).toLowerCase();
    if (!email || !val || typeof val !== 'object') continue;
    out[email] = {
      openedAlertedAt: slugPart(val.openedAlertedAt) || null,
      signedAlertedAt: slugPart(val.signedAlertedAt) || null,
      role: slugPart(val.role) || ''
    };
  }
  return out;
}
...
function normalizeSignNowPending(list) {
  ...
  return list.map((raw) => ({
    ...
    openedAt: slugPart(raw.openedAt) || null,
    openedByEmail: slugPart(raw.openedByEmail) || null,
    openedAlertedAt: slugPart(raw.openedAlertedAt) || null,
    signedAlertedAt: slugPart(raw.signedAlertedAt) || null,
    signerAlerts: normalizeSignerAlerts(raw.signerAlerts),
    invitees: Array.isArray(raw.invitees)
      ? raw.invitees.map((inv) => ({
        email: slugPart(inv?.email).toLowerCase(),
        role: slugPart(inv?.role),
        order: inv?.order == null ? null : Number(inv.order)
...
const DESK_LOCAL_KEYS = [
  ...
  // Money fields edited on the desk must not be clobbered by stale GHL custom fields.
  'purchasePrice',
  'assignmentFee',
  'emdDeposit',
  'ghlContactLocked',
...
/**
 * Parse money for deal storage. Non-numeric / empty-after-strip → null (never $0 from "N/A").
 * Zero is allowed only when the cleaned string is explicitly "0" / "0.00".
 */
...
/** Positive money required for legal docs (PSA/AOC). */
function requirePositiveMoney(value, fieldLabel) {
  const n = normalizeMoney(value);
  if (n == null || n <= 0) {
    const err = new Error(`${fieldLabel || 'Amount'} must be greater than $0`);
    err.code = 'MONEY_REQUIRED';
    throw err;
  }
  return n;
}

/** Cash purchase vs subject-to acquisition. Empty = unset. */
function normalizeDealType(raw) {
  const v = slugPart(raw).toLowerCase().replace(/[\s-]+/g, '_');
  if (v === 'cash' || v === 'cash_deal' || v === 'purchase_cash') return 'cash';
  if (
    v === 'subject_to'
    || v === 'subto'
    || v === 'sub_to'
...
function dealTypeLabel(dealType) {
  if (dealType === 'cash') return 'Cash deal';
  if (dealType === 'subject_to') return 'Subject-to deal';
  return '';
}

/** Text blob used to infer cash vs subject-to when dealType was never saved. */
function dealTypeSignalText(deal) {
  const parts = [];
  for (const d of Array.isArray(deal?.documents) ? deal.documents : []) {
    parts.push(d?.label, d?.name, d?.kind, d?.templateKey);
  }
  for (const p of Array.isArray(deal?.signNowPending) ? deal.signNowPending : []) {
    parts.push(p?.templateKey, p?.kind, p?.documentName, p?.label);
  }
  if (deal?.cashPsaSend) {
    parts.push('cash', deal.cashPsaSend.templateKey, deal.cashPsaSend.documentName);
  }
  if (deal?.subtoPsaSend || deal?.subjectToPsaSend) {
    const send = deal.subtoPsaSend || deal.subjectToPsaSend;
    parts.push('subto', send.templateKey, send.documentName);
  }
...
/**
 * Explicit dealType wins. Otherwise infer from PSA docs / SignNow sends.
 * Under Contract defaults to cash so list badges always show unless SubTo is detected.
 */
function resolveDealType(deal) {
  const explicit = normalizeDealType(deal?.dealType);
  if (explicit) return explicit;
  const signals = dealTypeSignalText(deal);
  if (
    /\bsub[_ -]?to\b/.test(signals)
    || /subject[_ -]?to/.test(signals)
    || signals.includes('subjectto')
    || signals.includes('subto')
  ) {
    return 'subject_to';
  }
  if (deal?.cashPsaSend || /\bcash\b/.test(signals)) return 'cash';
  if ((deal?.documents || []).some((d) => String(d?.kind || '') === 'purchase_contract')) {
    return 'cash';
  }
  return 'cash';
}
...
function normalizeDeal(raw = {}) {
  ...
  const deal = {
    ...
    stage,
    dealType: normalizeDealType(raw.dealType || raw.contractType || raw.purchaseType),
    purchasePrice,
    assignmentFee,
    profitOverride,
    profit: null,
    ...
    contractSellers: normalizeContractSellers(raw.contractSellers),
    ...
    cashPsaSend: normalizeSendState(raw.cashPsaSend),
    ...
    buyerPitches: normalizeBuyerPitches(raw.buyerPitches),
    buyerOffers: normalizeBuyerOffers(raw.buyerOffers),
    selectedBuyerOfferId: slugPart(raw.selectedBuyerOfferId) || null,
    walkthroughs: normalizeWalkthroughs(raw.walkthroughs),
    investorBaseUrl: normalizeInvestorBaseUrl(raw.investorBaseUrl),
...
function normalizeAlertFlags(raw) {
  ...
  return {
    underContract: Boolean(src.underContract),
...
const BUYER_PITCH_STATUSES = new Set(['waiting', 'passed', 'won']);

function normalizeBuyerPitches(list) {
  if (!Array.isArray(list)) return [];
  return list.map((row) => {
    if (!row || typeof row !== 'object') return null;
    const buyerId = slugPart(row.buyerId || row.fundId);
    if (!buyerId) return null;
    const status = slugPart(row.status).toLowerCase();
    if (!BUYER_PITCH_STATUSES.has(status)) return null;
    return {
      buyerId,
      buyerName: slugPart(row.buyerName || row.name),
      status,
      at: slugPart(row.at) || new Date().toISOString(),
      by: slugPart(row.by),
      note: slugPart(row.note)
    };
  }).filter(Boolean);
}

/**
 * Scheduled buyer walkthroughs on a deal.
 * @returns {{ id, buyerName, phone, email, at, createdAt, createdBy, dayOfNotifiedAt }[]}
 */
function normalizeWalkthroughs(list) {
  if (!Array.isArray(list)) return [];
  return list.map((row) => {
    if (!row || typeof row !== 'object') return null;
    const buyerName = slugPart(row.buyerName || row.name || row.buyer);
    const at = slugPart(row.at || row.datetime || row.when);
    if (!buyerName || !at) return null;
    const d = new Date(at);
    if (Number.isNaN(d.getTime())) return null;
    return {
      id: slugPart(row.id) || `wt_${crypto.randomBytes(6).toString('hex')}`,
      buyerName,
      phone: slugPart(row.phone || row.buyerPhone || row.contactPhone),
      email: slugPart(row.email || row.buyerEmail || row.contactEmail).toLowerCase(),
      at: d.toISOString(),
      createdAt: slugPart(row.createdAt) || new Date().toISOString(),
      createdBy: slugPart(row.createdBy),
      dayOfNotifiedAt: slugPart(row.dayOfNotifiedAt) || null
    };
  }).filter(Boolean).sort((a, b) => String(a.at).localeCompare(String(b.at)));
}

/** Local calendar YYYY-MM-DD in America/Phoenix (desk default). */
function calendarDayKey(isoOrDate, timeZone = 'America/Phoenix') {
...
/**
 * Upcoming walkthrough relative to "today" in desk TZ.
 * @returns {{ kind: 'today'|'tomorrow'|null, walkthrough: object|null }}
 */
function walkthroughUrgency(deal, now = new Date()) {
  const list = normalizeWalkthroughs(deal?.walkthroughs);
  if (!list.length) return { kind: null, walkthrough: null };
  const today = calendarDayKey(now);
  const tomorrowDate = new Date(now.getTime() + 24 * 60 * 60 * 1000);
  const tomorrow = calendarDayKey(tomorrowDate);
  const todayHit = list.find((w) => calendarDayKey(w.at) === today);
  if (todayHit) return { kind: 'today', walkthrough: todayHit };
  const tomHit = list.find((w) => calendarDayKey(w.at) === tomorrow);
  if (tomHit) return { kind: 'tomorrow', walkthrough: tomHit };
  return { kind: null, walkthrough: null };
}

/** Desk-local offer log on Under Contract profiles (separate from catalog buyerPitches). */
function normalizeBuyerOffers(list) {
  if (!Array.isArray(list)) return [];
  return list.map((row) => {
    if (!row || typeof row !== 'object') return null;
    const buyerName = slugPart(row.buyerName || row.name);
    const offerAmount = normalizeMoney(row.offerAmount ?? row.amount ?? row.price);
    if (!buyerName || offerAmount == null) return null;
    return {
      id: slugPart(row.id) || `bo_${crypto.randomBytes(6).toString('hex')}`,
      buyerName,
      offerAmount,
      phone: slugPart(row.phone || row.buyerPhone || row.contactPhone),
      email: slugPart(row.email || row.buyerEmail || row.contactEmail).toLowerCase(),
      catalogBuyerId: slugPart(row.catalogBuyerId) || null,
      ghlContactId: slugPart(row.ghlContactId) || null,
      updatedAt: slugPart(row.updatedAt) || new Date().toISOString(),
      updatedBy: slugPart(row.updatedBy)
    };
  }).filter(Boolean);
}

/** Marketing / Investor Base listing URL for a deal (desk-local). */
function normalizeInvestorBaseUrl(value) {
...
/** HTTPS GIF hosts allowed on team messages (Giphy / Tenor / Klipy CDNs). */
...
function normalizeGifAttachment(raw) {
  if (!raw) return null;
  if (typeof raw === 'string') raw = { url: raw };
  if (typeof raw !== 'object') return null;
  const url = slugPart(raw.url);
  if (!url) return null;
  let host = '';
  try {
    const u = new URL(url);
  ...
  if (!isAllowedGifHost(host)) return null;
  let previewUrl = slugPart(raw.previewUrl);
  if (previewUrl) {
    try {
      const p = new URL(previewUrl);
      if (p.protocol !== 'https:' || !isAllowedGifHost(p.hostname)) previewUrl = url;
    } catch (_) {
      previewUrl = url;
    }
  } else {
  ...
  return {
    url,
    previewUrl,
    title: slugPart(raw.title).slice(0, 160),
    provider: slugPart(raw.provider || 'giphy').slice(0, 32) || 'giphy',
    id: slugPart(raw.id).slice(0, 80)
  };
}
...
function normalizeTeamMessages(list) {
  ...
  return list.map((raw) => {
    ...
    const gif = normalizeGifAttachment(raw.gif || raw.attachment || null);
    const body = slugPart(raw.body);
    return {
      id: slugPart(raw.id) || `tm_${crypto.randomBytes(6).toString('hex')}`,
      fromUser,
      body: body || (gif ? (gif.title || 'GIF') : ''),
      gif,
      createdAt: slugPart(raw.createdAt) || new Date().toISOString(),
      readBy: {
...
const ACCESS_TYPES = new Set(['key', 'lockbox', 'door_code', 'open', 'seller_will_open']);
...
/** Display closing as M/D/YYYY (4-digit year for legal docs). */
...
function normalizeAccessType(value) {
  ...
  if (/seller[_\s-]*will[_\s-]*open|seller[_\s-]*opens?|owner[_\s-]*will[_\s-]*open/.test(v)) {
    return 'seller_will_open';
  }
...
function accessLabel(type) {
  return ({
    ...
    door_code: 'Door code',
    open: 'Open',
    seller_will_open: 'Seller Will Open'
  })[type] || '';
}
...
/** Up to two PSA / Amendment counterparties persisted on the deal. */
function normalizeContractSellers(raw) {
  if (!Array.isArray(raw)) return [];
  return raw
    .map((s) => ({
      name: slugPart(s && s.name),
      email: slugPart(s && s.email).toLowerCase()
    }))
...
/**
 * Resolve 1–2 sellers for Amendment / PSA prefill.
 * Prefer stored contractSellers, then cash PSA invitees + sellerNames, then owner fields.
 */
function resolveContractSellers(deal) {
  const stored = normalizeContractSellers(deal?.contractSellers);
  ...
  const cashPending = pending.find((p) => {
    const key = String(p?.templateKey || '').toLowerCase();
    const kind = String(p?.kind || '').toLowerCase();
    return key === 'cash' || kind === 'purchase_contract' || kind === 'cash' || kind === 'psa';
  });
  const invitees = Array.isArray(cashPending?.invitees) ? cashPending.invitees : [];
  if (invitees.length) {
    const fromInv = invitees
      .map((inv, i) => ({
        name: slugPart(inv.name) || names[i] || (i === 0 ? slugPart(deal?.ownerName) : ''),
        email: slugPart(inv.email).toLowerCase()
      }))
      .filter((s) => s.name && s.email)
      .slice(0, 2);
  ...
  const s1Name = slugPart(deal?.ownerName) || names[0] || '';
  const s1Email = slugPart(deal?.ownerEmail || deal?.email).toLowerCase();
  if (!s1Name || !s1Email) return [];
  const out = [{ name: s1Name, email: s1Email }];
  if (names[1]) {
    // Name known from PSA join string; email must be filled in the form.
    out.push({ name: names[1], email: '' });
  }
  return out;
}
...
function normalizeAocSend(raw) {
  ...
  return {
    ...
    signedAt: slugPart(raw.signedAt) || null,
    lastRemindedAt: slugPart(raw.lastRemindedAt) || null,
    lastRemindedBy: slugPart(raw.lastRemindedBy) || null,
    buyerName: slugPart(raw.buyerName || raw.assigneeName),
    buyerPhone: slugPart(raw.buyerPhone || raw.assigneePhone),
    buyerEmail: slugPart(raw.buyerEmail).toLowerCase(),
...
function indexEntryFromDeal(deal) {
  return {
    ...
    zip: deal.zip,
    stage: deal.stage,
    dealType: normalizeDealType(deal.dealType),
...
function enrichDealForDisplay(deal, opts = {}) {
  ...
  // Board list skips heavy arrays (docs/media/team) — full profile loads them on open.
  const documents = lite
    ? []
    : (Array.isArray(deal.documents) ? deal.documents : []).map((d) => ({
      ...d,
      viewUrl: `/api/leads/admin/contracts/${encodeURIComponent(deal.dealId)}/documents/${encodeURIComponent(d.id)}`
    }));
  const sellerMedia = lite ? [] : enrichSellerMediaForDisplay(deal);
  ...
  const result = {
    ...
    buyerFound: (() => {
      ...
      if (stage === 'buyer_signed_aoc' || stage === 'buyer_found' || stage === 'funded') return 'yes';
    ...
    dealType: resolveDealType(deal),
    dealTypeLabel: dealTypeLabel(resolveDealType(deal)),
    ...
    mediaZipUrl: lite
      ? undefined
      : `/api/leads/admin/contracts/${encodeURIComponent(deal.dealId)}/media/zip`,
    amendmentDefaults: lite
      ? undefined
      : {
        seller: resolveAmendmentDefaults(deal, null, 'seller'),
        end_buyer: resolveAmendmentDefaults(deal, null, 'end_buyer')
      },
    teamMessages: lite ? [] : normalizeTeamMessages(deal.teamMessages),
    sellerSms: lite
      ? {
        lastInboundAt: sms.lastInboundAt || null,
        lastInboundPreview: sms.lastInboundPreview || '',
        lastCheckedAt: sms.lastCheckedAt || null
      }
      : sms,
    sellerSmsPreview: sms.lastInboundPreview || '',
    sellerSmsAt: sms.lastInboundAt || null,
    sellerSmsUnread: opts.username
      ? isSellerSmsUnreadForUser(deal, opts.username)
      : false,
    lead: lite ? null : (lead || null)
  };

  if (lite) {
    // Drop bulky deal-file payloads the board never renders.
    delete result.documents;
    delete result.sellerMedia;
    delete result.teamMessages;
    delete result.lead;
    delete result.amendmentDefaults;
    delete result.mediaZipUrl;
    delete result.signNowPending;
    delete result.conditionScan;
    delete result.buyerPitches;
    delete result.photographerSchedule;
    if (Array.isArray(result.buyerOffers)) {
      result.buyerOffers = result.buyerOffers.map((o) => ({
        id: o && o.id,
        buyerName: o && o.buyerName
      }));
    }
  }
...
/**
 * Contract Tracker board = dispo stages + Waiting for Signatures (contract_sent).
 * Pipeline board = all deal stages. KPIs still use proofTotals (dispo-only).
 */
function filterDealsForBoard(deals, board = 'contracts') {
  ...
  return list.filter((d) => {
    ...
    return isDispoStage(stage) || stage === 'contract_sent';
  });
}

/**
 * Brad-safe card for pre-UC / Waiting for Signatures: address + imagery only.
 * Full enrich for admin or dispo-stage (UC+) deals.
 * @param {{ lite?: boolean }} [opts] — lite skips docs/media/team payload (board list).
 */
function projectDealForViewer(deal, username, opts = {}) {
  if (!deal) return null;
  const enriched = enrichDealForDisplay(deal, { username, lite: Boolean(opts.lite) });
  const isBrad = String(username || '').trim().toLowerCase() === 'brad';
  const stage = canonicalizeDealStage(enriched.stage);
  // Brad full desk only on UC+ — Waiting for Signatures is tracker glance only.
  if (!isBrad || isDispoStage(stage)) return enriched;
  return {
...
/** Board/list projection — same brad rules, slim payload. */
function projectDealForBoard(deal, username) {
  return projectDealForViewer(deal, username, { lite: true });
}

function listDealsForBoard(board, username) {
  const filtered = filterDealsForBoard(listDeals(), board);
  return filtered.map((d) => projectDealForBoard(d, username));
}
...
function assertBradCanWriteDeal(deal, username) {
  ...
  // Waiting for Signatures (contract_sent) and earlier sales stages — admin only.
  if (isSalesStage(stage)) {
    const err = new Error(
      stage === 'contract_sent'
        ? 'Waiting for Signatures is admin-only — disposition can view the tracker only'
        : 'Disposition partner cannot edit sales-stage deals'
    );
...
/** Per-deal serial queue — prevents last-write-wins between desk / SignNow / SMS / GHL. */
const dealWriteQueues = new Map();

function withDealWriteLock(dealId, fn) {
  const key = String(dealId || '_unknown');
  const prev = dealWriteQueues.get(key) || Promise.resolve();
  const run = prev
    .catch(() => {})
    .then(() => fn());
  dealWriteQueues.set(key, run.finally(() => {
    if (dealWriteQueues.get(key) === run) dealWriteQueues.delete(key);
  }));
  return run;
}
...
function upsertDeal(raw) {
  ...
  index.sort(compareDealsByProcess);
  writeContractsIndex(index);
  return merged;
}

/** Async upsert under per-deal queue (use from concurrent API/sync paths). */
async function upsertDealSafe(raw) {
  const id = raw && raw.dealId;
  if (!id) return upsertDeal(raw);
  return withDealWriteLock(id, () => upsertDeal(raw));
}
...
/**
 * Assignment fee for KPIs/boards.
 * Prefer selected (or highest) buyer offer − purchase lockup; else stored assignmentFee.
 */
function resolveDealAssignmentFee(deal) {
  if (!deal || typeof deal !== 'object') return null;
  const purchase = deal.purchasePrice == null || deal.purchasePrice === ''
    ? null
    : Number(deal.purchasePrice);
  const offers = Array.isArray(deal.buyerOffers) ? deal.buyerOffers.slice() : [];
  if (offers.length && purchase != null && Number.isFinite(purchase)) {
    ...
    if (!picked) {
      ...
      picked = offers[0] || null;
    }
    const amt = picked == null ? null : Number(picked.offerAmount);
    if (amt != null && Number.isFinite(amt)) return amt - purchase;
  }
  if (deal.assignmentFee == null || deal.assignmentFee === '') return null;
  const stored = Number(deal.assignmentFee);
...
function proofTotals(deals = listDeals()) {
  ...
  // openAssignmentFees = not-yet-funded only (Command Center projected)
  let openAssignmentFees = 0;
  // pendingAssignmentFees = ALL non-terminated dispo deals (Pending Assignments KPI)
  let pendingAssignmentFees = 0;
  ...
  // Waiting for Signatures (PSA sent, not yet signed) — feeds Pending Signatures KPI.
  let pendingSignatures = 0;
  // Count of funded deals with a real assignment fee — denominator for the average.
  let fundedFeeCount = 0;

  for (const d of list) {
    const stage = canonicalizeDealStage(d.stage);
    if (stage === 'contract_sent') pendingSignatures += 1;
    if (!isDispoStage(stage)) continue;
    if (byStage[stage] != null) byStage[stage] += 1;
    // Terminated never counts toward assignment money
    if (stage === 'terminated') continue;

    const fee = resolveDealAssignmentFee(d);
    const purchase = d.purchasePrice == null ? null : Number(d.purchasePrice);
    const photoCost = d.photoCost == null ? 0 : Number(d.photoCost);
    const p = computeDealPayouts(fee, photoCost, settings);
    const isClosed = stage === 'funded';

    // Pending Assignments: every live dispo deal (any stage except terminated)
    if (fee != null && Number.isFinite(fee)) pendingAssignmentFees += fee;

    if (isClosed) {
      if (fee != null && Number.isFinite(fee)) { closedAssignmentFees += fee; fundedFeeCount += 1; }
      ...
      // Projected / still-open pipeline only
  ...
  const openCount = byStage.under_contract + byStage.buyer_signed_aoc + byStage.buyer_found;
  const avgFundedAssignmentFee = fundedFeeCount > 0 ? closedAssignmentFees / fundedFeeCount : 0;
  return {
    dealCount: list.filter((d) => isDispoStage(canonicalizeDealStage(d.stage))).length,
    byStage,
    underContract: byStage.under_contract,
    buyerSignedAoc: byStage.buyer_signed_aoc,
    ...
    pendingSignatures,
    fundedFeeCount,
    avgFundedAssignmentFee,
    openCount,
    // Pending Assignments KPI: all non-terminated dispo assignment fees combined
    pendingAssignmentFees,
    // Current open pipeline only (not funded) — Command Center projected
    openAssignmentFees,
    openPurchasePrice,
    openTcPay,
    openAcqPay,
...
function createDealFromVaultLead(leadId, extras = {}) {
  ...
  const ownerName = extras.ownerName != null ? slugPart(extras.ownerName) : (lead.ownerName || existing?.ownerName || '');
  const ownerEmail = extras.ownerEmail != null
    ? slugPart(extras.ownerEmail).toLowerCase()
    : slugPart(existing?.ownerEmail || lead.email || existing?.email || '').toLowerCase();
  const email = extras.email != null
    ? slugPart(extras.email).toLowerCase()
    : (ownerEmail || lead.email || existing?.email || '');
  const deal = upsertDeal({
    ...
    ownerName,
    ownerEmail,
    sellerNames: extras.sellerNames != null ? slugPart(extras.sellerNames) : (existing?.sellerNames || ownerName || ''),
    contractSellers: extras.contractSellers != null
      ? normalizeContractSellers(extras.contractSellers)
      : (existing?.contractSellers || []),
    phone: extras.phone != null ? slugPart(extras.phone) : ((lead.phones && lead.phones[0]) || existing?.phone || ''),
    email,
    streetViewUrl: lead.streetViewUrl || existing?.streetViewUrl || '',
    satelliteUrl: lead.satelliteUrl || existing?.satelliteUrl || '',
    ...
    emdDeposit: extras.emdDeposit != null ? extras.emdDeposit : existing?.emdDeposit,
    notes: extras.notes != null ? extras.notes : existing?.notes,
    dealType: extras.dealType != null ? extras.dealType : existing?.dealType,
    source: existing?.source === 'ghl' ? 'ghl' : 'vault'
  });
  applyCatalogStatusForDeal(deal);
  fireDealTransitionAlerts(existing, deal).catch((err) => {
    console.warn('[team-notify] vault create alerts failed:', err.message);
  });
  return deal;
}

/**
 * Fill-blanks APN + legal description from REAPI onto linked lead and deal.aocSend.
 * Credit-safe: skips PropertyDetail when both fields already present.
 */
async function ensureDealContractParcel(dealId, opts = {}) {
  const {
    ensureContractParcelFields,
    readContractParcelFields,
    seedAocSendParcel,
    text: parcelText
  } = require('./contract-parcel');
  ...
  let lead = deal.leadId ? getLead(deal.leadId) : null;
  const subject = lead || {
    address: deal.address,
    city: deal.city,
    state: deal.state,
    zip: deal.zip,
    parcel: deal.aocSend?.apn,
    propertyDetails: {
  ...
  const current = readContractParcelFields(lead, deal);
  let reapi = opts.reapi;
  if (reapi === undefined) {
    try {
      ...
      reapi = apiKey
        ? createReapiClient({ apiKey, baseUrl: config.REALESTATE_API_BASE })
        : null;
    } catch (_) {
      reapi = null;
    }
  ...
  const out = await ensureContractParcelFields(subject, reapi, {
    force: opts.force === true,
    lead,
    deal
  });

  if (out.ok && out.lead && deal.leadId && (out.filled || []).length) {
    lead = upsertLead(out.lead);
  } else if (out.ok && out.lead && !deal.leadId) {
    lead = out.lead;
  }

  const fields = (() => {
    // Prefer freshly pulled detail over stale aocSend placeholders.
    if (out.detail && (parcelText(out.detail.apn) || parcelText(out.detail.legalDescription))) {
      return {
        apn: parcelText(out.detail.apn) || (out.fields && out.fields.apn) || current.apn,
        legalDescription: parcelText(out.detail.legalDescription)
          || (out.fields && out.fields.legalDescription)
          || current.legalDescription
      };
    }
    return out.fields || current;
  })();
  const seeded = seedAocSendParcel(deal, fields, { force: opts.force === true });
  if (seeded.changed) {
    deal = upsertDeal(seeded.deal);
  }
  ...
  return {
    ...
    pulled: Boolean(out.pulled),
    filled: out.filled || [],
    fields: readContractParcelFields(lead, deal),
    deal,
    lead,
    error: out.ok === false ? (out.error || null) : null,
    code: out.ok === false ? (out.code || null) : null
  };
}
...
function patchDeal(dealId, patch = {}) {
  ...
  const allowed = [
    'stage', 'dealType', 'purchasePrice', 'assignmentFee', 'profitOverride',
    ...
    'ownerEmail', 'sellerNames', 'contractSellers', 'cashPsaSend',
    ...
    'ghlContactLocked', 'photographerSchedule', 'conditionScan', 'buyerPitches', 'buyerOffers',
    'selectedBuyerOfferId', 'walkthroughs',
    'investorBaseUrl'
  ];
  const next = { ...existing };
  ...
  if (Object.prototype.hasOwnProperty.call(patch, 'dealType')) {
    next.dealType = normalizeDealType(patch.dealType);
  }
  ...
  if (Object.prototype.hasOwnProperty.call(patch, 'buyerPitches')) {
    next.buyerPitches = normalizeBuyerPitches(patch.buyerPitches);
  }
  if (Object.prototype.hasOwnProperty.call(patch, 'buyerOffers')) {
    next.buyerOffers = normalizeBuyerOffers(patch.buyerOffers);
  }
  // Buyer EMD submitted → advance into Buyer Submitted EMD stage (past AOC)
  if (Object.prototype.hasOwnProperty.call(patch, 'buyerEmdSubmitted')) {
    next.buyerEmdSubmitted = normalizeYesNo(patch.buyerEmdSubmitted);
    const emdYes = next.buyerEmdSubmitted === 'yes';
    const st = canonicalizeDealStage(next.stage);
    if (emdYes && (st === 'under_contract' || st === 'buyer_signed_aoc')) {
      next.stage = 'buyer_found';
    }
  }
  if (Object.prototype.hasOwnProperty.call(patch, 'selectedBuyerOfferId')) {
    const sid = slugPart(patch.selectedBuyerOfferId);
    next.selectedBuyerOfferId = sid || null;
  }
  if (Object.prototype.hasOwnProperty.call(patch, 'walkthroughs')) {
    next.walkthroughs = normalizeWalkthroughs(patch.walkthroughs);
  }
  // Never wholesale-replace documents from a partial client array — merge by id.
  if (Object.prototype.hasOwnProperty.call(patch, 'documents')) {
    if (Array.isArray(patch.documents) && patch.documents.length === 0 && !patch.replaceDocuments) {
      // Explicit empty without replace flag: keep existing vault
      next.documents = existing.documents || [];
    } else if (Array.isArray(patch.documents)) {
      next.documents = mergeDocuments(existing.documents || [], patch.documents);
    }
  }
  if (Object.prototype.hasOwnProperty.call(patch, 'investorBaseUrl')) {
    next.investorBaseUrl = normalizeInvestorBaseUrl(patch.investorBaseUrl);
  }
...
function addTeamMessage(dealId, { fromUser, body, gif } = {}) {
  const existing = getDeal(dealId);
  if (!existing) {
    const err = new Error('Deal not found');
    err.code = 'NOT_FOUND';
  ...
  const gifAtt = normalizeGifAttachment(gif);
  if (!text && !gifAtt) {
    const err = new Error('Message body or GIF required');
  ...
  const msg = {
    ...
    fromUser: user,
    body: text || (gifAtt ? (gifAtt.title || 'GIF') : ''),
    gif: gifAtt,
    createdAt: now,
    readBy: { admin: null, brad: null, [user]: now },
    reactions: emptyTeamReactions()
  };
  msg.readBy[user] = now;
  // Replying means the sender has seen the thread — clear their unread on this deal.
  const prior = normalizeTeamMessages(existing.teamMessages || []).map((m) => ({
    ...m,
    readBy: {
      ...m.readBy,
  ...
  const teamMessages = normalizeTeamMessages([...prior, msg]);
...
function markTeamMessagesRead(dealId, username) {
  ...
  const user = slugPart(username).toLowerCase() === 'brad' ? 'brad' : 'admin';
  const now = new Date().toISOString();
  // Always stamp "now" so open/click/reply reliably clears the banner even if
  // a prior empty-string readBy slipped through normalization edge cases.
  const teamMessages = normalizeTeamMessages(existing.teamMessages || []).map((m) => ({
    ...m,
    readBy: {
      admin: m.readBy?.admin || null,
      brad: m.readBy?.brad || null,
      [user]: now
    }
  }));
  const saved = upsertDeal({ ...existing, teamMessages });
...
/**
 * SMS + email Brandon & Brad once per walkthrough on its calendar day.
 * Persists dayOfNotifiedAt so reloads do not re-spam.
 */
async function processWalkthroughDayOfAlerts(deal) {
  if (!deal?.dealId) return { notified: [] };
  const list = normalizeWalkthroughs(deal.walkthroughs);
  if (!list.length) return { notified: [] };
  ...
  const next = [];
  for (const w of list) {
    if (calendarDayKey(w.at) !== today || w.dayOfNotifiedAt) {
      next.push(w);
      continue;
    }
    try {
      await notify.alertWalkthroughToday({ deal, walkthrough: w });
      next.push({ ...w, dayOfNotifiedAt: new Date().toISOString() });
      notified.push(w.id);
      changed = true;
    } catch (err) {
      console.warn('[walkthroughs] notify failed:', err.message);
      next.push(w);
    }
  }
  if (changed) {
    upsertDeal({ ...getDeal(deal.dealId), walkthroughs: normalizeWalkthroughs(next) });
  }
  return { notified };
}
...
async function fireDealTransitionAlerts(before, after) {
  ...
  // First time a deal lands on the Contract Tracker (dispo board) — Brad + Brandon.
  // Skips terminated-only landings; one-shot so GHL re-syncs do not re-spam.
  const nowOnTracker = isDispoStage(after.stage) && slugPart(after.stage) !== 'terminated';
  const wasOnTracker = before
    && isDispoStage(before.stage)
    && slugPart(before.stage) !== 'terminated';
  if (nowOnTracker && !wasOnTracker && !flags.underContract) {
    await notify.alertUnderContract({ deal: after }).catch(() => {});
    nextFlags.underContract = true;
    fired.push('underContract');
  }
  ...
  // Day-of walkthrough SMS/email (once per walkthrough row)
  try {
    const walkFired = await processWalkthroughDayOfAlerts(after);
    if (walkFired?.notified?.length) fired.push('walkthroughToday');
  } catch (err) {
    console.warn('[walkthroughs] day-of notify failed:', err.message);
  }
...
function isPurchaseContractDocument(doc) {
  if (!doc || typeof doc !== 'object') return false;
  const kind = String(doc.kind || '').toLowerCase();
  if (kind === 'purchase_contract' || kind === 'cash' || kind === 'psa' || kind === 'subto' || kind === 'subject_to') {
    return true;
  }
  const label = String(doc.label || '').toLowerCase();
  if (label === 'subto' || label === 'subject_to' || label === 'cash' || label === 'psa') return true;
  const name = String(doc.name || '').toLowerCase();
  return /\b(purchase|psa|sub\s*-?\s*to|subject\s*-?\s*to)\b/.test(name)
    && !/\b(amendment|addendum|assignment|aoc|jv)\b/.test(name);
}

/**
 * Resolve the original PSA signed date for amendments.
 * Prefer explicit deal/GHL fields, then signed send-state, then a PSA already in Documents.
 */
function resolveOriginalAgreementDate(deal, contact = null) {
  const candidates = [
    ...
    deal?.subjectToPsaSend?.signedAt
  ];

  for (const raw of candidates) {
    const formatted = formatClosingSlash(raw);
    if (formatted) return formatted;
  }

  const pending = Array.isArray(deal?.signNowPending) ? deal.signNowPending : [];
  for (const item of pending) {
    const key = String(item?.templateKey || '').toLowerCase();
    const kind = String(item?.kind || '').toLowerCase();
    const status = String(item?.status || '').toLowerCase();
    const isPsa = key === 'cash' || key === 'subto' || key === 'subject_to'
      || kind === 'purchase_contract' || kind === 'cash' || kind === 'psa' || kind === 'subto';
    if (!isPsa) continue;
    if (status !== 'signed' && status !== 'completed' && status !== 'fulfilled') continue;
    const formatted = formatClosingSlash(
      item.signedAt || item.completedAt || item.openedAt || item.lastCheckedAt || item.sentAt
    );
    if (formatted) return formatted;
  }

  const psaDocs = (deal?.documents || []).filter(isPurchaseContractDocument);
  if (psaDocs.length) {
    // Prefer earliest upload — usually when the signed PSA was attached.
    const sorted = psaDocs.slice().sort((a, b) =>
      String(a.uploadedAt || '').localeCompare(String(b.uploadedAt || ''))
    );
    for (const doc of sorted) {
      const formatted = formatClosingSlash(doc.signedAt || doc.uploadedAt);
      if (formatted) return formatted;
    }
  }
...
/** Persist inferred PSA date once so Amendment form + future sends stay unblocked. */
function ensureOriginalAgreementDate(dealId, contact = null) {
  const existing = getDeal(dealId);
  if (!existing) return null;
  if (formatClosingSlash(existing.originalAgreementDate)) return existing;
  const inferred = resolveOriginalAgreementDate(existing, contact);
  if (!inferred) return existing;
  return setOriginalAgreementDateFromPsa(dealId, inferred) || existing;
}

/**
 * Download a signed SignNow PSA into deal documents and fill desk PSA fields.
 */
async function attachSignedSignNowPsa(dealId, input = {}) {
  const existing = getDeal(dealId);
  if (!existing) {
    const err = new Error('Deal not found');
    err.code = 'NOT_FOUND';
  ...
  const documentId = slugPart(input.documentId || input.signNowDocumentId);
  if (!documentId) {
    const err = new Error('SignNow documentId required');
    err.code = 'MISSING_SIGNNOW_DOCUMENT';
    throw err;
  }

  const already = (existing.documents || []).find(
    (d) => d.signNowDocumentId === documentId || d.id === `sn_${documentId}`
  );

  let document = already || null;
  let deal = existing;
  if (!already) {
    const { downloadDocumentPdf, isSignNowConfigured } = require('./signnow-client');
    if (!isSignNowConfigured()) {
      const err = new Error('SignNow is not configured (SIGNNOW_ACCESS_TOKEN)');
      err.code = 'SIGNNOW_NOT_CONFIGURED';
      throw err;
    }
    const buf = await downloadDocumentPdf(documentId);
    const dealType = normalizeDealType(input.dealType || existing.dealType);
    const label = dealType === 'subject_to'
      ? 'Subject-To Purchase Agreement'
      : (dealType === 'cash' ? 'Cash Purchase Agreement' : 'Purchase Agreement');
    const result = addDealDocument(dealId, {
      id: `sn_${documentId}`,
      ...
      label: dealType === 'subject_to' ? 'subto' : (dealType || 'purchase_contract')
    });
    deal = result.deal;
  ...
  const patch = { ...deal };
  const dealType = normalizeDealType(input.dealType || deal.dealType);
  if (dealType) patch.dealType = dealType;

  if (input.purchasePrice != null && input.purchasePrice !== '') {
    patch.purchasePrice = normalizeMoney(input.purchasePrice);
  }
  if (input.emdDeposit != null && input.emdDeposit !== '') {
    patch.emdDeposit = normalizeMoney(input.emdDeposit);
  }
  if (slugPart(input.sellerName)) {
    patch.ownerName = slugPart(input.sellerName);
    patch.sellerNames = slugPart(input.sellerName);
  }
  if (slugPart(input.sellerEmail)) {
    const email = slugPart(input.sellerEmail).toLowerCase();
    patch.ownerEmail = email;
    if (!slugPart(patch.email)) patch.email = email;
  }

  const apn = slugPart(input.apn);
  const legalDescription = slugPart(input.legalDescription || input.legal);
  if (apn || legalDescription) {
    patch.aocSend = normalizeAocSend({
      ...(deal.aocSend || {}),
      status: (deal.aocSend && deal.aocSend.status) || 'template_pending',
      apn: apn || deal.aocSend?.apn || '',
      legalDescription: legalDescription || deal.aocSend?.legalDescription || ''
    });
  }

  deal = upsertDeal(patch);

  if (slugPart(input.signedDate || input.originalAgreementDate)) {
    deal = setOriginalAgreementDateFromPsa(
      deal.dealId,
      input.signedDate || input.originalAgreementDate
    ) || deal;
  }

  return { deal: getDeal(deal.dealId), document };
}
...
function resolveAmendmentDefaults(deal, contact = null, partyType = 'seller') {
  ...
  const originalAgreementDate = resolveOriginalAgreementDate(deal, contact);
  ...
  if (party === 'end_buyer') {
    ...
    if (counterpartyName && counterpartyEmail) {
      sellers = [{ name: counterpartyName, email: counterpartyEmail }];
    }
  } else {
    sellers = resolveContractSellers(deal);
    if (!sellers.length) {
      counterpartyName = slugPart(
        deal?.ownerName
          || deal?.sellerNames
      ...
      // Fill missing seller-1 email from GHL contact when PSA only left a name join.
      if (!sellers[0].email) {
        sellers[0].email = slugPart(contact?.email || '').toLowerCase();
      }
...
async function requestAocSend(dealId, input = {}, actor = '') {
  let existing = healDealAddressFromLinkedLead(dealId) || getDeal(dealId);
  if (!existing) {
    const err = new Error('Deal not found');
    err.code = 'NOT_FOUND';
    throw err;
  ...
  const buyerName = slugPart(input.buyerName || input.assigneeName);
  const buyerPhone = slugPart(input.buyerPhone || input.assigneePhone);
  if (!buyerName || !buyerPhone) {
    const err = new Error('Buyer name and phone are required');
    err.code = 'MISSING_BUYER_INFO';
    throw err;
  }

  let legalDescription = slugPart(input.legalDescription);
  let apn = slugPart(input.apn);
  if (!legalDescription || !apn) {
    try {
      const ensured = await ensureDealContractParcel(dealId);
      const fields = ensured.fields || {};
      if (!apn) apn = slugPart(fields.apn);
      if (!legalDescription) legalDescription = slugPart(fields.legalDescription);
      if (ensured.deal) existing = ensured.deal;
      else {
        const refreshed = getDeal(dealId);
        if (refreshed) existing = refreshed;
      }
    } catch (_) {
      /* ensure is best-effort; SignNow missing-field check remains */
    }
  }
  if (!apn) {
    const lead = existing.leadId ? getLead(existing.leadId) : null;
    apn = slugPart(lead?.propertyDetails?.apn || lead?.parcel || existing.aocSend?.apn);
  }
  if (!legalDescription) {
    const lead = existing.leadId ? getLead(existing.leadId) : null;
    legalDescription = slugPart(
      lead?.propertyDetails?.legalDescription || existing.aocSend?.legalDescription
    );
  }
...
/**
 * Remind unsigned parties on the existing AOC SignNow document.
 */
async function requestAocReminder(dealId, actor = '') {
  ...
  const aoc = existing.aocSend && typeof existing.aocSend === 'object' ? existing.aocSend : null;
  if (!aoc?.signNowDocumentId) {
    const err = new Error('Send the AOC first before sending a reminder');
    err.code = 'AOC_NOT_SENT';
    throw err;
  }
  try {
    const { remindAocUnsigned } = require('./signnow-send');
    const reminder = await remindAocUnsigned(existing);
    let aocSend = normalizeAocSend({
      ...aoc,
      lastRemindedAt: new Date().toISOString(),
      lastRemindedBy: slugPart(actor) || null,
      status: reminder.status === 'signed' ? 'signed' : (aoc.status || 'sent'),
      signedAt: reminder.status === 'signed' ? (aoc.signedAt || new Date().toISOString()) : aoc.signedAt
    });
    // Preserve lastReminded* even if normalizeAocSend strips unknown fields — re-merge after.
    aocSend = {
      ...aocSend,
      lastRemindedAt: new Date().toISOString(),
      lastRemindedBy: slugPart(actor) || null
    };
    const saved = upsertDeal({ ...existing, aocSend });
    return { deal: saved, reminder };
  } catch (err) {
    const wrapped = new Error(err.message || 'AOC reminder failed');
    wrapped.code = err.code || 'SIGNNOW_REMIND_FAILED';
    wrapped.deal = existing;
    throw wrapped;
  }
}
...
async function markBuyerFound(dealId, input = {}, actor = '') {
  ...
  let aocSend = normalizeAocSend({
    ...
    requestedAt: new Date().toISOString(),
    requestedBy: slugPart(actor) || null,
    buyerName: buyerContactName || buyerEntity,
    buyerPhone,
    buyerEmail,
    buyerEmd: input.buyerEmd,
    closingDate,
  ...
  // Persist buyer fields first; advance to buyer_signed_aoc after AOC send
  // succeeds (or skipSignNow). EMD stage (buyer_found) is separate.
  let saved = upsertDeal({
    ...existing,
    // keep prior stage until AOC path succeeds
  ...
  if (skipSignNow) {
    ...
    saved = upsertDeal({ ...saved, stage: 'buyer_signed_aoc', aocSend });
    applyCatalogStatusForDeal(saved);
    return {
      deal: saved,
...
/**
 * Confirm + complete JV between hardwired Brandon + Brad via SignNow.
 * Auto-signs both parties, downloads PDF, imports into the deal Documents list.
 */
async function requestJvSend(dealId, input = {}, actor = '') {
  ...
  try {
    ...
    const snClient = require('./signnow-client');
    const jv = await sendJvForDeal(saved);
    const signedAt = jv.signedAt || new Date().toISOString();
    ...
    // Import signed PDF into deal Documents immediately (no invite wait).
    let imported = null;
    const already = (saved.documents || []).some(
      (d) => d.signNowDocumentId === jv.documentId || d.id === `sn_${jv.documentId}`
    );
    if (!already) {
      const buf = await snClient.downloadDocumentPdf(jv.documentId);
      const baseName = slugPart(jv.documentName) || `JV for ${saved.address || saved.dealId}`;
      const result = addDealDocument(saved.dealId, {
        id: `sn_${jv.documentId}`,
        kind: 'jv',
      ...
      // Re-assert signed status after document merge.
      jvAgreement = normalizeJvAgreement({
        ...(saved.jvAgreement || jvAgreement),
        status: 'signed',
        signedAt,
        signNowDocumentId: jv.documentId,
        lastError: null
    ...
    return {
      ...
      jv: {
        ...
        message: jv.message || 'JV signed and imported into this deal.'
      }
    };
  } catch (err) {
    jvAgreement = normalizeJvAgreement({
...
async function requestAmendmentSend(dealId, input = {}, actor = '') {
  ...
  // Pull seller email/name from GHL contact page when deal store is missing them.
  let contact = input.contact || null;
  if (partyType === 'seller' && existing.ghlContactId) {
    const hasEmail = Boolean(slugPart(
      input.sellerEmail || input.counterpartyEmail || existing.ownerEmail || existing.email
    ));
    if (!hasEmail || !contact) {
      try {
        const ghl = require('./ghl-client');
        if (ghl.isConfigured()) {
          const raw = await ghl.getContact(existing.ghlContactId);
          contact = ghl.summarizeContactMoney(raw);
        }
      } catch (_) { /* optional — form/deal email may still work */ }
    }
  }

  // Heal date from PSA in Documents / SignNow signed state when the desk field is empty.
  const healed = ensureOriginalAgreementDate(existing.dealId, contact) || existing;
  const dealForDefaults = healed.dealId ? healed : existing;
  const defaults = resolveAmendmentDefaults(dealForDefaults, contact, partyType);

  // Always use PSA / GHL / document-inferred signed date — never trust a client override
  const originalAgreementDate = slugPart(defaults.originalAgreementDate);
  if (!originalAgreementDate) {
    const dealType = resolveDealType(dealForDefaults);
    const hint = dealType === 'subject_to'
      ? 'Attach the signed SubTo in Documents (or set the PSA signed date) before sending an amendment.'
      : 'Sync from GHL, attach the signed PSA in Documents, or open the profile so the signed date can load first.';
    const err = new Error(`Original PSA agreement date is missing on this deal. ${hint}`);
    err.code = 'MISSING_ORIGINAL_DATE';
    throw err;
  }

  let sellers = Array.isArray(input.sellers)
    ? input.sellers
      .map((s) => ({
        name: slugPart(s && s.name),
        email: slugPart(s && s.email).toLowerCase()
      ...
      .filter((s) => s.name && s.email)
    : [];
  const sellerCount = Number(input.sellerCount) || sellers.length || 1;
  if (sellerCount >= 2 && sellers.length < 2) {
    const err = new Error('Seller 2 name + email required when sending to two sellers');
    err.code = 'MISSING_SELLER_2';
    throw err;
  }
  if (sellerCount === 1) sellers = sellers.slice(0, 1);
  ...
  const contractSellers = normalizeContractSellers(sellers);

  let amendmentSend = normalizeSendState({
    ...(dealForDefaults.amendmentSend || {}),
    status: 'sending',
    requestedAt: new Date().toISOString(),
    requestedBy: slugPart(actor) || null
  });
  ...
  const patchFields = {
    // Keep PSA date on the deal once inferred so later amendment opens stay unlocked.
    originalAgreementDate
  };
  if (partyType === 'seller') {
    patchFields.ownerName = sellers[0].name;
    patchFields.ownerEmail = sellers[0].email;
    patchFields.email = sellers[0].email;
    patchFields.sellerNames = sellerNames;
    patchFields.contractSellers = contractSellers;
  }
  ...
  try {
    ...
    pushSignNowPending(saved.dealId, {
      ...
      kind: 'amendment',
      status: 'sent',
      invitees: Array.isArray(amd.invitees) ? amd.invitees : sellers.map((s, i) => ({
        email: s.email,
        role: s.role || (i === 0 ? 'Seller 1' : 'Seller 2'),
        order: i + 1
      })),
      signerAlerts: {}
    });
    ...
    return {
      ...
      amendment: {
        ...
        message: amd.message,
        sellers: amd.sellers,
        // Actual SignNow invitees only (Buyer/Wunderhaus already stamped — not invited)
        invitees: Array.isArray(amd.invitees) ? amd.invitees : undefined
      }
    };
  } catch (err) {
    amendmentSend = normalizeSendState({
...
/**
 * Send Cash PSA on an existing deal (retry / Documents panel).
 */
async function requestCashPsaSend(dealId, input = {}, actor = '') {
  const existing = healDealAddressFromLinkedLead(dealId) || getDeal(dealId);
  if (!existing) {
    const err = new Error('Deal not found');
    err.code = 'NOT_FOUND';
  ...
  if (!sellers.length) {
    const name = slugPart(input.sellerName || existing.ownerName || existing.sellerNames);
    const email = slugPart(input.sellerEmail || existing.ownerEmail || existing.email).toLowerCase();
    if (name && email) sellers = [{ name, email }];
  }
  sellers = sellers
    .map((s) => ({ name: slugPart(s.name), email: slugPart(s.email).toLowerCase() }))
    .filter((s) => s.name && s.email);
  if (!sellers.length) {
    const err = new Error('Seller name + email required to send a cash purchase agreement');
    err.code = 'MISSING_SELLERS';
    throw err;
  }
  ...
  if (purchasePrice == null) {
    const err = new Error('Purchase price is required to send a cash purchase agreement');
    err.code = 'MISSING_PURCHASE_PRICE';
    throw err;
  }

  const closingDate = slugPart(input.closingDate) || existing.closingDate || '';
  const emdDeposit = input.emdDeposit != null && input.emdDeposit !== ''
    ? normalizeMoney(input.emdDeposit)
    : (existing.emdDeposit != null ? existing.emdDeposit : 100);
  const sellerNames = sellers.map((s) => s.name).join(' / ');
  const contractSellers = normalizeContractSellers(sellers);

  let cashPsaSend = normalizeSendState({
    ...(existing.cashPsaSend || {}),
    status: 'sending',
    requestedAt: new Date().toISOString(),
    requestedBy: slugPart(actor) || null,
    signedAt: null,
  ...
  let saved = upsertDeal({
    ...existing,
    purchasePrice,
    closingDate: closingDate || existing.closingDate,
    emdDeposit: emdDeposit != null ? emdDeposit : 100,
    ownerName: sellers[0].name,
    ownerEmail: sellers[0].email,
    email: sellers[0].email || existing.email,
    sellerNames,
    contractSellers,
    cashPsaSend
  });
  ...
  try {
    await ensureDealContractParcel(saved.dealId, { force: false }).catch(() => null);
    saved = healDealAddressFromLinkedLead(saved.dealId) || getDeal(saved.dealId) || saved;
    const { sendCashForDeal } = require('./signnow-send');
    const psa = await sendCashForDeal(saved, {
      ...input,
      sellers,
      purchasePrice,
      emdDeposit,
      legalDescription: input.legalDescription || saved.aocSend?.legalDescription
    });
    cashPsaSend = normalizeSendState({
      ...cashPsaSend,
      status: 'sent',
      signNowDocumentId: psa.documentId,
      lastError: null
    });
    // Cash PSA via SignNow ⇒ mark deal type for board + UC alerts
    saved = upsertDeal({ ...saved, cashPsaSend, dealType: 'cash' });
    pushSignNowPending(saved.dealId, {
      documentId: psa.documentId,
      documentName: psa.documentName,
      templateKey: 'cash',
      kind: 'purchase_contract',
      status: 'sent',
      invitees: Array.isArray(psa.invitees) ? psa.invitees : (psa.sellers || []).map((s, i) => ({
        email: s.email,
        role: s.role || (i === 0 ? 'Seller 1' : 'Seller 2'),
        order: i + 1
      })),
    ...
    saved = upsertDeal({ ...saved, cashPsaSend });
    const wrapped = new Error(err.message || 'Cash PSA SignNow send failed');
    wrapped.code = err.code || 'SIGNNOW_SEND_FAILED';
    wrapped.deal = saved;
    throw wrapped;
  }
...
/**
 * Create (or reuse) a Vault-linked deal at contract_sent and send Cash or Subject-to PSA.
 */
async function requestSendNewPsa(input = {}, actor = '') {
  const leadId = slugPart(input.leadId);
  if (!leadId) {
    const err = new Error('leadId required');
    err.code = 'MISSING_LEAD';
    throw err;
  }
  const lead = getLead(leadId);
  if (!lead) {
  ...
  const dealType = normalizeDealType(input.dealType) || 'cash';
  if (dealType !== 'cash' && dealType !== 'subject_to') {
    const err = new Error('Choose Cash or Subject-to before sending a purchase agreement');
    err.code = 'MISSING_DEAL_TYPE';
    throw err;
  }
  ...
  sellers = sellers
    ...
    .filter((s) => s.name && s.email);
  if (!sellers.length) {
    const err = new Error('At least one seller name + email is required');
    err.code = 'MISSING_SELLERS';
    throw err;
  }
  const sellerCount = Number(input.sellerCount) || sellers.length;
  if (sellerCount >= 2 && sellers.length < 2) {
    const err = new Error('Seller 2 name + email required when sending to two sellers');
    err.code = 'MISSING_SELLER_2';
    throw err;
  ...
  const existing = findDealByLeadId(leadId);
  if (existing) {
    const stage = canonicalizeDealStage(existing.stage);
    if (stage === 'under_contract' || stage === 'buyer_signed_aoc' || stage === 'buyer_found' || stage === 'funded') {
      const err = new Error(
        'This lead already has an Under Contract deal. Open that deal to send documents.'
      );
      err.code = 'DEAL_ALREADY_UNDER_CONTRACT';
      err.deal = existing;
      throw err;
    }
  }
  ...
  const sellerNames = sellers.map((s) => s.name).join(' / ');
  const deal = createDealFromVaultLead(leadId, {
    stage: 'contract_sent',
    dealType,
    purchasePrice,
    closingDate: slugPart(input.closingDate),
    emdDeposit: input.emdDeposit,
    ownerName: sellers[0].name,
    ownerEmail: sellers[0].email,
    email: sellers[0].email,
    sellerNames,
    contractSellers: sellers,
    notes: input.notes
  });

  try {
    if (dealType === 'subject_to') {
      return await requestSubtoPsaSend(deal.dealId, {
        ...input,
        dealType: 'subject_to',
        sellers,
        purchasePrice,
    ...
    return await requestCashPsaSend(deal.dealId, {
      ...input,
      dealType: 'cash',
      sellers,
      purchasePrice,
    ...
    // Deal stays at contract_sent for retry — Vault lead already hidden.
    throw err;
  }
}

/**
 * Send Subject-To PSA via SignNow based on deal fields.
 */
async function requestSubtoPsaSend(dealId, input = {}, actor = '') {
  const existing = healDealAddressFromLinkedLead(dealId) || getDeal(dealId);
  if (!existing) {
    const err = new Error('Deal not found');
    err.code = 'NOT_FOUND';
  ...
  try {
    const { sendSubtoForDeal } = require('./signnow-send');
    const psa = await sendSubtoForDeal({ ...existing, dealType: 'subject_to' }, input);
    pushSignNowPending(existing.dealId, {
      documentId: psa.documentId,
      documentName: psa.documentName,
      templateKey: 'subto',
      kind: 'purchase_contract',
    ...
    const wrapped = new Error(err.message || 'SubTo PSA SignNow send failed');
    wrapped.code = err.code || 'SIGNNOW_SEND_FAILED';
    wrapped.deal = getDeal(dealId);
    throw wrapped;
  }
}
...
async function requestDocumentSend(dealId, kind, input = {}, actor = '') {
  ...
  if (k === 'subto' || k === 'subject_to') {
    return requestSubtoPsaSend(dealId, input, actor);
  }
  if (k === 'cash') {
    return requestCashPsaSend(dealId, input, actor);
  }
  if (k === 'purchase_contract' || k === 'psa') {
    const existing = getDeal(dealId);
    const dealType = normalizeDealType(input.dealType) || resolveDealType(existing || {});
    if (dealType === 'subject_to') return requestSubtoPsaSend(dealId, input, actor);
    return requestCashPsaSend(dealId, input, actor);
  }
...
/**
 * Re-queue SignNow docs from amendmentSend / aocSend / jvAgreement when the
 * pending row was lost but the send state still has a document id.
 */
function rehydrateSignNowPendingFromSendStates(deal) {
  if (!deal?.dealId) return deal;
  const known = new Set(
    (deal.signNowPending || []).map((p) => String(p.documentId || p.signNowDocumentId || '').trim()).filter(Boolean)
  );
  const inDocs = new Set(
    (deal.documents || [])
      .map((d) => String(d.signNowDocumentId || '').trim())
      .filter(Boolean)
  );
  const additions = [];

  function consider(sendState, templateKey, kind) {
    const documentId = String(sendState?.signNowDocumentId || '').trim();
    if (!documentId) return;
    if (known.has(documentId) || inDocs.has(documentId)) return;
    const status = String(sendState.status || '').toLowerCase();
    if (status === 'signed' && inDocs.has(documentId)) return;
    additions.push({
      documentId,
      documentName: sendState.documentName || null,
      templateKey,
      kind,
      status: status === 'signed' ? 'signed' : 'sent',
      sentAt: sendState.requestedAt || sendState.sentAt || new Date().toISOString()
    });
    known.add(documentId);
  }

  consider(deal.amendmentSend, 'amendment', 'amendment');
  consider(deal.aocSend, 'aoc', 'aoc');
  // JV is auto-signed + imported — do not put it in the awaiting-signatures queue.
  consider(deal.cashPsaSend, 'cash', 'purchase_contract');

  if (!additions.length) return deal;
  return pushSignNowPendingMany(deal.dealId, additions) || getDeal(deal.dealId) || deal;
}

function pushSignNowPendingMany(dealId, entries) {
  const existing = getDeal(dealId);
  if (!existing) return null;
  const pending = normalizeSignNowPending([
    ...(existing.signNowPending || []),
...
/**
 * Match SignNow account library docs to this deal by address in the document name.
 * Recovers packages that were signed in SignNow but never written to signNowPending
 * (e.g. send UI succeeded in SignNow but local pending was lost, or desk never open).
 */
async function rehydrateSignNowPendingFromLibrary(deal, libraryDocs = null) {
  const { isSignNowConfigured, listRecentDocuments, getDocument, isDocumentFullySigned } = require('./signnow-client');
  const {
    inferTemplateKeyFromDocName,
    signNowDocNameMatchesDeal,
    kindFromTemplateKey
  } = require('./signnow-send');

  if (!deal?.dealId || !isSignNowConfigured()) return deal;
  if (!slugPart(deal.address)) return deal;

  let docs = libraryDocs;
  if (!Array.isArray(docs)) {
    try {
      docs = await listRecentDocuments(60);
    } catch (err) {
      console.warn('[signnow] library list failed:', err.message);
      return deal;
    }
  }
  ...
  for (const row of docs) {
    const documentId = String(row?.id || row?.document_id || '').trim();
    const documentName = String(row?.document_name || row?.name || '').trim();
    if (!documentId || !documentName) continue;
    if (known.has(documentId) || inDocs.has(documentId)) continue;
    if (!signNowDocNameMatchesDeal(documentName, deal)) continue;
    const templateKey = inferTemplateKeyFromDocName(documentName);
    // Library recovery is for counterparty packages that must land on the profile.
    // Limit to amendment so we don't spam historic AOC/JV signed alerts on first boot.
    if (templateKey !== 'amendment') continue;
    matches.push({ documentId, documentName, templateKey });
  }

  if (!matches.length) return deal;

  // Prefer the newest fully-signed (or invited) package per kind.
  const byKind = new Map();
  for (const m of matches) {
    let fullySigned = false;
    let hasInvites = false;
    try {
      const snDoc = await getDocument(m.documentId);
      fullySigned = isDocumentFullySigned(snDoc);
      hasInvites = Array.isArray(snDoc?.field_invites) && snDoc.field_invites.length > 0;
    } catch (_) {
      continue;
    }
    // Skip abandoned template copies with no invites (double-click leftovers).
    if (!fullySigned && !hasInvites) continue;
    const prev = byKind.get(m.templateKey);
    if (!prev || (fullySigned && !prev.fullySigned)) {
      byKind.set(m.templateKey, { ...m, fullySigned });
    }
  }

  const additions = [...byKind.values()].map((m) => ({
    documentId: m.documentId,
    documentName: m.documentName,
    templateKey: m.templateKey,
    kind: kindFromTemplateKey(m.templateKey),
    status: m.fullySigned ? 'signed' : 'sent',
    sentAt: new Date().toISOString(),
    recoveredFromLibrary: true
  }));

  if (!additions.length) return deal;
  console.log(
    `[signnow] rehydrated ${additions.length} pending for ${deal.dealId}:`,
    additions.map((a) => `${a.kind}:${a.documentId.slice(0, 10)}`).join(', ')
  );
  return pushSignNowPendingMany(deal.dealId, additions) || getDeal(deal.dealId) || deal;
}

async function ensureSignNowPendingForDeal(deal, { libraryDocs = null, scanLibrary = false } = {}) {
  let next = rehydrateSignNowPendingFromSendStates(deal);
  if (scanLibrary) {
    next = await rehydrateSignNowPendingFromLibrary(next, libraryDocs);
  }
  return next;
}

function isPsaDocument(d) {
  if (!d || typeof d !== 'object') return false;
  const k = String(d.kind || '').toLowerCase();
  const lab = String(d.label || d.name || '').toLowerCase();
  if (k === 'purchase_contract' || k === 'psa' || k === 'cash') return true;
  if (lab === 'cash' || lab === 'purchase_contract' || lab === 'psa') return true;
  if (/purchase\s*agreement|cash\s*psa|\bpsa\b/.test(lab) && !/\b(amendment|aoc|jv)\b/.test(lab)) {
    return true;
  }
  return false;
}

/** True when signed Purchase Agreement is imported into deal Documents. */
function dealHasImportedSignedPsa(deal) {
  if (!deal) return false;
  const docs = Array.isArray(deal.documents) ? deal.documents : [];
  const psaDoc = docs.find((d) => isPsaDocument(d));
  if (!psaDoc) return false;
  const psaStatus = String(deal.cashPsaSend?.status || deal.subtoPsaSend?.status || '').toLowerCase();
  // Strong path: SignNow marked PSA signed.
  if (psaStatus === 'signed') return true;
  // Imported from SignNow (or labeled purchase) even if send-state lagged.
  if (psaDoc.source === 'signnow' || psaDoc.signNowDocumentId) return true;
  return false;
}

function dealNeedsAutoJv(deal) {
  if (!dealHasImportedSignedPsa(deal)) return false;
  const jv = deal.jvAgreement && typeof deal.jvAgreement === 'object' ? deal.jvAgreement : {};
  const st = String(jv.status || '').toLowerCase();
  if (st === 'signed' || st === 'sending') return false;
  if (jv.signNowDocumentId) return false;
  // Already have a JV PDF on the deal
  const hasJvDoc = (Array.isArray(deal.documents) ? deal.documents : []).some((d) => {
    const k = String(d?.kind || d?.label || '').toLowerCase();
    return k === 'jv' || k === 'jv_agreement';
  });
  if (hasJvDoc) return false;
  return true;
}

/**
 * After signed PSA is imported into Documents, auto-complete JV (Brandon + Brad).
 * Idempotent — skips if JV already signed/imported.
 */
async function maybeAutoJvAfterSignedPsa(dealId, actor = 'system') {
  const deal = getDeal(dealId);
  if (!deal) return { skipped: true, reason: 'not_found' };
  if (!dealNeedsAutoJv(deal)) {
    return { skipped: true, reason: 'not_eligible' };
  }
  try {
    const out = await requestJvSend(
      dealId,
      { notes: 'Auto JV after signed Purchase Agreement imported', auto: true },
      actor || 'system'
    );
    console.log('[jv] auto after signed PSA:', dealId, out.jv?.documentId || 'ok');
    return { skipped: false, deal: out.deal, jv: out.jv };
  } catch (err) {
    console.warn('[jv] auto after signed PSA failed:', dealId, err.message);
    // Persist error on jvAgreement without throwing — PSA import still succeeded.
    try {
      const cur = getDeal(dealId);
      if (cur) {
        upsertDeal({
          ...cur,
          jvAgreement: normalizeJvAgreement({
            ...(cur.jvAgreement || {}),
            status: 'error',
            lastError: `Auto JV after PSA: ${err.message}`,
            requestedAt: new Date().toISOString(),
    ...
    return { skipped: false, error: err.message, code: err.code };
  }
}

async function syncSignedSignNowDocuments(dealId, opts = {}) {
  let existing = getDeal(dealId);
  if (!existing) {
    const err = new Error('Deal not found');
    err.code = 'NOT_FOUND';
    throw err;
  }
  existing = await ensureSignNowPendingForDeal(existing, {
    libraryDocs: opts.libraryDocs || null,
    scanLibrary: Boolean(opts.scanLibrary)
  });
  let out;
  if (!(existing.signNowPending || []).length) {
    out = { deal: existing, ingested: 0, pending: [], added: [], alerts: [] };
  } else {
    const { ingestSignedSignNowDocuments } = require('./signnow-send');
    out = await ingestSignedSignNowDocuments(existing, {
      addDealDocument,
      upsertDeal,
      patchPending: setSignNowPending
    });
  }
  // Cash PSA sign promotes contract_sent → under_contract — keep Vault catalog in sync.
  if (out?.deal) {
    applyCatalogStatusForDeal(out.deal);
    out.deal = getDeal(dealId) || out.deal;
  }
  // Signed PSA in Documents → auto-complete JV (no manual Complete JV needed).
  if (out?.deal && dealNeedsAutoJv(out.deal)) {
    const jvOut = await maybeAutoJvAfterSignedPsa(dealId, opts.actor || 'system');
    if (jvOut?.deal) out.deal = jvOut.deal;
    out.autoJv = jvOut;
  }
  return out;
}
...
const SIGNNOW_LIBRARY_SCAN_TTL_MS = 5 * 60 * 1000;
let signNowLibraryScanAt = 0;
let signNowBgTimer = null;
let signNowBgRunning = false;

/**
 * Auto-import fully signed SignNow packages into deal documents for any deal
 * with pending invites. Periodically rescans the SignNow library so packages
 * that lost their pending row still land on the property profile + fire alerts.
 * Throttled so board polls stay cheap.
 */
async function syncPendingSignNowAcrossDeals(opts = {}) {
  const { isSignNowConfigured, listRecentDocuments } = require('./signnow-client');
  if (!isSignNowConfigured()) return { checked: 0, ingested: 0, deals: 0, rehydrated: 0 };

  const now = Date.now();
  const forceLibrary = Boolean(opts.forceLibraryScan);
  const scanLibrary = forceLibrary || (now - signNowLibraryScanAt >= SIGNNOW_LIBRARY_SCAN_TTL_MS);
  let libraryDocs = null;
  if (scanLibrary) {
    try {
      libraryDocs = await listRecentDocuments(60);
      signNowLibraryScanAt = now;
    } catch (err) {
      console.warn('[signnow] background library scan failed:', err.message);
    }
  }

  // Always rehydrate send-state ids; library scan is periodic.
  const openDeals = listDeals().filter((d) => {
    const stage = slugPart(d.stage);
    return stage && stage !== 'dead' && stage !== 'lost';
  });

  let rehydrated = 0;
  for (const deal of openDeals) {
    const before = (deal.signNowPending || []).length;
    const next = await ensureSignNowPendingForDeal(deal, {
      libraryDocs,
      scanLibrary: Boolean(libraryDocs)
    });
    if (((next.signNowPending || []).length) > before) rehydrated += 1;
  }

  const candidates = listDeals().filter((d) => {
    if (!(d.signNowPending || []).length) return false;
    const mem = signNowSyncMemory.get(d.dealId) || 0;
    if (!opts.force && now - mem < SIGNNOW_SYNC_TTL_MS) return false;
    return true;
  });
  if (!candidates.length) {
    return { checked: 0, ingested: 0, deals: 0, rehydrated };
  }
  ...
  await mapPool(candidates, SIGNNOW_SYNC_CONCURRENCY, async (deal) => {
    ...
    try {
      checked += 1;
      const out = await syncSignedSignNowDocuments(deal.dealId, { scanLibrary: false });
  ...
  return { checked, ingested, deals: dealsTouched, rehydrated };
}

/**
 * Server-side poll so opened/signed alerts and PDF attach do not require
 * someone to keep Contract Tracker open in a browser.
 */
function startSignNowBackgroundSync(opts = {}) {
  if (signNowBgTimer) return { already: true };
  const ms = Math.max(30000, Number(opts.intervalMs) || 60000);
  const tick = async () => {
    if (signNowBgRunning) return;
    signNowBgRunning = true;
    try {
      const out = await syncPendingSignNowAcrossDeals();
      if (out.ingested || out.rehydrated) {
        console.log(
          `[signnow] bg sync ingested=${out.ingested} rehydrated=${out.rehydrated} checked=${out.checked}`
        );
      }
    } catch (err) {
      console.warn('[signnow] bg sync failed:', err.message);
    } finally {
      signNowBgRunning = false;
    }
  };
  // First pass shortly after boot so a redeploy recovers already-signed packages.
  setTimeout(() => { tick(); }, 8000);
  signNowBgTimer = setInterval(tick, ms);
  if (typeof signNowBgTimer.unref === 'function') signNowBgTimer.unref();
  console.log(`[signnow] background sync every ${Math.round(ms / 1000)}s`);
  return { started: true, intervalMs: ms };
}

function stopSignNowBackgroundSync() {
  if (signNowBgTimer) {
    clearInterval(signNowBgTimer);
    signNowBgTimer = null;
  }
}
...
function addDealDocument(dealId, input = {}) {
  ...
  const patch = { ...existing, documents };
  // Uploading a signed PSA unlocks amendments even when GHL signed date never synced.
  if (isPurchaseContractDocument(doc) && !formatClosingSlash(existing.originalAgreementDate)) {
    const seeded = formatClosingSlash(
      input.signedDate || input.originalAgreementDate || doc.uploadedAt
    );
    if (seeded) patch.originalAgreementDate = seeded;
  }
  const saved = upsertDeal(patch);
...
function assertSafeExternalMediaUrl(url) {
  let parsed;
  try {
  ...
  if (
    ...
    err.code = 'MEDIA_HOST_BLOCKED';
    throw err;
  }
  // Optional allowlist via env (comma-separated host suffixes); when unset, block only private nets.
  const allow = String(process.env.MEDIA_PROXY_ALLOW_HOSTS || '')
    .split(',')
  ...
  if (allow.length) {
    ...
    if (!ok) {
      const err = new Error('Media URL host not on allowlist');
      err.code = 'MEDIA_HOST_BLOCKED';
      throw err;
    }
  }
...
async function fetchSellerMediaBytes(url) {
  const target = assertSafeExternalMediaUrl(url);
  let lastErr = null;
  try {
    const ghl = require('./ghl-client');
  ...
  try {
    ...
    if (res.url) {
      try { assertSafeExternalMediaUrl(res.url); } catch (blocked) {
        const e = new Error(blocked.message || 'Redirect host blocked');
        e.code = blocked.code || 'MEDIA_HOST_BLOCKED';
        throw e;
      }
    }
    return {
...
module.exports = {
  ...
  requestAocReminder,
  requestJvSend,
  maybeAutoJvAfterSignedPsa,
  dealHasImportedSignedPsa,
  dealNeedsAutoJv,
  ...
  startSignNowBackgroundSync,
  stopSignNowBackgroundSync,
  rehydrateSignNowPendingFromSendStates,
  rehydrateSignNowPendingFromLibrary,
  ensureSignNowPendingForDeal,
  pushSignNowPending,
  setSignNowPending,
  setOriginalAgreementDateFromPsa,
  resolveOriginalAgreementDate,
  ensureOriginalAgreementDate,
  resolveAmendmentDefaults,
  resolveContractSellers,
  normalizeContractSellers,
  addTeamMessage,
  markTeamMessagesRead,
  listUnreadTeamForUser,
  toggleTeamMessageReaction,
  normalizeGifAttachment,
  isAllowedGifHost,
  TEAM_REACTION_KEYS,
  TEAM_REACTION_EMOJI,
  ...
  normalizeRehabInfo,
  rehabInfoFilled,
  normalizeBuyerOffers,
  normalizeWalkthroughs,
  calendarDayKey,
  walkthroughUrgency,
  processWalkthroughDayOfAlerts,
  normalizeInvestorBaseUrl,
