...
function projectDealForViewer(deal, username) {
  ...
  // Brad sees full desk on UC+ and Waiting for Signatures (PSA pending).
  if (!isBrad || isDispoStage(stage) || stage === 'contract_sent') return enriched;
  return {
...
function assertBradCanWriteDeal(deal, username) {
  ...
  // Waiting for Signatures is on Contract Tracker — Brad may work those deals.
  if (isSalesStage(stage) && stage !== 'contract_sent') {
