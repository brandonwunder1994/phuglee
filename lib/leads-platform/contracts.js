...
async function markBuyerFound(dealId, input = {}, actor = '') {
  ...
  // Persist buyer fields first; only advance stage to buyer_found after AOC send
  // succeeds (or skipSignNow). Keeps board truthful when SignNow fails.
  ...
  if (skipSignNow) {
    ...
    saved = upsertDeal({ ...saved, stage: 'buyer_found', aocSend });
    applyCatalogStatusForDeal(saved);
    return {
      deal: saved,
...
async function requestSendNewPsa(input = {}, actor = '') {
  ...
  if (existing) {
    ...
    if (stage === 'under_contract' || stage === 'buyer_found' || stage === 'funded') {
      const err = new Error(
...
async function syncSignedSignNowDocuments(dealId, opts = {}) {
  ...
  if (!(existing.signNowPending || []).length) {
    return { deal: existing, ingested: 0, pending: [], added: [], alerts: [] };
  }
  const { ingestSignedSignNowDocuments } = require('./signnow-send');
  const out = await ingestSignedSignNowDocuments(existing, {
    addDealDocument,
    upsertDeal,
