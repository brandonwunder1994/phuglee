...
async function requestAmendmentSend(dealId, input = {}, actor = '') {
  ...
  const defaults = resolveAmendmentDefaults(existing, contact, partyType);
  ...
  if (!originalAgreementDate) {
    const dealType = resolveDealType(existing);
    const hint = dealType === 'subject_to'
      ? 'Set the PSA signed date or attach the signed SubTo before sending an amendment.'
      : 'Sync from GHL or open the profile so the signed date can load first.';
