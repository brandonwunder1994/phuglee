...
async function labelDealMedia(dealId, { force = false, runScan = true } = {}) {
  ...
  for (const item of media) {
    ...
    try {
      ...
      const out = await labelImageBuffer(buf, item.mimeType || 'image/jpeg');
      if (!out.ok) {
        failedN += 1;
        continue;
      }
      const nextMedia = contracts.normalizeSellerMediaList(deal.sellerMedia).map((m) => {
        if (m.id !== item.id) return m;
        return { ...m, aiLabel: out.label };
      });
      // refresh deal pointer
  ...
  jobState.set(dealId, {
    status: 'ready',
    error: failedN && !labeledN ? 'Label failures' : null,
    labeledN,
    failedN,
