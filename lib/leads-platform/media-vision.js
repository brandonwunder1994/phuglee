  ...
  for (const model of GEMINI_MODELS) {
    ...
    for (let attempt = 0; attempt < 3; attempt++) {
      try {
        ...
        lastErr = `HTTP ${result.status} ${String(result.body || '').slice(0, 160)}`;
        const hardQuota = result.status === 429 && /quota|billing|plan/i.test(lastErr);
        if (hardQuota) {
          return { ok: false, error: lastErr, code: 'GEMINI_QUOTA', hardQuota: true, status: 429 };
        }
        const retryable = result.status === 429 || result.status === 503 || result.status === 500;
        // 404 model missing — skip to next model immediately
...
async function labelDealMedia(dealId, { force = false, runScan = true } = {}) {
  ...
  for (const item of media) {
    ...
    try {
      ...
      if (!out.ok && out.hardQuota) {
        failedN += 1;
        lastError = out.error || 'Gemini quota exceeded';
        console.warn('[media-vision] hard quota — stopping batch', dealId, lastError);
        break;
      }
      if (!out.ok && /429|rate|503|overload/i.test(String(out.error || '')) && !out.hardQuota) {
      ...
      if (!out.ok) {
        ...
        if (out.hardQuota || /quota|billing/i.test(String(lastError))) break;
