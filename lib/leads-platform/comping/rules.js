...
/** Hard size band: larger of ±10% and ±250 sf (small homes starve under pure %). */
function sizeHardDelta(subSqft) {
  const sqft = Number(subSqft) || 0;
  return Math.max(Math.round(sqft * 0.10), 250);
}

/** Soft edge: larger of ±6.7% and ±100 sf (prefer nearby size). */
function sizeSoftDelta(subSqft) {
  const sqft = Number(subSqft) || 0;
  return Math.max(Math.round(sqft * 0.067), 100);
}
...
function ruleSizeBand(subject, candidate) {
  ...
  const absDiff = Math.abs(candSqft - subSqft);
  const hard = sizeHardDelta(subSqft);
  const soft = sizeSoftDelta(subSqft);
  if (absDiff > hard) {
    return {
      id: 'size_band',
      status: 'fail',
      detail: `Sqft ${candSqft} outside ±${hard} of ${subSqft}`,
    };
  }
  if (absDiff > soft) {
    return {
      id: 'size_band',
      status: 'soft',
      detail: `Sqft ${candSqft} near edge of ±${hard} band`,
    };
  }
  return { id: 'size_band', status: 'pass', detail: `Sqft ${candSqft} within ±${soft} of ${subSqft}` };
}
