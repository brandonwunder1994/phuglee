...
const { mapReapiCompsResponse, formatReapiAddress } = require('./reapi-client');
...
function buildLeadComps(scoredComps, arvIncluded) {
  ...
  return scoredComps.map((scored) => {
    ...
    const key = compKey(c);
    const address = formatReapiAddress(c.address);
    return {
      address,
      soldDate: c.soldDate,
      price: c.price,
      ...
      streetViewUrl: streetViewUrl({ lat: c.lat, lng: c.lng, address }),
    };
  });
}
...
/**
 * Optional MLS actives pull for soft/hot market tag.
 *
 * OFF by default — PropertySearch bills ~1 credit PER RECORD returned.
 * size:25 was burning up to ~25 credits per Comp click for a soft DOM signal.
 * Set COMPING_FETCH_ACTIVES=1 only when you explicitly accept that cost.
 */
async function fetchActives(reapi, subject) {
  if (String(process.env.COMPING_FETCH_ACTIVES || '').trim() !== '1') {
    return [];
  }
  ...
  try {
    const res = await reapi.propertySearch({
      ...
      radius: 1.0,
      mls_active: true,
      // Keep tiny if ever re-enabled — never 25 again without a budget gate.
      size: 5,
    });
