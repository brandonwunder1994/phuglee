...
async function runConcentricLoop(subject, reapi, opts) {
  ...
  for (const step of radiiPlan) {
    ...
    const body = {
      ...
      longitude: subject.lng,
      radius,
      bedrooms_min: Math.max(0, subject.beds - 1),
      bedrooms_max: subject.beds + 1,
      bathrooms_min: Math.max(0, subject.baths - 1),
      bathrooms_max: subject.baths + 1,
    };
    if (subject.sqft > 0) {
      body.living_square_feet_min = Math.round(subject.sqft * 0.9);
      body.living_square_feet_max = Math.round(subject.sqft * 1.1);
    }
    if (subject.yearBuilt > 0) {
      body.year_built_min = subject.yearBuilt - 10;
      body.year_built_max = subject.yearBuilt + 10;
    }
...
async function runAutoComp(lead, opts = {}) {
  ...
  if (reapi.propertyDetail) {
    try {
      detail = await reapi.propertyDetail({
        address: lead.address,
        city: lead.city,
        state: lead.state,
        latitude: lead.lat,
  ...
  const report = {
    ...
    arv: confidence === 'blocked' ? null : arvResult.arv,
    ...
    sanity: {
      ...
      notes: detail.estimatedValue
        ? ['Vendor AVM shown for sanity only — not used as ARV']
        : [],
    },
  ...
  const leadPatch = {
    ...
    estARV: confidence === 'blocked' ? null : arvResult.arv,
  };
