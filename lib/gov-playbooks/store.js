...
function playbookCountFromFile(filePath) {
  try {
    if (!fs.existsSync(filePath)) return 0;
    const raw = JSON.parse(fs.readFileSync(filePath, 'utf8'));
    return Array.isArray(raw.playbooks) ? raw.playbooks.length : 0;
  } catch (_) {
    return 0;
  }
}
...
/**
 * Ensure a runtime catalog exists. Prefer the richer seed when production
 * only has the tiny bootstrap seed (research playbooks used to live only
 * in gitignored catalog.json and never reached Railway).
 */
function ensureSeeded() {
  ...
  if (!fs.existsSync(seed)) {
    if (!fs.existsSync(dest)) writeJsonAtomic(dest, emptyCatalog());
    return;
  }

  const seedCount = playbookCountFromFile(seed);
  const destCount = playbookCountFromFile(dest);

  // Missing runtime, or seed is clearly the researched catalog and dest is the stub.
  if (!fs.existsSync(dest) || (seedCount > destCount && destCount < 50 && seedCount >= 50)) {
