...
describe('analyze concurrency policy', () => {
  it('keeps a 5–10 worker band and avoids global fleet freezes', () => {
    const configSrc = fs.readFileSync(path.join(root, 'public/js/config.js'), 'utf8');
    assert.match(configSrc, /MAX_SAFE_CONCURRENT\s*=\s*10/);
    ...
    assert.match(geminiSrc, /GEMINI_MAX_CONCURRENT\s*=\s*10/);
