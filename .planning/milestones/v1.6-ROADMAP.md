...
**Phases:** 42–47
**Total Plans:** 12
**Audit:** passed (24/24) — see v1.6-MILESTONE-AUDIT.md

## Overview

Admin-only global Filter brain — grouped Approve/Deny trains type + phrase rules so every future city upload improves for all customers.
...
- ðŸ“‹ **v1.6 Filter Superpower Brain** â€” Phases 42â€“47 (planned â€” GSD roadmap)

## Active Work

**Active milestone:** v1.6 Filter Superpower Brain (M7)

**Goal:** Admin-only global Filter brain â€” grouped Approve/Deny trains type + phrase rules so every future city upload improves for all customers.

**Execute only when user says go:** `/gsd:plan-phase 42` then execute 42â†’47
...
### v1.6 Filter Superpower Brain (Phases 42â€“47)

- [x] **Phase 42: Brain store + runtime apply** â€” Global durable brain file applied on every process
- [x] **Phase 43: Review payload + grouping** â€” Full FN rows, type groups, signals, stable rowIds
- [x] **Phase 44: Admin Train brain UX** â€” Two train sections, group âœ“/âœ—, admin-only chrome
- [x] **Phase 45: Decisions + type rules + list mutation** â€” Mutate current list + live type learning
- [x] **Phase 46: Phrase mining + brain panel** â€” Proposed phrases + admin rule management
- [x] **Phase 47: Hardening + metrics + docs** â€” Undo, caps, metrics, docs, QA green

### Phase dependency

```text
42 â†’ 43 â†’ 44 â†’ 45 â†’ 46 â†’ 47
```
...
**Goal:** A durable global Filter brain exists and active type rules change tagging outcomes on every upload for all users â€” without suppressing water shut-off.
...
**Goal:** After process, admins (and the API) have a reviewable false-negative pool and stacked violation-type groups with signals â€” not only thin discard previews.
...
**Success Criteria** (what must be TRUE):
  ...
  3. Deny writes an active global `suppress_type` rule; Approve writes an active global `promote_type` rule â€” next process for any user respects them
...
Plans:
- [x] 45-01-PLAN.md â€” Pure applyDecision matrix + type rules + audit events (DEC-01â€“05 TDD)
- [x] 45-02-PLAN.md â€” requireAdmin + POST /api/bridge/brain/decisions (DEC-06 + persist)
- [x] 45-03-PLAN.md â€” Client Train Approve/Deny wire + lastResult re-render
...
Plans:
- [x] 46-01-PLAN.md â€” Pure phrase miner + decisions hook; proposed never auto-live (PHRASE-01, PHRASE-02)
- [x] 46-02-PLAN.md â€” Admin brain API + Filter brain panel activate/reject/disable (PHRASE-03)
...
Plans:
- [x] 47-01-PLAN.md â€” Caps, version 409, split undo, metrics, train polish, TAGGING-RULES, phase gate (HARD-01â€“04)
...
Phases 37â€“41 delivered territory/theater product surfaces. Closed before M7 planning.
...
| 24 | Home â€” Signature Rebuild | complete |
| 25 | Auth Flows | complete |
| 26 | Shell Pages | complete |
| 27 | Form Forge â€” Signature Pass | complete |
| 28 | Analyzer â€” Signature Pass | complete |
...
*Roadmap updated: 2026-07-09 â€” v1.6 Filter Superpower Brain phases 42â€“47 (GSD roadmapper)*
