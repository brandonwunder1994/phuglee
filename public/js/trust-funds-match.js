/**
 * Trust Funds buy-box matcher v2
...
(function (root) {
  ...
  function rankFunds(funds, deal, opts) {
    ...
    rows.sort((a, b) => {
      if (landFirst && dealHasInputs(deal)) {
        const aLand = (a.fund.strategyClusters || []).includes('land') ? 1 : 0;
        const bLand = (b.fund.strategyClusters || []).includes('land') ? 1 : 0;
        if (aLand !== bLand && (a.tier === TIER_STRONG || b.tier === TIER_STRONG)) {
          // Prefer land Strong over house when scoring land deals, but still by tier first
        }
      }
