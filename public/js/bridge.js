...
(function () {
  ...
  const geocodioFileInput = document.getElementById('bridge-geocodio-file');
  const geocodioStartBtn = document.getElementById('bridge-geocodio-start');
  const geocodioFileNameEl = document.getElementById('bridge-geocodio-file-name');
  const geocodioStatusEl = document.getElementById('bridge-geocodio-status');
  const geocodioProgressEl = document.getElementById('bridge-geocodio-progress');
  const geocodioProgressBar = document.getElementById('bridge-geocodio-progress-bar');
  const geocodioDownloadsEmpty = document.getElementById('bridge-geocodio-downloads-empty');
  const geocodioDownloadsWrap = document.getElementById('bridge-geocodio-downloads-wrap');
  const geocodioDownloadsBody = document.getElementById('bridge-geocodio-downloads-body');
  const geocodioUsageBtn = document.getElementById('bridge-geocodio-usage-btn');
  const geocodioUsageDialog = document.getElementById('bridge-geocodio-usage-dialog');
  const geocodioUsageClose = document.getElementById('bridge-geocodio-usage-close');
  const geocodioUsageRefresh = document.getElementById('bridge-geocodio-usage-refresh');
  const geocodioUsageBody = document.getElementById('bridge-geocodio-usage-body');
  const geocodioUsageTotals = document.getElementById('bridge-geocodio-usage-totals');
  ...
  /* —— Geocodio clean —— */
  function setGeocodioStatus(msg, kind) {
    if (!geocodioStatusEl) return;
    geocodioStatusEl.textContent = msg || '';
    geocodioStatusEl.classList.remove('is-error', 'is-success');
    if (kind === 'error') geocodioStatusEl.classList.add('is-error');
    if (kind === 'success') geocodioStatusEl.classList.add('is-success');
  }

  function stopGeocodioPoll() {
    if (geocodioPollTimer) {
      clearInterval(geocodioPollTimer);
      geocodioPollTimer = null;
    }
  }

  function formatGeocodioWhen(iso) {
    if (!iso) return '—';
    try {
  ...
  function geocodioStatusPill(status) {
    const s = String(status || 'unknown').toLowerCase();
    const cls =
      s === 'complete' ? 'is-complete'
        : s === 'partial' ? 'is-partial'
          : s === 'running' || s === 'queued' ? 'is-running'
            : s === 'failed' ? 'is-failed'
              : '';
    return `<span class="bridge-geocodio-status-pill ${cls}">${esc(s)}</span>`;
  }

  function renderGeocodioJobs(jobs) {
    const list = Array.isArray(jobs) ? jobs : [];
    if (!geocodioDownloadsBody) return;
    if (!list.length) {
      setHidden(geocodioDownloadsEmpty, false);
      setHidden(geocodioDownloadsWrap, true);
      geocodioDownloadsBody.innerHTML = '';
      return;
    }
    setHidden(geocodioDownloadsEmpty, true);
    setHidden(geocodioDownloadsWrap, false);
    geocodioDownloadsBody.innerHTML = list.map((job) => {
      const id = esc(job.id || '');
      const canDl = job.status === 'complete' || job.status === 'partial';
      const actions = [];
      if (canDl) {
        actions.push(
          `<button type="button" class="phuglee-btn phuglee-btn-secondary phuglee-btn-sm" data-geo-dl="${id}">Download</button>`
        );
      }
      actions.push(
        `<button type="button" class="phuglee-btn phuglee-btn-secondary phuglee-btn-sm bridge-list-action--danger" data-geo-del="${id}">Delete</button>`
      );
      return `<tr data-geo-job="${id}">
        <td>${esc(formatGeocodioWhen(job.createdAt))}</td>
  ...
  async function refreshGeocodioJobs() {
    try {
      ...
      if (active) {
        ...
        const pct = active.inputRows
          ...
          : 0;
        if (geocodioProgressEl) setHidden(geocodioProgressEl, false);
        if (geocodioProgressBar) geocodioProgressBar.style.width = `${pct}%`;
        const keyBit = active.currentKeyEmail
          ? ` · ${active.currentKeyEmail}`
          : '';
        setGeocodioStatus(
          active.message
            || `${active.status}: ${active.processed || 0} / ${active.inputRows || 0} · kept ${active.kept || 0}${keyBit}`
        );
        if (!geocodioPollTimer) {
          geocodioPollTimer = setInterval(() => {
      ...
      setGeocodioStatus(err.message || 'Could not load Geocodio jobs', 'error');
      throw err;
    }
  }

  async function startGeocodioJob() {
    if (!geocodioSelectedFile) {
      setGeocodioStatus('Choose a CSV or Excel file first.', 'error');
      return;
    }
    if (geocodioStartBtn) geocodioStartBtn.disabled = true;
    setGeocodioStatus('Uploading…');
    if (geocodioProgressEl) setHidden(geocodioProgressEl, false);
    if (geocodioProgressBar) geocodioProgressBar.style.width = '2%';
    try {
      const form = new FormData();
      form.append('file', geocodioSelectedFile, geocodioSelectedFile.name);
      const res = await fetch('/api/bridge/geocodio/jobs', {
        method: 'POST',
        headers: bridgeHeaders(),
        body: form,
        cache: 'no-store'
  ...
  async function downloadGeocodioJob(jobId) {
    const res = await fetch(`/api/bridge/geocodio/jobs/${encodeURIComponent(jobId)}/download`, {
      headers: bridgeHeaders(),
      cache: 'no-store'
    });
    if (!res.ok) {
    ...
    const filename = match?.[1] || `geocodio-clean-${jobId}.csv`;
    const url = URL.createObjectURL(blob);
  ...
  async function deleteGeocodioJobUi(jobId) {
    if (!window.confirm('Delete this Geocodio result?')) return;
    await fetchJson(`/api/bridge/geocodio/jobs/${encodeURIComponent(jobId)}`, {
      method: 'DELETE'
    });
    await refreshGeocodioJobs();
  }

  async function openGeocodioUsageModal() {
    if (!geocodioUsageDialog) return;
    if (typeof geocodioUsageDialog.showModal === 'function') {
      geocodioUsageDialog.showModal();
    } else {
      geocodioUsageDialog.setAttribute('open', '');
    }
    await refreshGeocodioUsage();
  }

  function closeGeocodioUsageModal() {
    if (!geocodioUsageDialog) return;
    if (typeof geocodioUsageDialog.close === 'function') geocodioUsageDialog.close();
    else geocodioUsageDialog.removeAttribute('open');
  }
  ...
  geocodioFileInput?.addEventListener('change', () => {
    const file = geocodioFileInput.files && geocodioFileInput.files[0];
    geocodioSelectedFile = file || null;
    if (geocodioFileNameEl) {
      if (file) {
        geocodioFileNameEl.textContent = file.name;
        setHidden(geocodioFileNameEl, false);
      } else {
        setHidden(geocodioFileNameEl, true);
      }
    }
    if (geocodioStartBtn) geocodioStartBtn.disabled = !file;
  });

  geocodioStartBtn?.addEventListener('click', () => {
    startGeocodioJob().catch((e) => setGeocodioStatus(e.message, 'error'));
  });

  geocodioDownloadsBody?.addEventListener('click', (event) => {
    const t = event.target;
    if (!(t instanceof Element)) return;
    const dl = t.closest('[data-geo-dl]');
    if (dl) {
      const id = dl.getAttribute('data-geo-dl');
      downloadGeocodioJob(id).catch((e) => setGeocodioStatus(e.message, 'error'));
      return;
    }
    const del = t.closest('[data-geo-del]');
    if (del) {
      const id = del.getAttribute('data-geo-del');
      deleteGeocodioJobUi(id).catch((e) => setGeocodioStatus(e.message, 'error'));
    }
  });

  geocodioUsageBtn?.addEventListener('click', () => {
    openGeocodioUsageModal().catch((e) => setGeocodioStatus(e.message, 'error'));
  });
  geocodioUsageClose?.addEventListener('click', () => closeGeocodioUsageModal());
  geocodioUsageRefresh?.addEventListener('click', () => {
    refreshGeocodioUsage().catch((e) => setGeocodioStatus(e.message, 'error'));
  });
  geocodioUsageDialog?.addEventListener('click', (event) => {
    if (event.target === geocodioUsageDialog) closeGeocodioUsageModal();
  });
